...
{
    public partial class frmBase_User_Log : clsForm_Extends
    {
        ...
        {
            string strSql = " Where T_U_ID <> ''  ";
            
          

            //기록일자로 검색 -1
            if ((mtxtMakDate1.Text.Replace("-", "").Trim() != "") && (mtxtMakDate2.Text.Replace("-", "").Trim() == ""))
                strSql = strSql + " And Left( Job_Time,10) = '" + mtxtMakDate1.Text.Trim() + "'";

            //기록일자로 검색 -2
            if ((mtxtMakDate1.Text.Replace("-", "").Trim() != "") && (mtxtMakDate2.Text.Replace("-", "").Trim() != ""))
            {
                strSql = strSql + " And Left( Job_Time,10) >= '" + mtxtMakDate1.Text.Trim() + "'";
                strSql = strSql + " And Left( Job_Time,10) <= '" + mtxtMakDate2.Text.Trim() + "'";
            }


            if (txtR_Id_Code.Text.Trim() != "")
                strSql = strSql + " And T_U_ID= '" + txtR_Id_Code.Text.Trim() + "'";

            

            Tsql = Tsql + strSql ;
            Tsql = Tsql + " Order By T_U_ID , Job_Time DESC ";
        }
        ...
        {   
            string Tsql = "";            
            Tsql = "SELECT T_U_ID ,Form_Name_Caption , Job_Time , Job_ETC " ;
            Tsql = Tsql + ", '','','','','',''  " ;             
            Tsql = Tsql + " FROM tbl_User_Log  (nolock) " ;

            //Tsql = Tsql + "  where T_U_ID = 'ksw77627' and Form_Name = 'frmSell_Cancel'  " ;
            //Tsql = Tsql + "  and LEFT (Job_Time ,10 ) = '2016-03-28' ";
        ...
        private void dGridView_Base_Header_Reset()
        {
            ...
            string[] g_HeaderText = {"사용자_아이디"  , "대상화면"   , "작업시간"  , "작업내역"   , ""        
                                , ""   , ""    , ""   , ""    , ""
                                    };
            cgb.grid_col_header_text = g_HeaderText;

            int[] g_Width = { 100, 100, 100, 500, 0  
                             ,0, 0, 0, 0, 0  
        ...
        private void Set_gr_dic(ref DataSet ds, ref Dictionary<int, object[]> gr_dic_text, int fi_cnt)
        {
            ...
            String T_Job_ETC = m_crypt.Decrypt (ds.Tables[base_db_name].Rows[fi_cnt][3].ToString ());
        ...
        {
            Excel_Export_File_Name = this.Text; // "Log_Select";
