...
{
    public partial class frmBase_User_ETC : clsForm_Extends
    {
        ...
        private void Save_Base_Data(ref int Save_Error_Check)
        {
            ...
            {                
                ...
                string StrSql = "";
                cls_Connect_DB Temp_Connect = new cls_Connect_DB();
                StrSql = "Insert into tbl_User_ETC " ;
                StrSql = StrSql + " ( T_ETC ,  Visible_TF ,  Visible_Date , Visible_User  , RecordID  , RecordTime )" ;
                StrSql = StrSql + " Values (" ;
        
                StrSql = StrSql + "'" + txtETC.Text + "'," + Visible_TF  + ",'" + mtxtMakDate1.Text.Replace ("-","").Trim () + "'" ;
                StrSql = StrSql + ",'" + Visible_User + "', '" + cls_User.gid  + "', Convert(Varchar(25),GetDate(),21) ";
                StrSql = StrSql + " )" ;    
        ...
        private void Set_Tbl_User()
        {
            ...
            string Tsql = "";
            Tsql = "select 'V',  user_Ncode,  user_id, u_name , tbl_user.Phone ";            
            Tsql = Tsql + " From tbl_user (nolock) ";
            Tsql = Tsql + " order by user_id ";
        ...
        private void ETC_Grid_Set()
        {
            ...
            Tsql = "Select T_ETC, Visible_Date ";
            Tsql = Tsql + ", Case When Visible_TF = 0 Then  '안보여줌' ELSE '보여줌' END  ";
            Tsql = Tsql + ", Case When Visible_User = '전체' then Visible_User ELSE '선택' END ";
            Tsql = Tsql + ", RecordID ";
            Tsql = Tsql + " ,RecordTime     ,T_index , Visible_TF , Visible_User ,'' ";
        ...
        private void dGridView_ETC_Header_Reset()
        {
            ...
            string[] g_HeaderText = {"내역"  , "공지_시작일"   , "공지_여부"  , "공지_적용자"   , "기록자"        
                                , "기록일자"   , "_T_index"    , "_Visible_TF"  , "" , ""                                
                                };

            int[] g_Width = { 500, 100, 90, 90, 100
                            ,140 , 0 , 0 , 0 , 0                          
                            };
        ...
        private void dGridView_Base_2_DoubleClick(object sender, EventArgs e)
        {
            ...
            if ((sender as DataGridView).CurrentRow.Cells[0].Value != null)
            {
                ...
                mtxtMakDate1.Text = (sender as DataGridView).CurrentRow.Cells[1].Value.ToString();

                chk_VTF.Checked = false;
                if ((sender as DataGridView).CurrentRow.Cells[7].Value.ToString() == "1")
                    chk_VTF.Checked = true ;

                string Visible_User = (sender as DataGridView).CurrentRow.Cells[8].Value.ToString();
                ...
                if (Visible_User != "전체")
                {
                    ...
                    for (int cnt = 0; cnt < t_Memu.Length; cnt++)
                    {
                        if (t_Memu[cnt] != "")
                        {
                            ...
                            for (int i = 0; i <= dGridView_Base.Rows.Count - 1; i++)
                            {
                                if (dGridView_Base.Rows[i].Cells[2].Value.ToString() == t_Memu[cnt])
                                {
                                    dGridView_Base.Rows[i].Cells[0].Value = "V";
                                }
                            }
