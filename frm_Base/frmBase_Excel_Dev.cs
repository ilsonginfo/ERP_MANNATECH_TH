...
namespace MLM_Program
{
    public partial class frmBase_Excel_Dev : Form
    {
        ...
        public delegate DevExpress.XtraGrid.Views.Grid.GridView Send_Export_Excel_Info_Dele(ref string Excel_Export_From_Name, ref string Excel_Export_File_Name);
        public event Send_Export_Excel_Info_Dele Send_Export_Excel_Info;


        public frmBase_Excel_Dev()
        {
            InitializeComponent();
        }
        ...
        private void frmBase_Excel_Dev_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void frmBase_Excel_Dev_Load(object sender, EventArgs e)
        {
        ...
        private void Base_Grid_Set()
        {
            ...
            DevExpress.XtraGrid.Views.Grid.GridView Temp_Grid = Exp_dgv;
            ...
            for (int i = 0; i <= Temp_Grid.Columns.Count - 1; i++)
            {
                ...
                //{
                if(Temp_Grid.Columns[i].Visible)
                    Set_gr_dic( ref gr_dic_text,Temp_Grid.Columns[i].Caption.ToString(), i);  //데이타를 배열에 넣는다.                    
                //}                
            }
            ...
            for (int i = 0; i < Temp_Grid.Columns.Count - 1; i++)
            {
               // if (Temp_Grid.Columns[i].Caption != null && Temp_Grid.Columns[i].Caption != "" && Temp_Grid.Columns[i].Visible == true && Temp_Grid.Columns[i].Caption.Substring(0, 1) != "_")
               // {
               //
               // }
               // else
               //     Temp_Grid.Rows[i].Visible = false;
            }
        ...
        private void dGridView_Base_Header_Reset()
        {
            ...
            string[] g_Cols = { "Selected", "Caption" };
        ...
        private void User_Check_Load()
        {
             ...
             try
            {
                ...
                TSql = TSql + " Where  Excel_Name ='" + Excel_Export_File_Name + "'";
        ...
        private void Save_Base_Data()
        {
            ...
            if (ReCnt == 0)
            {
                ...
                TSql = TSql + "'" + cls_User.gid.ToUpper() + "','" + Excel_Export_File_Name + "', '" + CheckList + "','') ";
        ...
        public void GridViewExcel(DevExpress.XtraGrid.Views.Grid.GridView grid)
        {
            grid.SaveLayoutToRegistry("Temp1");
            ...
            foreach (DataGridViewRow row in dGridView_Base.Rows)
            {
                ...
                {
                    string Caption = row.Cells[1].Value.ToString();
            ...
            Class.DevGridControlService devGridControlService = new Class.DevGridControlService();
            devGridControlService.baseview = grid;
            devGridControlService.ExportExcel(Excel_Export_File_Name);

            grid.RestoreLayoutFromRegistry("Temp1");
            ...
            try
            {
                ...
                int k = 0;
                string[] colHeader = new string[grid.Columns.Count];
                for (int i = 0; i < grid.Columns.Count; i++)
                {
                    oSheet.Cells[1, i + 1] = grid.Columns[i].Caption;
                ...
                object[,] saNames = new object[grid.RowCount, grid.Columns.Count];
                ...
                for (int i = 0; i < grid.RowCount; i++)
                {
                    for (int j = 0; j < grid.Columns.Count; j++)
                    {
                        ...
                        string T_string = grid.Columns[j].Caption.ToString();
