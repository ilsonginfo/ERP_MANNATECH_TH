...
{
    public partial class frmBase_Member_Search_birthday : clsForm_Extends
    {
        StringEncrypter encrypter = new StringEncrypter(cls_User.con_EncryptKey, cls_User.con_EncryptKeyIV);

        cls_Grid_Base cgb = new cls_Grid_Base();
        private const string base_db_name = "tbl_Memberinfo";
        //20191017 구현호대리 - 회원조회-생일조회중복창
        ...
        public frmBase_Member_Search_birthday()
        {
            InitializeComponent();
        }
        ...
        private void Base_Grid_Set()
        {
            ...
            if (cls_app_static_var.Member_Number_1 >0)
                sb.AppendLine(" tbl_Memberinfo.mbid + '-' + Convert(Varchar,tbl_Memberinfo.mbid2) ");
            else
                sb.AppendLine(" tbl_Memberinfo.mbid2 ");
            sb.AppendLine(" ,tbl_Memberinfo.M_Name ");
            //sb.AppendLine(" ,case when isnull(tbl_Memberinfo.Cpno, '') = '' then right(tbl_Memberinfo.BirthDay, 2) + tbl_Memberinfo.BirthDay_M + tbl_Memberinfo.BirthDay_D ELSE tbl_Memberinfo.Cpno END cpno");
            sb.AppendLine(" ,right(tbl_Memberinfo.BirthDay, 2) + tbl_Memberinfo.BirthDay_M + tbl_Memberinfo.BirthDay_D as cpno");
            sb.AppendLine(" ,tbl_Memberinfo.RegTime ");
            sb.AppendLine(" ,Isnull(tbl_Business.Name,'') as B_Name");
            
            if (cls_app_static_var.Member_Number_1 > 0)
                sb.AppendLine(" ,tbl_Memberinfo.Nominid + '-' + Convert(Varchar,tbl_Memberinfo.Nominid2) ");
            else
            ...
            {
                sb.AppendLine(" And tbl_Memberinfo.Birthday + '-' + tbl_Memberinfo.Birthday_M + '-' + tbl_Memberinfo.Birthday_D = '" + txtMemberName.Text.Trim() + "' ");
                sb.AppendLine(" And   tbl_Memberinfo.Cpno = '" + encrypter.Encrypt(txtCpno.Text.Trim()) + "' ");
            }
