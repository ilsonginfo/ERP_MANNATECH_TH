...
{
    public partial class frmSell_Select_Group_Rece : clsForm_Extends
    {
        ...
        public frmSell_Select_Group_Rece()
        {
            InitializeComponent();
        }
        ...
        private Boolean Check_TextBox_Error()
        {
            ...
            if (mtxtSellDate2.Text.Replace("-", "").Replace("_", "").Trim() == "")
        ...
        private void Make_Base_Query(ref string Tsql)
        {
            ...
            Tsql = "Select   Recordid , RecordName , Card_Price ,   Card_Cnt ,  Bank_Price ,   Bank_Cnt , Cash_Price ,   Cash_Cnt ,Sum_Price,Sum_Cnt  ";
            Tsql = Tsql + " From ufn_Get_Recordid_Sell_Info_004 ('" + mtxtSellDate1.Text.Replace("-", "").Trim() + "','" + mtxtSellDate2.Text.Replace("-", "").Trim() + "')";            
        }
        ...
        private void dGridView_Base_Header_Reset()
        {
            ...
            string[] g_HeaderText = {"사용자" ,"성명" , "카드금액"   , "카드수"  , "가상계좌"   
                                , "가상계좌수"        , "현금"   , "현금수"    , "총매출액"   , "총건수"                             
                                    };
        ...
        private void Item_Grid_Set(string ItemCode )
        {
            ...
            Tsql = Tsql + " LEFT JOIN tbl_Base_Change_Detail Ch_T (nolock) ON Ch_T.M_Detail_S = 'tbl_Sales_Cacu' And  Ch_T.M_Detail = Convert(Varchar,tbl_Sales_Cacu.C_TF) ";
            Tsql = Tsql + " Left Join tbl_SellType  (nolock) On tbl_SellType.SellCode=tbl_SalesDetail.SellCode  ";
                        
           //Make_Base_Query_(ref Tsql);

            Tsql = Tsql + " Where tbl_SalesDetail.Ga_Order = 0 And tbl_SalesDetail.SellCode <> '' "; 
            //기록일자로 검색 -1
            if ((mtxtSellDate1.Text.Replace("-", "").Trim() != "") && (mtxtSellDate2.Text.Replace("-", "").Trim() == ""))
                Tsql = Tsql + " And Replace(Left( tbl_Sales_Cacu.recordtime ,10),'-','') = '" + mtxtSellDate1.Text.Replace("-", "").Trim() + "'";

            //기록일자로 검색 -2
            if ((mtxtSellDate1.Text.Replace("-", "").Trim() != "") && (mtxtSellDate2.Text.Replace("-", "").Trim() != ""))
            {
                Tsql = Tsql + " And Replace(Left( tbl_Sales_Cacu.recordtime ,10),'-','') >= '" + mtxtSellDate1.Text.Replace("-", "").Trim() + "'";
                Tsql = Tsql + " And Replace(Left( tbl_Sales_Cacu.recordtime ,10),'-','') <= '" + mtxtSellDate2.Text.Replace("-", "").Trim() + "'";
            }


            Tsql = Tsql + " And  tbl_Sales_Cacu.Recordid = '" + ItemCode + "'";
            Tsql = Tsql + " Order by tbl_Sales_Cacu.C_TF ASC , tbl_Sales_Cacu.RecordTime DESC  ";
            ...
            double Sum_10 = 0; double Sum_11 = 0; double Sum_12 = 0; ; double Sum_13 = 0;
            //double Sum_16 = 0;
            //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            Dictionary<int, object[]> gr_dic_text = new Dictionary<int, object[]>();
