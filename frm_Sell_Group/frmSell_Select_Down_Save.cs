...
namespace MLM_Program
{
    public partial class frmSell_Select_Down_Save : clsForm_Extends
    {
        ...
        Dictionary<string, double> dic_Down_Mem = new Dictionary<string, double>();
        Dictionary<string, double> dic_Down_Mem_Re = new Dictionary<string, double>();
        ...
        public frmSell_Select_Down_Save()
        {
            InitializeComponent();
        ...
        private void Base_Grid_Set()
        {
            ...
            if (opt_Nom.Checked == true)
                StrSql = "Select Max(N_LineCnt) ";
            else
                StrSql = "Select Max(LineCnt) ";
            ...
            if (Temp_Connect.Open_Data_Set(StrSql, base_db_name, ds_2, this.Name, this.Text) == false) return;
            int ReCnt2 = Temp_Connect.DataSet_ReCount;

            if (ReCnt2 == 0) return;

            int Max_LineCnt = int.Parse(ds_2.Tables[base_db_name].Rows[0][0].ToString());

            int Return_TF = 0;
            if (chk_Re.Checked == true) Return_TF = 1;
            ...
            StrSql = StrSql + " , CurGrade_NAme ";
            StrSql = StrSql + " , Bus_Name ";
            StrSql = StrSql + " , Nom2 ";
            StrSql = StrSql + " , Nom_Name ";


            StrSql = StrSql + " , SellPV ";

            StrSql = StrSql + " , LineCnt ";
            StrSql = StrSql + " , DownPV, DownPrice ";
            StrSql = StrSql + " , Re_DownPV, Re_DownPrice ";
            if (opt_Nom.Checked == true)
                StrSql = StrSql + " From ufn_AX_Tree_MemGroup_Nom_OV_2 ('" + mtxtSellDate1.Text.Replace("-", "").Trim() + "','" + mtxtSellDate2.Text.Replace("-", "").Trim() + "'," + Return_TF + "," + Sham_TF + ")";
            else
                StrSql = StrSql + " From ufn_AX_Tree_MemGroup_OV_Save_1 ('" + mtxtSellDate1.Text.Replace("-", "").Trim() + "','" + mtxtSellDate2.Text.Replace("-", "").Trim() + "'," + Return_TF + "," + Sham_TF + ")";
            StrSql = StrSql + " Order by mbid, mbid2,  Curposition ";
            ...
            Dictionary<int, double> gr_dic_Down = new Dictionary<int, double>();
            Dictionary<int, double> gr_dic_Down_Re = new Dictionary<int, double>();
            //gr_dic_Down = null; 
            int ArCnt = 0;
            for (int fi_cnt = 0; fi_cnt <= ReCnt - 1; fi_cnt++)
            {

                if (dic_Down_Mem.ContainsKey(ds.Tables[base_db_name].Rows[fi_cnt][0].ToString()) == false)
                {
                    ArCnt = 3;
                    dic_Down_Mem[ds.Tables[base_db_name].Rows[fi_cnt][0].ToString()] = double.Parse(ds.Tables[base_db_name].Rows[fi_cnt][8].ToString());
                    dic_Down_Mem_Re[ds.Tables[base_db_name].Rows[fi_cnt][0].ToString()] = double.Parse(ds.Tables[base_db_name].Rows[fi_cnt][10].ToString());
                    if (gr_dic_Down.Count > 0)
                    {
                        if (dic_Down_Mem[ds.Tables[base_db_name].Rows[fi_cnt - 1][0].ToString()] > 0 || dic_Down_Mem_Re[ds.Tables[base_db_name].Rows[fi_cnt - 1][0].ToString()] < 0)
                        {
                            object[] row0 = new object[(Max_LineCnt * 2 )  + 11];

                            row0[0] = ds.Tables[base_db_name].Rows[fi_cnt - 1][0];
                            row0[1] = ds.Tables[base_db_name].Rows[fi_cnt - 1][1];
                            row0[2] = ds.Tables[base_db_name].Rows[fi_cnt - 1][2];
                            row0[3] = ds.Tables[base_db_name].Rows[fi_cnt - 1][3];
                            row0[4] = ds.Tables[base_db_name].Rows[fi_cnt - 1][4];
                            row0[5] = ds.Tables[base_db_name].Rows[fi_cnt - 1][5];
                            row0[6] = ds.Tables[base_db_name].Rows[fi_cnt - 1][6];  //본인매출


                            row0[7] = dic_Down_Mem[ds.Tables[base_db_name].Rows[fi_cnt - 1][0].ToString()];  //소실적
                            row0[8] = dic_Down_Mem_Re[ds.Tables[base_db_name].Rows[fi_cnt - 1][0].ToString()];  //소실적 반품
                            row0[9] = double.Parse(row0[7].ToString()) + double.Parse(row0[8].ToString());
                            row0[10] = double.Parse(row0[7].ToString()) + double.Parse(row0[8].ToString()) + double.Parse(row0[6].ToString());  
 

                            int T_Cnt = 1; 
                            for (int i = 1; i <= Max_LineCnt * 2  ; i++)
                            {
                                if (i % 2 == 1)
                                {
                                    if (gr_dic_Down.ContainsKey(T_Cnt) == true)
                                        row0[i  + 10] = gr_dic_Down[T_Cnt];
                                    else
                                        row0[i  + 10] = 0;
                                ...
                                {
                                    if (gr_dic_Down_Re.ContainsKey(T_Cnt) == true)
                                        row0[i  + 10] = gr_dic_Down_Re[T_Cnt];
                                    else
                                        row0[i  + 10] = 0;
                            ...
                            gr_dic_text[fi_cnt + 1] = row0; //뿌려지는 딕셔너리에 배열을 넣는다. 
                        }

                        gr_dic_Down.Clear();
                        gr_dic_Down_Re.Clear();                        
                    }
                    int linecnt =  int.Parse(ds.Tables[base_db_name].Rows[fi_cnt][7].ToString()) ;
                    gr_dic_Down[linecnt] = double.Parse(ds.Tables[base_db_name].Rows[fi_cnt][8].ToString());
                    gr_dic_Down_Re[linecnt] = double.Parse(ds.Tables[base_db_name].Rows[fi_cnt][10].ToString()); 

                }
                else
                {
                    ArCnt++;
                    if (dic_Down_Mem[ds.Tables[base_db_name].Rows[fi_cnt][0].ToString()] > double.Parse(ds.Tables[base_db_name].Rows[fi_cnt][8].ToString()))
                    {
                        dic_Down_Mem[ds.Tables[base_db_name].Rows[fi_cnt][0].ToString()] = double.Parse(ds.Tables[base_db_name].Rows[fi_cnt][8].ToString());
                        dic_Down_Mem_Re[ds.Tables[base_db_name].Rows[fi_cnt][0].ToString()] = double.Parse(ds.Tables[base_db_name].Rows[fi_cnt][10].ToString());
                    }
                ...
                //Set_gr_dic(ref ds, ref gr_dic_text, fi_cnt, Max_LineCnt);  //데이타를 배열에 넣는다.
                                
                
            }

            if (gr_dic_Down != null) gr_dic_Down.Clear();
            if (gr_dic_Down_Re != null) gr_dic_Down_Re.Clear();

            if (dic_Down_Mem != null) dic_Down_Mem.Clear();

            if (dic_Down_Mem_Re != null) dic_Down_Mem_Re.Clear();
            //object[] row0 = { ""  ,""  ,""  ,""  ,""
            //                  , ""  ,""  ,""  ,""  ,""
            //                };
            ...
            for (int fi_cnt = 0; fi_cnt <= cgb.basegrid.RowCount  - 1; fi_cnt++)
            {
                if (int.Parse(cgb.basegrid[11, fi_cnt].Value.ToString()) == 0 || int.Parse(cgb.basegrid[13, fi_cnt].Value.ToString()) == 0) 
                {
                    cgb.basegrid[ 7, fi_cnt].Value = "0";
                    cgb.basegrid[ 8, fi_cnt].Value = "0";
                    cgb.basegrid[ 9, fi_cnt].Value = "0";
                    cgb.basegrid[10, fi_cnt].Value = cgb.basegrid[6, fi_cnt].Value.ToString(); 
                }
            }
        ...
        private void dGridView_Base_Header_Reset()
        {
            ...
            int  Max_LineCnt = int.Parse (ds.Tables[base_db_name].Rows[0][0].ToString ());



            cgb.grid_col_Count = (Max_LineCnt * 2) + 11;            
            cgb.basegrid = dGridView_Base;            
            cgb.grid_select_mod = DataGridViewSelectionMode.FullRowSelect;
            cgb.grid_Frozen_End_Count = 2;
            //cgb.basegrid.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.AllCells;

            string[] g_HeaderText = new string[(Max_LineCnt * 2) + 11];

            //string[] g_HeaderText  ; //= {"회원_번호"  , "성명"   , "대실적"  , "PV"   , "OV"        
                                //, "Non-high"   , "PA"    , ""   , ""       , ""
                               // ,"" , "", "", "", ""
                                //    };

            g_HeaderText[0] = "회원_번호";
            g_HeaderText[1] = "성명";
            g_HeaderText[2] = "직급";
            g_HeaderText[3] = "센터명";
            g_HeaderText[4] = "추천인";
            ...
            g_HeaderText[6] = "본인매출PV";
            g_HeaderText[7] = "소실적";
            g_HeaderText[8] = "소실적반품";
            g_HeaderText[9] = "소실적합계PV";
            g_HeaderText[10] = "매출합계PV";
            ...
            for (int i = 1; i <= (Max_LineCnt * 2)  ; i++) 
            {
                if ((i % 2) == 1)
                {
                    g_HeaderText[i  + 10] = T_cnt.ToString() + "라인";
                    
                }
                else
                {
                    g_HeaderText[i  + 10] = T_cnt.ToString() + "라인_반품";
            ...
            //int[] g_Width = { 130, 90, 0, 90, 100   
            //                 ,100 , 100 , 0 , 0 , 0                         
            //                 ,0, 0 , 0 , 0 , 0   
            //                };

            int[] g_Width = new int[(Max_LineCnt * 2) + 11];
            ...
            Boolean[] g_ReadOnly = new Boolean[(Max_LineCnt * 2) + 11];
            for (int i = 0; i < (Max_LineCnt * 2) + 11; i++)
            {
                g_ReadOnly[i] = true;
            }
            ...
            DataGridViewContentAlignment[] g_Alignment = new DataGridViewContentAlignment[(Max_LineCnt * 2) + 11];

            g_Alignment[0] = DataGridViewContentAlignment.MiddleCenter;
            g_Alignment[1] = DataGridViewContentAlignment.MiddleCenter;
            g_Alignment[2] = DataGridViewContentAlignment.MiddleCenter;
            g_Alignment[3] = DataGridViewContentAlignment.MiddleCenter;
            g_Alignment[4] = DataGridViewContentAlignment.MiddleCenter;
            g_Alignment[5] = DataGridViewContentAlignment.MiddleCenter;

            g_Alignment[6] = DataGridViewContentAlignment.MiddleRight;
            g_Alignment[7] = DataGridViewContentAlignment.MiddleRight;
            g_Alignment[8] = DataGridViewContentAlignment.MiddleRight;
            g_Alignment[9] = DataGridViewContentAlignment.MiddleRight;
            g_Alignment[10] = DataGridViewContentAlignment.MiddleRight;

            for (int i = 1; i <= Max_LineCnt * 2 ; i++)
            {
                g_Alignment[i  + 10] = DataGridViewContentAlignment.MiddleRight; 
            }
            ...
            for (int i = 1; i <= Max_LineCnt * 2; i++)
            {
                gr_dic_cell_format[i+10] = cls_app_static_var.str_Grid_Currency_Type;
            }
        ...
        private void Set_gr_dic(ref DataSet ds, ref Dictionary<int, object[]> gr_dic_text, int fi_cnt, int Max_LineCnt )
        {

            object[] row0 = new object[Max_LineCnt + 3];

            row0[0] = ds.Tables[base_db_name].Rows[fi_cnt][0];
            row0[1] = ds.Tables[base_db_name].Rows[fi_cnt][1];
            row0[2] = ds.Tables[base_db_name].Rows[fi_cnt][2];

            for (int i = 1; i <= Max_LineCnt; i++)
            {
                row0[i + 2] = ds.Tables[base_db_name].Rows[fi_cnt][2];
            }
            ...
            //                    ,""
            //                    ,ds.Tables[base_db_name].Rows[fi_cnt]["SellPV"]
            //                    ,ds.Tables[base_db_name].Rows[fi_cnt]["DownPV"]
 
            //                    ,ds.Tables[base_db_name].Rows[fi_cnt]["non_Hight"]
            //                    ,ds.Tables[base_db_name].Rows[fi_cnt]["Pa_Cnt"]
            //                    ,""
            //                    ,""
            //                    ,""
            ...
            //                    ,""
            //                    ,""
            //                    //,SellPrice, SellPV, RE_SellPrice, RE_SellPV ";
        ...
        private void Make_Base_Query(ref string Tsql, string Search_Number)
        {
           ...
           Tsql = Tsql + ", tbl_Memberinfo.Cpno";
            ...
            Tsql = Tsql + " From ufn_Getsubtree_MemGroup ('" + Mbid + "'," + Mbid2 + " ) AS A";            
