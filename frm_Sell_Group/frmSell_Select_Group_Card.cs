...
{
    public partial class frmSell_Select_Group_Card : clsForm_Extends
    {
        ...
        Series series_Price = new Series();

        public frmSell_Select_Group_Card()
        {
            InitializeComponent();
        }
        ...
        private void Make_Base_Query(ref string Tsql)
        {
            ...
            //string[] g_HeaderText = {"카드_코드"  , "카드명"   , "사용건수"  , "결제_금액"   , "승인_금액"        
            //                    , ""   , ""    , ""   , ""    , ""                                
            //                        };

            Tsql = "Select  C_Code  ";
            Tsql = Tsql + " ,Isnull(tbl_Card.CardName,'')  ";
            Tsql = Tsql + " ,Count(C_CodeName)   ";
            Tsql = Tsql + " ,Sum(C_Price1) as Total1 ";
            Tsql = Tsql + " ,Sum(C_Price2) as Total2 ";
            Tsql = Tsql + " , '' , '' , '' , '' , '' ";
            ...
            Tsql = Tsql + " LEFT Join tbl_Card  (nolock) ON tbl_Sales_Cacu.C_code = tbl_Card.Ncode  And tbl_SalesDetail.Na_code = tbl_Card.Na_code ";
        ...
        {
            string strSql = " Where tbl_Sales_Cacu.C_TF  = 3  And tbl_SalesDetail.Ga_Order = 0 And tbl_SalesDetail.SellCode <> '' ";            
        ...
        {   
            ...
            Tsql = Tsql + " Group By C_Code,Isnull(tbl_Card.CardName,'') ";
            Tsql = Tsql + " Order By C_Code,Isnull(tbl_Card.CardName,'') ";
            ...
            for (int fi_cnt = 0; fi_cnt <= ReCnt - 1; fi_cnt++)
            {
                ...
                Push_data(series_Price, CardName, double.Parse(ds.Tables[base_db_name].Rows[fi_cnt][3].ToString()));

                CardName = ds.Tables[base_db_name].Rows[fi_cnt][1].ToString();
                CardCnt = int.Parse(ds.Tables[base_db_name].Rows[fi_cnt][2].ToString());

                Push_data(series_Item, CardName, CardCnt);
            }
        ...
        {
            Excel_Export_File_Name = this.Text; // "Sell_Group_Card";
        ...
        private void Item_Grid_Set(string ItemCode )
        {
            ...
            //string[] g_HeaderText = {"주문번호"  , ""   , "주문일자"  , "주문종류"   , "카드번호"        
            //                    , "승인번호"   , "결제_금액"    , "승인_금액"  , "회원_번호" , "성명"
            //                    ,"주문_센타명" , "" , ""  ,   ""  , "" 
            //                    };
            ...
            double Sum_07 = 0; double Sum_08 = 0;  //double Sum_08 = 0;            
            //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            Dictionary<int, object[]> gr_dic_text = new Dictionary<int, object[]>();
            ...
            if (gr_dic_text.Count > 0)
            {
                ...
                txt_PP_2.Text = string.Format(cls_app_static_var.str_Currency_Type, Sum_07);
                txt_PP_3.Text = string.Format(cls_app_static_var.str_Currency_Type, Sum_08);
            }
        ...
        private void Save_Nom_Line_Chart()
        {
            ...
            //---------------------------------------------------
            series_Price.Points.Clear();
            series_Price["DrawingStyle"] = "Emboss";
            series_Price["PointWidth"] = "0.5";
            series_Price.Name = cm._chang_base_caption_search("금액");
            series_Price.ChartType = SeriesChartType.Column;

            //series_Price.Legend = "Legend1";
            chart_Item.Series.Add(series_Price);
            //---------------------------------------------------
            ...
            series_Item.Name = cm._chang_base_caption_search("건수");            
