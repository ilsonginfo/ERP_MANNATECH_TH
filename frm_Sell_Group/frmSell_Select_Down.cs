...
{
    public partial class frmSell_Select_Down : clsForm_Extends
    {
        ...
        private void frmBase_From_Load(object sender, EventArgs e)
        {
            ...
            txt_BV_1.BackColor = cls_app_static_var.txt_Enable_Color;
            txt_BV_2.BackColor = cls_app_static_var.txt_Enable_Color;
        ...
        private Boolean Check_TextBox_Error()
        {
            ...
            if (mtxtSellDate4.Text.Replace("-", "").Trim() == "")
                mtxtSellDate4.Text = mtxtSellDate3.Text;
        ...
        {
            string SellDate1 = mtxtSellDate1.Text.Replace("-", "").Trim(); //start
            string SellDate2 = mtxtSellDate2.Text.Replace("-", "").Trim(); //end
            string SellDate3 = mtxtSellDate3.Text.Replace("-", "").Trim();  //start
            string SellDate4 = mtxtSellDate4.Text.Replace("-", "").Trim();  //end

            if (SellDate1.Equals(string.Empty))
                SellDate1 = "19000101";

            if (SellDate2.Equals(string.Empty))
                SellDate2 = "29991231";

            if (SellDate3.Equals(string.Empty))
                SellDate3 = "19000101";

            if (SellDate4.Equals(string.Empty))
                SellDate4 = "29991231";
            ...
            if (opt_Save.Checked ==true )
                StrSql = "EXEC Usp_Search_Mem_Down_Sell_Save ";
            else
                StrSql = "EXEC Usp_Search_Mem_Down_Sell_Nom ";
            StrSql = StrSql + " '" + idx_Mbid + "'";
            StrSql = StrSql + "," + idx_Mbid2.ToString();
            StrSql = StrSql + ",'" + SellDate1 + "'";
            StrSql = StrSql + ",'" + SellDate2 + "'";
            StrSql = StrSql + ",'" + SellDate3 + "'";
            StrSql = StrSql + ",'" + SellDate4 + "'";
            StrSql = StrSql + ",'" + combo_Se_Code.Text.Trim() + "'";
            StrSql = StrSql + "," + cls_app_static_var.Member_Number_1.ToString ()  ;
            ...
            double TotalPr = 0; double TotalPV = 0, TotalAV = 0;
            double Re_TotalPr = 0; double Re_TotalPV = 0, Re_TotalAV = 0;
            double ReturnPr = 0; double ReturnPV = 0, ReturnAV = 0;
            ...
            for (int fi_cnt = 0; fi_cnt <= ReCnt - 1; fi_cnt++)
            {
                ...
                Re_TotalPr = Re_TotalPr + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt][5].ToString());
                ReturnPr = ReturnPr + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt][6].ToString());

                TotalPV = TotalPV + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt][7].ToString());
                Re_TotalPV = Re_TotalPV + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt][8].ToString());
                ReturnPV = ReturnPV + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt][9].ToString());

                TotalAV = TotalAV + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt][10].ToString());
                Re_TotalAV = Re_TotalAV + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt][11].ToString());
                ReturnAV = ReturnAV + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt][12].ToString());
            ...
            {
                object[] row1 = { ""  ,"합계"  ,""  ,""  
                                  , TotalPr  ,Re_TotalPr ,ReturnPr,  TotalPV, Re_TotalPV,ReturnPV,TotalAV,Re_TotalAV,ReturnAV
                                };
        ...
        private void dGridView_Base_Header_Reset()
        {
            ...
            string[] g_HeaderText = {"회원_번호"  , "성명"   , "대수"  , "위치"  , "주문금액"   
                                  , "반품금액" ,"순 주문금액",  "주문PV",  "반품PV","순 주문PV"     
                                   , "주문CV"    , "반품CV" ,"순 주문CV"                            
                                    };

            string[] g_ColsName = { "mbid2","mname","point1", "point2", "InputPrice"
                                , "refundPrice", };
        ...
        private void Make_Base_Query(ref string Tsql, string Search_Number)
        {
            ...
            Tsql = Tsql + "  ELSE (Select LEFT(A1.SellDate,4) +'-' + LEFT(RIGHT(A1.SellDate,4),2) + '-' + RIGHT(A1.SellDate,2) From tbl_SalesDetail AS A1 (nolock) Where A1.OrderNumber = tbl_SalesDetail.Re_BaseOrderNumber)  END ";
            //
            Tsql = Tsql + " , Case ReturnTF When 1 then '' ELSE  LEFT(SellDate_2,4) +'-' + LEFT(RIGHT(SellDate_2,4),2) + '-' + RIGHT(SellDate_2,2)  END ";
            //
            ...
            Tsql = Tsql + " ,TotalPrice , Totalpv, TotalCv  ";
            Tsql = Tsql + " ,TotalInputPrice ";
            Tsql = Tsql + " ,InputCash , InputCard ,InputPassbook +  InputPassbook_2 ";
            ...
            if (opt_Save.Checked == true)
                ...
                Tsql = Tsql + " From ufn_GetSubTree_NomGroup ('" + Mbid + "'," + Mbid2 + " ) AS A";
        ...
        private void Base_Grid_Set(string Search_Number )
        {
            ...
            double TotalBV = 0; double Re_TotalBV = 0;
            ...
            for (int fi_cnt = 0; fi_cnt <= ReCnt - 1; fi_cnt++)
            {
                ...
                {
                    TotalPr = TotalPr + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt][10].ToString());
                    TotalPV = TotalPV + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt][11].ToString());
                    TotalBV = TotalBV + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt][12].ToString());
                }
                else
                {
                    Re_TotalPr = Re_TotalPr + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt][10].ToString());
                    Re_TotalPV = Re_TotalPV + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt][11].ToString());
                    Re_TotalBV = Re_TotalBV + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt][12].ToString());
                }
            }
            ...
            txt_BV_1.Text = string.Format(cls_app_static_var.str_Currency_Type, TotalBV);
            txt_BV_2.Text = string.Format(cls_app_static_var.str_Currency_Type, Re_TotalBV);
        ...
        private void dGridView_Sell_Down_Header_Reset()
        {
            ...
            string[] g_HeaderText = {"주문번호"  , "주문_일자"   ,"정산 일자"   , "교환_반품_일자"  , "회원_번호"   , "성명"        
            ...
            int[] g_Width = { 130, 90,90, 110, 90, 90  
                             ,0, 0, 0, 90, 80  
                             ,80 , 80, 80, 80, 80
                             ,80 , 90, 130 , 90 , 0
                             ,90 , 130 , 0 , 0 , 0
