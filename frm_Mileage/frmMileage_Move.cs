...
namespace MLM_Program
{
    public partial class frmMileage_Move : clsForm_Extends
    {
        ...
        private void frmBase_From_Load(object sender, EventArgs e)
        {
            ...
            txt_Mile2.BackColor = cls_app_static_var.txt_Enable_Color;
        }
        ...
        private void mtxtMbid2_Click(object sender, EventArgs e)
        {
            MaskedTextBox mtb = (MaskedTextBox)sender;
            ...
            if (e.KeyChar == 13)
            {
                ...
                if (mtb.Text.Replace("-", "").Replace("_", "").Trim() != "")
                {
                    ...
                    if (reCnt == 1)
                    {
                        ...
                        if (mtb.Name == "mtxtMbid2")
                        {
                            txtName.Text = Search_Name;
                            if (Input_Error_Check(mtb, 0) == true)
                                Set_Form_Date2(mtb.Text);
                            //SendKeys.Send("{TAB}");
                    ...
                    else if (reCnt > 1)  //회원번호 비슷한 사람들이 많은 경우
                    {
                        ...
                        if (mtb.Name == "mtxtMbid2")
                        {
                            e_f.Send_Mem_Number += new frmBase_Member_Search.SendNumberDele(e_f_Send_Mem_Number2);
                            e_f.Call_searchNumber_Info += new frmBase_Member_Search.Call_searchNumber_Info_Dele(e_f_Send_MemNumber_Info2);
                        }
                        e_f.ShowDialog();
        ...
        void e_f_Send_MemNumber_Info2(ref string searchMbid, ref int searchMbid2, ref string seachName)
        {
            seachName = "";
            cls_Search_DB csb = new cls_Search_DB();
            csb.Member_Nmumber_Split(mtxtMbid2.Text.Trim(), ref searchMbid, ref searchMbid2);
        }

        //변경할려는 대상자에 대한 회원번호에서 회원 검색창을 뛰엇을 경우에
        void e_f_Send_Mem_Number2(string Send_Number, string Send_Name)
        {
            mtxtMbid2.Text = Send_Number; txtName3.Text = Send_Name;
            if (Input_Error_Check(mtxtMbid2, 0) == true)
                Set_Form_Date2(mtxtMbid2.Text);
        }
        ...
        void T_R_Key_Enter_13_Name(string txt_tag, TextBox tb)
        {
            if (txt_tag != "")
            {
                ...
                if (reCnt == 1)
                {
                    ...
                    if (tb.Name == "txtName3")
                    {
                        mtxtMbid2.Text = Search_Mbid; //회원명으로 검색해서 나온 사람이 한명일 경우에는 회원번호를 넣어준다.                    
                        if (Input_Error_Check(mtxtMbid2, 0) == true)
                            Set_Form_Date2(mtxtMbid2.Text);
                ...
                else if (reCnt != 1)  //동명이인이 존재해서 사람이 많을 경우나 또는 이름 없이 엔터친 경우에.
                {
                    ...
                    {
                        e_f.Send_Mem_Number += new frmBase_Member_Search.SendNumberDele(e_f_Send_Mem_Number2);
                        e_f.Call_searchNumber_Info += new frmBase_Member_Search.Call_searchNumber_Info_Dele(e_f_Send_MemName_Info2);
                    }
        ...
        void e_f_Send_MemName_Info2(ref string searchMbid, ref int searchMbid2, ref string seachName)
        {
            searchMbid = ""; searchMbid2 = 0;
        ...
        private void Base_Sub_Grid_Set()
        {
            ...
            Tsql = Tsql + " , PlusValue " ; //Case  When PlusValue > 0 then C1.T_Name When MinusValue >0  then C2.T_Name End ";

            Tsql = Tsql + " , Case  When MinusValue > 0 then Minus_OrderNumber End ";
            Tsql = Tsql + " , Case  When PlusValue > 0 then Plus_OrderNumber End ";
            ...
            string strSql = " Where   (PlusKind  = '71' OR MinusKind = '72' ) ";
        ...
        private void dGridView_Base_Sub_Header_Reset()
        {
            ...
            string[] g_HeaderText = {"회원번호",  "성명"  , "기록일"   , "준_마일리지"  , "받은_마일리지"          
                                , "준_회원"   , "받은_회원"    , "비고"  , "기록자"   , "T_index"                                       
                                };
        ...
        private Boolean Check_TextBox_Error()
        {
            ...
            if ( double.Parse(txt_Price_3.Text.Replace(",", "").Trim())  == 0 )
            {
                MessageBox.Show(cls_app_static_var.app_msg_rm.GetString("Msg_Sort_Mileage_Price")
                       + "\n" +
                       cls_app_static_var.app_msg_rm.GetString("Msg_Re_Action"));
            ...
            if (double.Parse(txt_Mile.Text.Replace(",", "").Trim()) < double.Parse(txt_Price_3.Text.Replace(",", "").Trim()))
            {
                MessageBox.Show(cls_app_static_var.app_msg_rm.GetString("Msg_Mile_Move_Not1")                        
                       + "\n" +
                       cls_app_static_var.app_msg_rm.GetString("Msg_Re_Action"));
                txt_Price_3.Focus(); return false;
            }
        ...
        private void Save_Base_Data(ref int Save_Error_Check)
        {
            ...
            string T_Mbid2 = mtxtMbid2.Text.Trim();
            string TMbid = ""; int TMbid2 = 0;
            csd.Member_Nmumber_Split(T_Mbid2, ref TMbid, ref TMbid2);

            if (T_Mbid == Mbid && Mbid2 == TMbid2)
            {
                MessageBox.Show(cls_app_static_var.app_msg_rm.GetString("Msg_Mile_Same_Member")                        
                       + "\n" +
                       cls_app_static_var.app_msg_rm.GetString("Msg_Re_Action"));
                mtxtMbid2.Focus(); return ;
            }
            ...
            try
            {
                ...
                ctm.Put_Plus_Mileage(TMbid, TMbid2, txtName3.Text.Trim()
                    , app_Pr, T_Mbid , "71"
        ...
        private void Set_Form_Date2(string T_Mbid)
        {
            string Mbid = ""; int Mbid2 = 0;
            Data_Set_Form_TF = 1;
            ...
            if (csb.Member_Nmumber_Split(T_Mbid, ref Mbid, ref Mbid2) == 1)
            {
                ...
                mtxtMbid2.Text = ds.Tables[base_db_name].Rows[0]["M_Mbid"].ToString();
                ...
                cls_tbl_Mileage ctm = new cls_tbl_Mileage();
                T_p = ctm.Using_Mileage_Search(Mbid, Mbid2, "");
                txt_Mile2.Text = string.Format(cls_app_static_var.str_Currency_Type, T_p);
            }
