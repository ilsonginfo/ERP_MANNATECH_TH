...
{
    public partial class frmSell_Select_Down_Mem : clsForm_Extends
    {
        ...
        public frmSell_Select_Down_Mem()
        {
            InitializeComponent();
        }
        ...
        private void Base_Grid_Set()
        {
            ...
            if (opt_Save.Checked ==true )
                StrSql = "EXEC Usp_Search_Mem_Down_Reg_Save ";
            else
                StrSql = "EXEC Usp_Search_Mem_Down_Reg_Nom ";
            StrSql = StrSql + " '" + idx_Mbid + "'";
            StrSql = StrSql + "," + idx_Mbid2.ToString();
            StrSql = StrSql + ",'" + mtxtSellDate1.Text.Replace("-", "").Trim() + "'";
            StrSql = StrSql + ",'" + mtxtSellDate2.Text.Replace("-", "").Trim() + "'";
        ...
        private void dGridView_Base_Header_Reset()
        {
            ...
            string[] g_HeaderText = {"회원_번호"  , "성명"   , "대수"  , "위치"   , "하선등록인원"        
                                , ""   , ""    , ""   , ""    , ""                             
                                    };
        ...
        private void Make_Base_Query(ref string Tsql, string Search_Number)
        {
            ...
            Tsql = Tsql + " LEFT JOIN tbl_Memberinfo (nolock) ON tbl_Memberinfo.Mbid = A.Mbid And tbl_Memberinfo.Mbid2 = A.Mbid2 ";
            Tsql = Tsql + " LEFT JOIN tbl_Business (nolock) ON tbl_Memberinfo.BusinessCode = tbl_Business.NCode And tbl_Memberinfo.Na_code = tbl_Business.Na_code  ";
        ...
        {
            string strSql = " Where  tbl_Memberinfo.Mbid2  > 0 " ; //  And A.lvl > 0    ";
            ...
            //가입일자로 검색 -1
            if ((mtxtSellDate1.Text.Replace("-", "").Trim() != "") && (mtxtSellDate2.Text.Replace("-", "").Trim() == ""))
                strSql = strSql + " And tbl_Memberinfo.Regtime = '" + mtxtSellDate1.Text.Replace("-", "").Trim() + "'";

            //가입일자로 검색 -2
            if ((mtxtSellDate1.Text.Replace("-", "").Trim() != "") && (mtxtSellDate2.Text.Replace("-", "").Trim() != ""))
            {
                strSql = strSql + " And tbl_Memberinfo.Regtime >= '" + mtxtSellDate1.Text.Replace("-", "").Trim() + "'";
                strSql = strSql + " And tbl_Memberinfo.Regtime <= '" + mtxtSellDate2.Text.Replace("-", "").Trim() + "'";
            }
            ...
            Tsql = Tsql + strSql;
            Tsql = Tsql + " Order by tbl_Memberinfo.Regtime DESC ";
        ...
        private void dGridView_Sell_Down_Header_Reset()
        {
            ...
            string[] g_HeaderText = {"등록일자"  , ""   , ""  , "회원_번호"   , "성명"        
                                , "주민번호"   , "등록_센타명"    , "_주문_센타명"   , ""    , ""
                                , ""   , ""  , ""   , ""   ,""
                                , ""     , ""    , "" , ""     , "기록자"
                                , "기록일", ""  , ""  , ""  ,""
