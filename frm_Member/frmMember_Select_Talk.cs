...
{
    public partial class frmMember_Select_Talk : clsForm_Extends
    {        

         StringEncrypter encrypter = new StringEncrypter(cls_User.con_EncryptKey, cls_User.con_EncryptKeyIV);
        
        cls_Grid_Base cgb = new cls_Grid_Base();
        private const string base_db_name = "tbl_Memberinfo_Talk_Mod";
        private int Data_Set_Form_TF;
                      

        public frmMember_Select_Talk()
        {
            InitializeComponent();
        ...
        private void Make_Base_Query(ref string Tsql)
        {
            ...
            Tsql = Tsql + " tbl_Memberinfo_Talk.Seq ";

            if (cls_app_static_var.Member_Number_1 > 0)
                Tsql = Tsql + ", tbl_Memberinfo_Talk.mbid + '-' + Convert(Varchar,tbl_Memberinfo_Talk.mbid2) ";
            else
                Tsql = Tsql + ", tbl_Memberinfo_Talk.mbid2 ";

            Tsql = Tsql + " , tbl_Memberinfo.M_Name ";
            Tsql = Tsql + " , tbl_Memberinfo_Talk.TalkContent ";
            Tsql = Tsql + " , tbl_Memberinfo_Talk.Recordid ";            
            Tsql = Tsql + " , tbl_Memberinfo_Talk.Recordtime ";
            Tsql = Tsql + " , '','',''";

            Tsql = Tsql + " FROM tbl_Memberinfo_Talk  (nolock) ";
            Tsql = Tsql + " LEFT JOIN tbl_Memberinfo  (nolock) ON tbl_Memberinfo_Talk.Mbid = tbl_Memberinfo.Mbid And tbl_Memberinfo_Talk.Mbid2 = tbl_Memberinfo.Mbid2 ";
        ...
        {
            string strSql = " Where tbl_Memberinfo_Talk.Seq > 0  ";
                ...
                )
            {
                ...
                {
                    strSql = strSql + " And tbl_Memberinfo_Talk.Mbid = '" + Mbid + "'";
                    strSql = strSql + " And tbl_Memberinfo_Talk.Mbid2 = " + Mbid2;
                }
            }
            ...
            if ((mtxtCDate1.Text.Replace("-", "").Trim() != "") && (mtxtCDate2.Text.Replace("-", "").Trim() == ""))
                strSql = strSql + " And Replace(Left(tbl_Memberinfo_Talk.Recordtime,10),'-','')  = '" + mtxtCDate1.Text.Replace("-", "").Trim() + "'";

            //변경일자로 검색 -2
            if ((mtxtCDate1.Text.Replace("-", "").Trim() != "") && (mtxtCDate2.Text.Replace("-", "").Trim() != ""))
            {
                strSql = strSql + " And Replace(Left(tbl_Memberinfo_Talk.Recordtime,10),'-','') >= '" + mtxtCDate1.Text.Replace("-", "").Trim() + "'";
                strSql = strSql + " And Replace(Left(tbl_Memberinfo_Talk.Recordtime,10),'-','') <= '" + mtxtCDate2.Text.Replace("-", "").Trim() + "'";
            }

            //변경자로 검색한 경우
            if (txtR_Id_Code.Text.Trim() != "")
                strSql = strSql + " And tbl_Memberinfo_Talk.Recordid  = '" + txtR_Id_Code.Text.Trim() + "'";


            //////변경 내역이 있는 경우
            ////if (txtChange_Code.Text.Trim() != "")
            ////    strSql = strSql + " And M_Detail  = '" + txtChange_Code.Text.Trim() + "'";
           
            

            //////센타코드로으로 검색
            ////if (txtCenter_Code.Text.Trim() != "")
            ////    strSql = strSql + " And tbl_Memberinfo_Talk.BusinessCode = '" + txtCenter_Code.Text.Trim() + "'";
            
            //////주민번호로 검색           
            ////if (txtCpno.Text.Trim() != "")
            ////    strSql = strSql + " And tbl_Memberinfo_Talk.Cpno   = '" + encrypter.Encrypt(txtCpno.Text.Trim()) + "'";
            ...
            Tsql = Tsql + strSql ;
            Tsql = Tsql + " Order by  tbl_Memberinfo_Talk.Seq DESC  ";
        }
        ...
        {   
            ...
            for (int fi_cnt = 0; fi_cnt <= ReCnt - 1; fi_cnt++)
            {
                ...
                ////Record_Code = ds.Tables[base_db_name].Rows[fi_cnt]["ModRecordid"].ToString();
        ...
        private void dGridView_Base_Header_Reset()
        {
            ...
            string[] g_HeaderText = {""  , "회원번호"  , "성명"   , "상담_내역", "기록자"   
                    , "기록일자"   , ""    , ""  , ""  
                                    };
        ...
        private void Set_gr_dic(ref DataSet ds, ref Dictionary<int, object[]> gr_dic_text, int fi_cnt)
        {
            ...
            object[] row0 = { ds.Tables[base_db_name].Rows[fi_cnt][0]
                                ...
                                ,str_3
                                ,encrypter.Decrypt(ds.Tables[base_db_name].Rows[fi_cnt][4].ToString ())
