...
{
    public partial class frmMember_Select_Cafe_Select : clsForm_Extends
    {
       ...
       // private ContextMenu popUpMenu = new ContextMenu();
        ...
        public frmMember_Select_Cafe_Select()
        {
            InitializeComponent();                                 
        }
        ...
        private Boolean Check_TextBox_Error()
        {
            ...
            {
                MessageBox.Show("마감월을 필히 입력해 주십시요."
                          + "\n" +
                         cls_app_static_var.app_msg_rm.GetString("Msg_Re_Action"));
                mtxtRegDate1.Focus(); return false;
            }
        ...
        private void Make_Base_Query(ref string Tsql)
        {
            ...
            //Tsql = "Delete From tbl_Memberinfo_Ten_Temp Where Gid =  '" + cls_User.gid + "'";
            //if (Temp_Connect.Insert_Data(Tsql, "Tbl_User_Ver", this.Name.ToString(), this.Text) == false) return;

            //Tsql = " EXEC Usp_Insert_tbl_Memberinfo_Ten_Temp '" + mtxtRegDate1.Text.Trim().Replace("-", "") + "', '" + cls_User.gid + "' ";
            //if (Temp_Connect.Insert_Data(Tsql, "Tbl_User_Ver", this.Name.ToString(), this.Text) == false) return;
            ...
            Tsql = Tsql + " , Grade30_ID ";
            Tsql = Tsql + " , Grade30_Name ";
            Tsql = Tsql + " , Case When tbl_Memberinfo.LeaveDate ='' then  ISNULL(C1.Grade_Name,'') ELSE '' End  Grade_Name ";

            Tsql = Tsql + " , Case When Isnull( tbl_Memberinfo_Ten.Mbid,'') = '' then '' ELSE 'Y' End ";

            //Tsql = Tsql + " , dbo.ufn_Ten_Mem_Count (tbl_Memberinfo.mbid ,'" + mtxtRegDate1.Text.Trim().Replace("-", "") + "') ";  //연속개월수
            //Tsql = Tsql + " ,Isnull(tbl_Memberinfo_Ten_Temp.Ten_Cnt , 0 ) ";
            Tsql = Tsql + " , 0 ";  //사용하지 않는 내역이기에 시간이 걸리기 때문에 계산을 하지 마라..
            Tsql = Tsql + " , isnull(M_Cafe.Cafe_Count,0) Cafe_Count  ";  // 마감월에 사용한 횟수


            Tsql = Tsql + " From tbl_Memberinfo (nolock) ";
            //Tsql = Tsql + " LEFT JOIN (Select Sum(Cafe_Count) AS Cafe_Count ,Mbid From tbl_Memberinfo_Cafe (nolock) Where LEFT(Replace(LEFT(RecordTime,10),'-',''),6)  = '" + mtxtRegDate1.Text.Trim ().Replace ("-","").Substring (0,6) + "' Group by Mbid ) ";
            Tsql = Tsql + " LEFT JOIN (Select Sum(Cafe_Count) AS Cafe_Count ,Mbid From tbl_Memberinfo_Cafe (nolock) Where LEFT(Replace(LEFT(RecordTime,10),'-',''),6)  = LEFT(convert(varchar, dateadd(month,2,'" + mtxtRegDate1.Text.Trim().Replace("-", "") + "') , 112),6)  Group by Mbid ) ";
            Tsql = Tsql + " AS M_Cafe on M_Cafe.Mbid = tbl_Memberinfo.Mbid ";

            Tsql = Tsql + " Left Join ufn_Search_F_Up_KR () AS Up_M ON Up_M.Mbid = tbl_Memberinfo.Mbid ";
           // Tsql = Tsql + " Left Join tbl_Memberinfo_Ten (nolock) ON tbl_Memberinfo_Ten.Mbid = tbl_Memberinfo.Mbid And   LEFT(App_Month ,6 ) = LEFT(convert(varchar, dateadd(month,-2,'" + mtxtRegDate1.Text.Trim().Replace("-", "") + "') , 112),6) "; 
            Tsql = Tsql + " Left Join tbl_Memberinfo_Ten (nolock) ON tbl_Memberinfo_Ten.Mbid = tbl_Memberinfo.Mbid And   LEFT(App_Month ,6 ) = '" + mtxtRegDate1.Text.Trim().Replace("-", "").Substring (0,6) + "'  "; 
        ...
        {
            string strSql = " Where (tbl_Memberinfo.Mbid IN (Select Distinct Mbid From tbl_Memberinfo_Ten (nolock) ) OR tbl_Memberinfo.SOL_S_Date <> '' )  ";
            strSql = strSql + " And  (Isnull( tbl_Memberinfo_Ten.Mbid,'') <> '' OR isnull(M_Cafe.Cafe_Count,0) <> 0 ) ";   //마감월 대상자만 보여지게 처리함.
            ...
            //    strSql = strSql + " And tbl_Memberinfo.BusinessCode <> '' ";
        ...
        {   
            ...
            cgb.basegrid.Columns[0].AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells;

            //for (int fi_cnt = 0; fi_cnt <= ReCnt - 1; fi_cnt++)
            //{
            //    if (ds.Tables[base_db_name].Rows[fi_cnt][21].ToString() == "N")
            //    {
            //        cgb.basegrid.Rows[fi_cnt].DefaultCellStyle.BackColor = System.Drawing.Color.PaleGoldenrod;
            //    }

            //    if (ds.Tables[base_db_name].Rows[fi_cnt][24].ToString() != "")
            //    {
            //        cgb.basegrid.Rows[fi_cnt].DefaultCellStyle.BackColor = System.Drawing.Color.PaleVioletRed;
            //    }
        ...
        private void dGridView_Base_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            //주민번호 복호화
            if (e.ColumnIndex == 3 && e.RowIndex != this.dGridView_Base.NewRowIndex)
            {
                e.Value = encrypter.Decrypt(e.Value.ToString(), "Cpno");
            }
        }
        ...
        private void dGridView_Base_Header_Reset()
        {
            ...
            //Tsql = Tsql + " , M_Cafedbo.ufn_Ten_Mem_Count ('',tbl_Memberinfo.mbid ) ";  //연속개월수
            //Tsql = Tsql + " , isnull(M_Cafe.Cafe_Count,0) Cafe_Count  ";  // 마감월에 사용한 횟수



            string[] g_HeaderText = {"" ,"회원_번호"  , "성명" , "직급" ,"연락처" 
                                ,"관리운영위원","관리운영위원명", "_직급"     , "_텐밀리언즈여부"   , "_연속개월수"   
                                , "음료이용횟수"     
                                
                                    };
            cgb.grid_col_header_text = g_HeaderText;

            int[] g_Width = { 0 , 100, 100, 100,130
                             , 120, 120 ,0, 0, 0
                             , 100
        ...
        private void Db_Grid_Popup(TextBox tb, TextBox tb1_Code)
        {
            ...
            if (tb.Name == "txtCenter")
                cgb_Pop.Next_Focus_Control = mtxtMbid ;
        ...
        private Boolean Chang_CenterCode()
        {
            ...
            //            StrSql = StrSql + " BusinessCode = '" + txtCenter2_Code.Text.Trim() + "'";
            ...
            //            BeforeDetail = dGridView_Base.Rows[i].Cells[28].Value.ToString();  //전 센타코드
