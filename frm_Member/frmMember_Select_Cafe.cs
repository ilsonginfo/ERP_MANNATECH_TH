...
{
    public partial class frmMember_Select_Cafe : clsForm_Extends
    {
                
        cls_Grid_Base cgb = new cls_Grid_Base();
        cls_Grid_Base cg_Sub = new cls_Grid_Base();

        private const string base_db_name = "tbl_Memberinfo_Cafe";
        private int Data_Set_Form_TF;

        public frmMember_Select_Cafe()
        {
            InitializeComponent();
        ...
        private void frmBase_From_Load(object sender, EventArgs e)
        {
            ...
            mtxtDate.Mask = cls_app_static_var.Date_Number_Fromat;
            mtxtDate.Text = cls_User.gid_date_time;
        ...
        private void Form_Clear_()
        {
            ...
            txt_BarCode.BorderStyle = BorderStyle.Fixed3D;
            //txtName.BorderStyle = BorderStyle.Fixed3D;
            //mtxtSellDate.BorderStyle = BorderStyle.Fixed3D;

            txt_BarCode.BackColor =  SystemColors.Window;
            ...
            cls_form_Meth ct = new cls_form_Meth();
            //ct.from_control_clear(this, txt_BarCode);
        ...
        private void Base_Sub_Grid_Set()
        {
            ...
            Tsql = "Select a.mbid2,A.M_NAME, b.[date] from tbl_memberinfo (nolock) a join tbl_memberinfo_barcord b on a.mbid2 = b.mbid2";
        ...
        private void Save_Base_Data(ref int Save_Error_Check)
        {
            ...
            {            
                ...
                StrSql = "INSERT INTO tbl_Memberinfo_Cafe ";
                StrSql = StrSql + " (";
                StrSql = StrSql + "  mbid , mbid2 , M_Name ";
                StrSql = StrSql + " , App_Month, Cafe_Count  ";
                StrSql = StrSql + " ,  ETC_1 ";
        ...
        {                      
            ...
            try
            {
                ...
                StrSql = "INSERT INTO tbl_Memberinfo_Cafe_Mod ";
                StrSql = StrSql + " Select  * ";
                StrSql = StrSql + ",'" + cls_User.gid + "'";
                StrSql = StrSql + ",Convert(Varchar(25),GetDate(),21) ";
                StrSql = StrSql + " From tbl_Memberinfo_Cafe (nolock) ";
                StrSql = StrSql + " Where seq = " + txtKey.Text; 

                Temp_Connect.Insert_Data(StrSql, base_db_name, Conn, tran);


                StrSql = "UpDate  tbl_Memberinfo_Cafe Set ";
                StrSql = StrSql + ", App_Month = ''";                            
                StrSql = StrSql + " Where seq = " + txtKey.Text; 
        ...
        private void Delete_Base_Data(ref int Delete_Error_Check)
        {
            ...
            try
            {
                ...
                StrSql = "Insert into  tbl_Memberinfo_Cafe_Mod ";
                StrSql = StrSql + " Select *  ";
                StrSql = StrSql + ",'" + cls_User.gid + "', Convert(Varchar(25),GetDate(),21) From tbl_Memberinfo_Cafe (nolock) ";
                StrSql = StrSql + " Where seq = " + txtKey.Text;
                
                Temp_Connect.Insert_Data(StrSql, base_db_name, Conn, tran);


                StrSql = "Delete From tbl_Memberinfo_Cafe  ";
                StrSql = StrSql + " Where seq = " + txtKey.Text;
        ...
        private void dGridView_Base_Sub_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (((sender as DataGridView).CurrentRow != null) && ((sender as DataGridView).CurrentRow.Cells[0].Value != null))
            {
                ...
                //string[] g_HeaderText = {"회원번호",  "성명"  , "_적용_일자"   , "이용횟수"  , "_구매_종류"          
                ...
                txt_BarCode.BorderStyle = BorderStyle .FixedSingle ;
                //txtName.BorderStyle = BorderStyle.FixedSingle ;
                txt_BarCode.BackColor = cls_app_static_var.txt_Enable_Color;
                //txtName.BackColor = cls_app_static_var.txt_Enable_Color;
                ...
                string Tsql = "";
                Tsql = "Select  ";
                Tsql = Tsql + " Count(Mbid)  Cafe_Count ";
                Tsql = Tsql + " From tbl_Memberinfo_Cafe (nolock) ";
                Tsql = Tsql + " Where Mbid = '" + T_Mbid.Trim() + "'";
                Tsql = Tsql + " And  Replace(LEFT(RecordTime,10),'-','')  = LEFT(Convert(Varchar, getdate(),112) ,8)  "; 
                Tsql = Tsql + " And  Seq <> " + txtKey.Text;
                Tsql = Tsql + " Group by Mbid  ";   
                ...
                if (ReCnt > 0)
                {
                    Cafe_Count = int.Parse(ds.Tables[base_db_name].Rows[0]["Cafe_Count"].ToString());
                }

                Tsql = " Select SoL_FLAG , Ten_FLAG  From  ufn_Mem_Ten_BarCode_Base_1 ('" + T_Mbid.Trim() + "', 0) ";

                DataSet ds_2 = new DataSet();
                //테이블에 맞게  DataSet에 내역을 넣고 제대로되었으면 true가 오고 아니면 걍 튀어나간다.
                if (Temp_Connect.Open_Data_Set(Tsql, base_db_name, ds_2) == false) return;
                int ReCnt2 = Temp_Connect.DataSet_ReCount;

                if (ReCnt2 > 0)
                {
                    int Cafe_Max_Cnt = 0;
                    if (ds_2.Tables[base_db_name].Rows[0]["SoL_FLAG"].ToString() == "Y") Cafe_Max_Cnt += 3;
                    if (ds_2.Tables[base_db_name].Rows[0]["Ten_FLAG"].ToString() == "Y") Cafe_Max_Cnt += 3;

                    if (Cafe_Max_Cnt - Cafe_Count > 0) ;
        ...
        private void Set_Form_Date(string T_Mbid)
        {
            ...
            if (csb.Member_Nmumber_Split(T_Mbid, ref Mbid, ref Mbid2) == 1)
            {
                ...
                Tsql = Tsql + " , tbl_Memberinfo.membership "; 
                ...
                txt_BarCode.Text = ds.Tables[base_db_name].Rows[0]["membership"].ToString();
        ...
        private void DTP_Base_CloseUp(object sender, EventArgs e)
        {
            ...
            if (dtp.Name == "DTP_Date")
                ct.form_DateTimePicker_Search_TextBox(this, (DateTimePicker)sender, mtxtDate);
        ...
        private void txt_BarCode_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                txt_BarCode_KeyDown(null, new KeyEventArgs(Keys.Enter));
            }
        }
        ...
        private void SetBarcode()
        {
            ...
            string Tsql = "";
            Tsql = "Select a.mbid2, a.m_name,b.[date] from tbl_memberinfo (nolock) a join tbl_memberinfo_barcord b on a.mbid = '' AND a.mbid2 = b.mbid2";
            //Tsql = "Select  ";
            //Tsql = Tsql + " tbl_Memberinfo.Mbid, tbl_Memberinfo.M_Name, tbl_Memberinfo.CurGrade ";
            //Tsql = Tsql + ",tbl_Memberinfo.SOL_S_Date , tbl_Memberinfo.SOL_E_Date ";
            //Tsql = Tsql + ", Isnull(M_Cafe.Cafe_Count,0 ) Cafe_Count "; 
            //Tsql = Tsql + ", Convert(Varchar, getdate(),112)  No_Date ";
            //Tsql = Tsql + ", Isnull(Grade_Name,'') Grade_Name  ";

            //Tsql = Tsql + " From tbl_Memberinfo (nolock) ";
            //Tsql = Tsql + " LEFT JOIN (Select Sum(Cafe_Count) AS Cafe_Count ,Mbid From tbl_Memberinfo_Cafe (nolock) Where Replace(LEFT(RecordTime,10),'-','')  = LEFT(Convert(Varchar, getdate(),112) ,8) Group by Mbid ) ";
            //Tsql = Tsql + " AS M_Cafe on M_Cafe.Mbid = tbl_Memberinfo.Mbid ";
            //Tsql = Tsql + " LEFT JOIN  tbl_Class  (nolock) ON tbl_Class.Grade_Cnt = tbl_Memberinfo.CurGrade  ";
            Tsql = Tsql + " Where a.membership = '" + txt_BarCode.Text.Trim() + "'";
            Tsql = Tsql + " AND  b.date = '" + mtxtDate.Text.Replace("-", "").Replace("_", "") + "'";
            ...
            int ReCnt = Temp_Connect.DataSet_ReCount;
            //김영수 팀장님이 있든 없든 메세지 띄울것을 요청주심
            if (ReCnt == 0)
            {
                Data_Set_Form_TF = 0;

                string Msg = " 컨벤션 날짜 " + mtxtDate.Text + "일의 참석자가 아닙니다.  "
                      + Environment.NewLine +
                      cls_app_static_var.app_msg_rm.GetString("Msg_Re_Action");
            ...
            else if (ReCnt >= 1)
            {
                ...
                txtName.Text = ds.Tables[base_db_name].Rows[0]["M_Name"].ToString();
                string Msg = " 컨벤션 날짜 " + mtxtDate.Text + "일의 컨벤션 참석자가 맞습니다.  " +
                    " \n 회원아이디 " + mtxtMbid.Text + "번 " + txtName.Text + "님";
