...
namespace MLM_Program
{
    public partial class frmMember_TreeView_Nom : clsForm_Extends
    {
        ...
        private void Make_TreeView_Save()
        {
            ...
            cls_Connect_DB Temp_Connect = new cls_Connect_DB();
            string Sql = "";
            string SDate1 = txtSellDate1.Text ;
            string SDate2 = txtSellDate2.Text ;

            if (SDate1 == "" ) SDate1 = "19000101" ;
            if (SDate2 == "" ) SDate2 = "21001231";

            Sql = "select ufn_GetSubTreeView_Sell_Nom.Mbid ";
            Sql = Sql + " , ufn_GetSubTreeView_Sell_Nom.Mbid2 ";
            Sql = Sql + " , ufn_GetSubTreeView_Sell_Nom.M_Name " ;
            Sql = Sql + " , lvl ";
            Sql = Sql + " , ufn_GetSubTreeView_Sell_Nom.idx_Nomin " ;
            Sql = Sql + " , ufn_GetSubTreeView_Sell_Nom.idx_Nomin2";
            Sql = Sql + " , Level_Cnt";
            Sql = Sql + " , ufn_GetSubTreeView_Sell_Nom.Save_Cur";
            Sql = Sql + " , MaxLevel  ";
            Sql = Sql + " , idx_Save ";
            Sql = Sql + " , idx_Save2 ";
            Sql = Sql + " , B_Name ";    //후원인명
            ...
            Sql = Sql + " From ufn_GetSubTreeView_Sell_Nom('" + Mbid + "'," + Mbid2 + ",'" + SDate1 + "','" + SDate2 + "','" + combo_Se_Code.Text.Trim() + "')";

            Sql = Sql + " Where ufn_GetSubTreeView_Sell_Nom.LineCnt <>0 ";
            Sql = Sql + " And   ufn_GetSubTreeView_Sell_Nom.LineCnt <= 100 ";
            Sql = Sql + " And   Lvl >= 0 ";
            Sql = Sql + " Order by lvl asc, ufn_GetSubTreeView_Sell_Nom.Save_Cur asc";
        ...
        private void Make_Tree_Text( DataSet ds, int RowCnt, ref string  TreeText, string  TextMbid  , string  TName)
        {
            ...
            if (chb_7.Checked == true)
            {
                ...
                if (cls_app_static_var.Member_Number_1 > 0)
                    TreeText = TreeText + " ▶" + cm._chang_base_caption_search("후원인") + ":" + T_N_Mbid + "-" + T_N_Mbid2.ToString();
                else
                    TreeText = TreeText + " ▶" + cm._chang_base_caption_search("후원인") + ":" + T_N_Mbid2.ToString();
            }
            if (chb_8.Checked == true) TreeText = TreeText + " " + ds.Tables["Tree_Save"].Rows[RowCnt]["B_Name"].ToString();
