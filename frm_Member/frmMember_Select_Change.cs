...
{
    public partial class frmMember_Select_Change : clsForm_Extends
    {
        ...
        public frmMember_Select_Change()
        {
            InitializeComponent();
        ...
        {
            string strSql = " Where Ch_T." + cls_app_static_var.Base_M_Detail_Ex + " IS NOT NULL";
            ...
            if ((mtxtCDate1.Text.Replace("-", "").Trim() != "") && (mtxtCDate2.Text.Replace("-", "").Trim() == ""))
                strSql = strSql + " And Replace(Left(A.ModRecordtime,10),'-','')  = '" + mtxtCDate1.Text.Replace("-", "").Trim() + "'";

            //변경일자로 검색 -2
            if ((mtxtCDate1.Text.Replace("-", "").Trim() != "") && (mtxtCDate2.Text.Replace("-", "").Trim() != ""))
            {
                strSql = strSql + " And Replace(Left(A.ModRecordtime,10),'-','') >= '" + mtxtCDate1.Text.Replace("-", "").Trim() + "'";
                strSql = strSql + " And Replace(Left(A.ModRecordtime,10),'-','') <= '" + mtxtCDate2.Text.Replace("-", "").Trim() + "'";
            }

            //변경자로 검색한 경우
            if (txtR_Id_Code.Text.Trim() != "")
                strSql = strSql + " And A.ModRecordid  = '" + txtR_Id_Code.Text.Trim() + "'";


            //변경 내역이 있는 경우
            if (txtChange_Code.Text.Trim() != "")
                strSql = strSql + " And ChangeDetail COLLATE Korean_Wansung_CS_AS  = '" + txtChange_Code.Text.Trim() + "'";
            ...
            Tsql = Tsql + strSql ;
            Tsql = Tsql + " Order by  A.ModRecordtime DESC  ";
        }
        ...
        private void dGridView_Base_Header_Reset()
        {
            ...
            string[] g_HeaderText = {"회원_번호"  , "성명"  , "핸드폰"   , "주민번호"  , "변경내역"   , "전_내역"        
                                , "후_내역"   , "변경자"    , "변경일"  , "센타명"
                                    };
            ...
            int[] g_Width = { 85, 90,  100, 0, 100, 100  
                             ,100, 90 , 150 , 100
                            };
        ...
        private void Set_gr_dic(ref DataSet ds, ref Dictionary<int, object[]> gr_dic_text, int fi_cnt)
        {
            ...
            cls_form_Meth cm = new cls_form_Meth();
            str_3 = cm._chang_base_caption_search(ds.Tables[base_db_name].Rows[fi_cnt][4].ToString());
