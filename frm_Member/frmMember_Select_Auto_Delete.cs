...
{
    public partial class frmMember_Select_Auto_Delete : clsForm_Extends
    {
        ...
        public frmMember_Select_Auto_Delete()
        {
            InitializeComponent();
        }           
        ...
        private void Save_ADS_End(ref int Save_Error_Check)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < dGridView_Base.Rows.Count; i++)
                {
                    if (dGridView_Base.Rows[i].Cells[0].Value.ToString() == "V")
                    {
                        int idx_Mbid2 = int.Parse (dGridView_Base.Rows[i].Cells[2].Value.ToString());
                        ...
                        {
                            StrSql = "EXEC [Usp_TH_SMS]   " + idx_Mbid2 + ",'','','3'";  //오토쉽 결제 해지
                        }
                        else
                        {
                            StrSql = "EXEC Usp_Insert_SMS_New  '23',''," + idx_Mbid2 + ",'" + Auto_Seq + "', ''";  //오토쉽 결제 해지 처리
                        }
                        //StrSql = "EXEC Usp_Insert_SMS '23',''," + idx_Mbid2 + ",'" + Auto_Seq + "', ''";  //오토쉽 결제 해지 처리
            ...
            try
            {
                ...
                for (int i = 0; i < dGridView_Base.Rows.Count; i++)
                {
                    if (dGridView_Base.Rows[i].Cells[0].Value.ToString() == "V")
                    {
                        ...
                        sb.AppendLine("SELECT [Auto_Seq],[mbid],[mbid2],[Req_Type], '30' as [Req_State],[Req_Date],[Req_Month],[AutoExtend],[Start_Date],[End_Date]");
                        sb.AppendLine(",[Extend_Date],[Proc_Date],[NextProc_Date],[TotalPrice],[TotalPV],[TotalCV],[Etc],[End_Reason],[Proc_Cnt],[DeliveryCharge]");
                        sb.AppendLine(",[CustomerGroupKey],[RecordID],[RecordTime]");
