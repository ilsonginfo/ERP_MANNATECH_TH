...
{
    public partial class frmMember_Nom_Change : clsForm_Extends
    {
        ...
        private Boolean  Input_Error_Check(MaskedTextBox  m_tb, int s_Kind )
        {
           ...
           // // Tsql = Tsql + " And  tbl_Memberinfo.Full_Save_TF  = 1 ";
            ...
            if (s_Kind == 3) //3인 경우는 새로운 지정 후원인인데.. 탈퇴나 라인중자가 아닌지를 체크한다.
            {
                ...
                //if (int.Parse(ds.Tables[base_db_name].Rows[0]["Sell_Mem_TF"].ToString()) == 1)
                //{
                ...
                if (Nominid_Loop_Check(Mbid, Mbid2) == false)
                {
                    m_tb.Focus(); return false;
                }
            }
        ...
        private Boolean Nominid_Loop_Check(string Mbid, int Mbid2)
        {
            ...
            //새로입력되는 상위 아이디를 검색했을때 내 위에 대상회원의 회원번호가 있는지를 체크한다.
            string Tsql = "";
            Tsql = "Select  ";           
            Tsql = "Select Mbid , Mbid2 , M_Name , lvl , Save_Cur " ;
            Tsql = Tsql + " From ufn_NominUp_Member_Search ('" + Mbid + "'," + Mbid2 + ") ";
        ...
        private void Set_Form_Date(string  T_Mbid)
        {
            ...
            if (csb.Member_Nmumber_Split(T_Mbid, ref Mbid, ref Mbid2) == 1)
            {
               ...
               Tsql = Tsql + ",  tbl_Memberinfo.Cpno  AS Cpno";
                ...
                Tsql = Tsql + " , Isnull(Nom.M_Name,'') AS Nom_Name ";
               Tsql = Tsql + ",  Nom.Cpno AS Nom_Cpno";

                Tsql = Tsql + " , LEFT(Nom.RegTime,4) +'-' + LEFT(RIGHT(Nom.RegTime,4),2) + '-' + RIGHT(Nom.RegTime,2)  As Nom_RegTime  ";
                Tsql = Tsql + " , Nom.hptel AS Nom_hptel ";
                Tsql = Tsql + " , Nom.address1 +'-' + Nom.address2 AS Nom_Address ";
                ...
                mtxtMbid2.Text = ds.Tables[base_db_name].Rows[0]["T_Nominid"].ToString();
                txtName2.Text = ds.Tables[base_db_name].Rows[0]["Nom_Name"].ToString();
                txtCpno2.Text = encrypter.Decrypt(ds.Tables[base_db_name].Rows[0]["Nom_Cpno"].ToString(), "Cpno");
                txtRDate2.Text = ds.Tables[base_db_name].Rows[0]["Nom_RegTime"].ToString();
                txtHptel2.Text = encrypter.Decrypt(ds.Tables[base_db_name].Rows[0]["Nom_hptel"].ToString());
        ...
        private void Set_Form_Date(string T_Mbid, int T_sort)
        {
            ...
            if (csb.Member_Nmumber_Split(T_Mbid, ref Mbid, ref Mbid2) == 1)
            {
                ...
                //if (cls_app_static_var.Member_Number_1 > 0)
                //    Tsql = Tsql + " ,tbl_Memberinfo.Nominid + '-' + Convert(Varchar,tbl_Memberinfo.Nominid2) AS Nominid ";
                //else
                //    Tsql = Tsql + " ,tbl_Memberinfo.Nominid2 AS Nominid ";
                ...
                Tsql = Tsql + " From tbl_Memberinfo (nolock) ";
                //Tsql = Tsql + " LEFT JOIN tbl_Memberinfo Sav (nolock) ON tbl_Memberinfo.Nominid = Sav.Mbid And tbl_Memberinfo.Nominid2 = Sav.Mbid2 ";
                //Tsql = Tsql + " LEFT JOIN tbl_Memberinfo Nom (nolock) ON tbl_Memberinfo.Nominid = Nom.Mbid And tbl_Memberinfo.Nominid2 = Nom.Mbid2 ";
                //Tsql = Tsql + " LEFT JOIN tbl_Business (nolock) ON tbl_Memberinfo.BusinessCode = tbl_Business.NCode ";
                //Tsql = Tsql + " Left Join tbl_Bank On tbl_Memberinfo.bankcode=tbl_Bank.ncode ";
                //Tsql = Tsql + " Left Join tbl_Class C1 On tbl_Memberinfo.CurGrade=C1.Grade_Cnt ";          
                ...
                txtCpno3.Text = encrypter.Decrypt(ds.Tables[base_db_name].Rows[0]["Cpno"].ToString(),"Cpno");

                //if (txtCpno3.Text != "")
                //    txtCpno3.Text = txtCpno3.Text.Substring(0, 6) + "*******";
        ...
        private Boolean Check_TextBox_Error()
        {
            ...
            string New_Nominid = ""; int New_Nominid2 = 0;
            string Old_Nominid = ""; int Old_Nominid2 = 0;

            string T_Mbid = mtxtMbid2.Text;
            cls_Search_DB csb = new cls_Search_DB();
            if (csb.Member_Nmumber_Split(T_Mbid, ref Old_Nominid, ref Old_Nominid2) != 1)//입력된 회원번호 내역이 올바르게 입력 되었는지를 체크함
            {
                //mtxtMbid.Focus(); return false;
            }
            ...
            if (csb.Member_Nmumber_Split(T_Mbid, ref New_Nominid, ref New_Nominid2) != 1)
            {
                mtxtMbid3.Focus(); return false;
            }
            ...
            if ((New_Nominid == Old_Nominid) && (New_Nominid2 == Old_Nominid2) )
            {
            ...
            if ((New_Nominid == Self_Mbid) && (New_Nominid2 == Self_Mbid2))
            {
        ...
        private void Save_Base_Data(ref int Save_Error_Check)
        {
            ...
            int N_LineCnt = csd.N_LineCnt_Search_Nom(NEW_idx, NEW_idx2);

            cls_Connect_DB Temp_Connect = new cls_Connect_DB();

            string StrSql = "";
            StrSql = "Update tbl_Memberinfo Set ";
            StrSql = StrSql + "  Nominid = '" + NEW_idx + "'";
            StrSql = StrSql + " ,Nominid2 = " + NEW_idx2.ToString();
            StrSql = StrSql + " ,N_LineCnt = " + N_LineCnt;
        ...
        private void mtxtMbid_Click(object sender, EventArgs e)
        {
            ...
            //ct.from_control_clear(grB_Search, mtb);
