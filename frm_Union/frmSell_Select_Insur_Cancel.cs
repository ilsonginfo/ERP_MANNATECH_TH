...
{
    public partial class frmSell_Select_Insur_Cancel : clsForm_Extends
    {
        ...
        public frmSell_Select_Insur_Cancel()
        {
            InitializeComponent();
        }
        ...
        private void Make_Base_Query(ref string Tsql)
        {
            ...
            if (cls_app_static_var.Member_Number_1 > 0)
                sb.AppendLine( " tbl_SalesDetail.mbid + '-' + Convert(Varchar,tbl_SalesDetail.mbid2) ");
            else
                sb.AppendLine( " tbl_SalesDetail.mbid2 ");

            sb.AppendLine( " ,tbl_SalesDetail.M_Name ");

            sb.AppendLine( ", tbl_Memberinfo.Cpno ");

            sb.AppendLine( " , ISNULL(C1.Grade_Name,'') ");

            sb.AppendLine(" ,ISNULL(RS1.OrderNumber, tbl_SalesDetail.OrderNumber) OrderNumber");
            sb.AppendLine( " ,Isnull(tbl_Business.Name,'') as B_Name");
            sb.AppendLine(" ,CONVERT(VARCHAR, CAST(ISNULL(RS1.SellDate, tbl_SalesDetail.SellDate) AS DATETIME), 23) SellDate");
            sb.AppendLine( " , tbl_SellType.SellTypeName SellCodeName  ");

            sb.AppendLine( " ,Case When tbl_SalesDetail.ReturnTF = 5 Then '" + cm._chang_base_caption_search("취소") + "'");
            sb.AppendLine( "  When RS1.ReturnTF = 2 Then '" + cm._chang_base_caption_search("반품") + "'");
            sb.AppendLine( "  When RS1.ReturnTF = 3 Then '" + cm._chang_base_caption_search("부분반품") + "'");
            sb.AppendLine( " END ReturnTFName ");

            sb.AppendLine( " , '' ");
            sb.AppendLine( " , tbl_SalesDetail.TotalPrice ");
            sb.AppendLine( " , tbl_SalesDetail.TotalInputPrice ");
            sb.AppendLine( " , '' ");
            sb.AppendLine( " , '' ");
            sb.AppendLine( " , '' ");
            ...
            sb.AppendLine( " , '' ");
            sb.AppendLine( " , '' ");
            sb.AppendLine( " , Case  When tbl_Memberinfo.Sell_Mem_TF = 0 then 'BP' ELSE  'PC' End");


            sb.AppendLine( " , Case tbl_Memberinfo.LeaveCheck When 1 then '활동' When 0 then '탈퇴' End");
            sb.AppendLine( " , Case tbl_Memberinfo.LineUserCheck When 1 then '사용' When 0 then '중지' End ");
            sb.AppendLine( " , Case When tbl_Memberinfo.LeaveDate <> '' Then  LEFT(tbl_Memberinfo.LeaveDate,4) +'-' + LEFT(RIGHT(tbl_Memberinfo.LeaveDate,4),2) + '-' + RIGHT(tbl_Memberinfo.LeaveDate,2) ELSE '' End ");
            sb.AppendLine( " , Case When tbl_Memberinfo.LineUserDate <> '' Then  LEFT(tbl_Memberinfo.LineUserDate,4) +'-' + LEFT(RIGHT(tbl_Memberinfo.LineUserDate,4),2) + '-' + RIGHT(tbl_Memberinfo.LineUserDate,2) ELSE '' End ");
            sb.AppendLine( " , tbl_SalesDetail.recordid  ");

            sb.AppendLine( " , tbl_SalesDetail.recordtime ");
            sb.AppendLine( " , tbl_SalesDetail.BusCode ");


            sb.AppendLine( " From tbl_SalesDetail (nolock) ");
            sb.AppendLine(" LEFT JOIN tbl_SalesDetail_TF (nolock) ON tbl_SalesDetail.OrderNumber = tbl_SalesDetail_TF.OrderNumber");
            ...
            sb.AppendLine(" LEFT JOIN tbl_Memberinfo Sav (nolock) ON tbl_Memberinfo.Saveid = Sav.Mbid And tbl_Memberinfo.Saveid2 = Sav.Mbid2");
            ...
            sb.AppendLine(" LEFT JOIN tbl_Business(nolock) ON tbl_SalesDetail.BusCode = tbl_Business.NCode");
            ...
            sb.AppendLine(" LEFT Join tbl_SalesDetail RS1(NOLOCK) ON tbl_SalesDetail.OrderNumber = RS1.Re_BaseOrderNumber");
            sb.AppendLine(" LEFT JOIN  TLS_ASSOCIATION_LOG(NOLOCK) ON REQ_TYPE = 'C' AND REQ_TYPE = 'C' AND CANCEL_RESULT = 'Y' AND tbl_SalesDetail.OrderNumber = ORDER_ID");
            sb.AppendLine(" LEFT JOIN tbl_Sales_Insu(NOLOCK) ON tbl_Sales_Insu.Send_Flag = 'Y' AND tbl_Sales_Insu.Back_Flag = 'Y' AND tbl_Sales_Insu.OrderNumber = tbl_SalesDetail.OrderNumber ");
        ...
        private void Make_Base_Query_(ref string Tsql)
        {
            ...
            sb.AppendLine("Where tbl_SalesDetail.ReturnTF in (1, 5)");
            sb.AppendLine("And (");
            sb.AppendLine(" (tbl_SalesDetail.InsuranceNumber <> '' AND tbl_SalesDetail.ReturnTF = 5 AND RS1.ReturnTF = 1 AND tbl_SalesDetail.InsuranceNumber_Cancel <> 'Y')");
            sb.AppendLine(" OR (tbl_SalesDetail.InsuranceNumber <> '' AND tbl_SalesDetail.ReturnTF = 1 AND ISNULL(RS1.ReturnTF, 0) = 2 AND  tbl_SalesDetail.InsuranceNumber_Cancel <> 'Y')");
            sb.AppendLine(" OR (tbl_SalesDetail.InsuranceNumber <> '' AND tbl_SalesDetail.ReturnTF = 1 AND ISNULL(RS1.ReturnTF, 0) = 3 AND RS1.InsuranceNumber_Cancel <> 'Y'))");
            sb.AppendLine(" AND tbl_Sales_Insu.OrderNumber IS NULL");
            sb.AppendLine(" AND ISNULL(tbl_SalesDetail.INS_Num_Cancel_Err, '') <> '3001'");
            sb.AppendLine(" AND ISNULL(RS1.INS_Num_Cancel_Err, '') <> '3001'");
            sb.AppendLine(" And  tbl_SalesDetail.W_T_TF = 0  ");
            ...
            if ((mtxtRegDate1.Text.Replace("-","").Trim() != "") && (mtxtRegDate2.Text.Replace("-","").Trim() == ""))
                sb.AppendLine(" And tbl_SalesDetail.SellDate = '" + mtxtRegDate1.Text.Replace("-","").Trim() + "'");

            //가입일자로 검색 -2
            if ((mtxtRegDate1.Text.Replace("-","").Trim() != "") && (mtxtRegDate2.Text.Replace("-","").Trim() != ""))
            {
                sb.AppendLine(" And tbl_SalesDetail.SellDate >= '" + mtxtRegDate1.Text.Replace("-","").Trim() + "'");
                sb.AppendLine(" And tbl_SalesDetail.SellDate <= '" + mtxtRegDate2.Text.Replace("-","").Trim() + "'");
            }


            //기록일자로 검색 -1
            if ((mtxtMakDate1.Text.Replace("-", "").Trim() != "") && (mtxtMakDate2.Text.Replace("-", "").Trim() == ""))
                sb.AppendLine(" And Replace(Left( tbl_SalesDetail.recordtime ,10),'-','') = '" + mtxtMakDate1.Text.Replace("-", "").Trim() + "'");

            //기록일자로 검색 -2
            if ((mtxtMakDate1.Text.Replace("-", "").Trim() != "") && (mtxtMakDate2.Text.Replace("-", "").Trim() != ""))
            {
                sb.AppendLine(" And Replace(Left( tbl_SalesDetail.recordtime ,10),'-','') >= '" + mtxtMakDate1.Text.Replace("-", "").Trim() + "'");
                sb.AppendLine(" And Replace(Left( tbl_SalesDetail.recordtime ,10),'-','') <= '" + mtxtMakDate2.Text.Replace("-", "").Trim() + "'");
            }
            ...
            if (txtCenter_Code.Text.Trim() != "")
                sb.AppendLine(" And tbl_SalesDetail.BusCode = '" + txtCenter_Code.Text.Trim() + "'");

            if (txtR_Id_Code.Text.Trim() != "")
                sb.AppendLine(" And tbl_SalesDetail.recordid = '" + txtR_Id_Code.Text.Trim() + "'");

            //if (txtBank_Code.Text.Trim() != "")
            //    sb.AppendLine(" And tbl_Memberinfo.BankCode = '" + txtBank_Code.Text.Trim() + "'");

            if (opt_Leave_2.Checked == true)
                sb.AppendLine(" And tbl_Memberinfo.LeaveDate = ''");

            if (opt_Leave_3.Checked == true)
                sb.AppendLine(" And tbl_Memberinfo.LeaveDate <> ''");

            if (opt_Line_2.Checked == true)
                sb.AppendLine(" And tbl_Memberinfo.LineUserDate = ''");
            ...
            if (opt_sell_2.Checked == true)
                sb.AppendLine(" And tbl_Memberinfo.Sell_Mem_TF = 0 ");

            if (opt_sell_3.Checked == true)
                sb.AppendLine(" And tbl_Memberinfo.Sell_Mem_TF = 1 ");


            if (opt_C_02.Checked == true)
                sb.AppendLine(" And tbl_SalesDetail.BusCode <> '' ");
            ...
            sb.AppendLine(" And  ISNULL(tbl_SalesDetail.BusCode, '')  in ( Select Center_Code From ufn_User_In_Center ('" + cls_User.gid_CenterCode + "','" + cls_User.gid_CountryCode + "') )");
            ...
            Tsql = Tsql + Environment.NewLine  + " Order by tbl_Memberinfo.Mbid, tbl_Memberinfo.Mbid2 ASC ";
        }
        ...
        private void Base_Grid_Set()
        {
            ...
            {
                nP1 += Convert.ToInt32(ds.Tables[0].Rows[fi_cnt]["TotalPrice"]);
                Set_gr_dic(ref ds, ref gr_dic_text, fi_cnt);  //데이타를 배열에 넣는다.

            }

            txt_SumCnt.Text = string.Format(cls_app_static_var.str_Currency_Type, ReCnt);
            txt_P_1.Text = string.Format(cls_app_static_var.str_Currency_Type, nP1);
        ...
        private void dGridView_Base_Header_Reset()
        {
            ...
            string[] g_HeaderText = {"선택" ,"회원_번호"  , "성명"   , "주민번호"  , "현직급"    
                                , "주문_번호"     , "판매센타"   , "구매일자"     , "구매종류"   , "구분"   
                                , "", "판매금액"   , "결제액"  , ""   , ""   
                                ,"", ""     , ""    , "" , ""     
                                , "구분", "활동_여부", ""  , "탈퇴일"  , ""  
                                ,"기록자", "기록일", ""
        ...
        private Boolean Chang_CenterCode()
        {
            ...
            int Err_Cnt = 0, Suc_Cnt = 0, Same_Cnt = 0, Send_Cnt = 0, Stop_SW = 0;
            ...
            try
            {
                ...
                for (int i = 0; i < dGridView_Base.Rows.Count; i++)
                {
                    if (dGridView_Base.Rows[i].Cells[0].Value.ToString() == "V")
                    {
                        ...
                        Cancel_InsurancerNumber(OrderNumber, ref Err_Cnt, ref Suc_Cnt, ref Same_Cnt, ref Send_Cnt, ref Stop_SW);

                        if (Stop_SW == 1)
                            goto _insur_Send_Stop;
                    }
            ...
            {
                if (Err_Cnt > 0 || Suc_Cnt > 0 || Send_Cnt > 0 || Same_Cnt > 0)
                {
                    MessageBox.Show("공제번호 취소 요청 : " + Send_Cnt
                         + "\n" + "공제번호 취소 성공 : " + Suc_Cnt
                        + "\n" + "공제번호 취소 실패 : " + Err_Cnt
                        + "\n" + "공제번호 이미취소 : " + Same_Cnt
                       + "\n" + "입니다.");
        ...
        _insur_Send_Stop:
            MessageBox.Show("확인후 추후에 재전송 해주십시요.");
            return false;
        ...
        private void Cancel_InsurancerNumber(string T_ord_N, ref int Err_Cnt, ref int Suc_Cnt, ref int Same_Cnt, ref int Send_Cnt, ref int Stop_SW)
        {
            ...
            string Req = csg.Dir_Connect_Send_Cancel(T_ord_N);

            if (Req != "" && Req.Length >= 4)
                CC_Code = Req.Substring(0, 4);
