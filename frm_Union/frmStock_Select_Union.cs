...
{
    public partial class frmStock_Select_Union : clsForm_Extends
    {
        ...
        private void Make_Base_Query(ref string Tsql)
        {
            ...
            //                     ,  "회원_번호"   , "성명"  , "주민번호"     , "상품코드"    , "상품명" 
            //                     , "주문_수량"    , "출고_수량" , "출고지" , "주문_종류"   , "구분"   
            //                     , "배송구분" , "등록_센타명"    , "주문_센타명"      , ""    , ""                              
            //                      , "" , ""    , ""      , ""  , ""
            //                        };
            cls_form_Meth cm = new cls_form_Meth();
           ...
           Tsql = Tsql + ",tbl_Memberinfo.Cpno ";
            ...
            Tsql = Tsql + " From tbl_StockOutput ";
        ...
        private void Make_Base_Query_(ref string Tsql)
        {
            ...
            string strSql = " Where Out_FL ='001' And tbl_StockOutput.ItemCount >0 And  tbl_SalesDetail.M_name is not null  And tbl_SalesDetail.ReturnTF <> 5   ";
            strSql = strSql + " And  ( ";
            strSql = strSql + " (tbl_SalesDetail.ReturnTF =1 And tbl_SalesDetail.InsuranceNumber <> '') ";
            strSql = strSql + " OR  (tbl_SalesDetail.ReturnTF >= 3   ";
            ...
            if (txtCenter2_Code.Text.Trim() != "")
                strSql = strSql + " And tbl_StockOutput.Out_C_Code = '" + txtCenter2_Code.Text.Trim() + "'";
        ...
        private Boolean Chang_CenterCode()
        {
            ...
            //{
                ...
                for (int i = 0; i < dGridView_Base.Rows.Count; i++)
                {
                    ...
                    {
                        OrderNumber = dGridView_Base.Rows[i].Cells[3].Value.ToString();
                        ...
                        StrSql = "EXEC p_mlmunion_Out '" + cls_app_static_var.T_Company_Code + "','" + OrderNumber + "'," + SalesItemIndex + ",1";

                        Temp_Connect.Update_Data(StrSql,"","",1);

                        StrSql = "Insert into T_REALMLM_Stock_Send_TF (Out_Index , OrderNumber ,Salesitemindex,Send_ID,Send_Time ) Values (";
                        StrSql = StrSql + "'" + Out_Index + "','" + OrderNumber + "'," + SalesItemIndex + " ,'" + cls_User.gid + "',Convert(Varchar(25),GetDate(),21) ";
                        StrSql = StrSql + ")";

                        Temp_Connect.Insert_Data(StrSql, "T_REALMLM_Stock_Send_TF", this.Name.ToString(), this.Text);
        ...
        private void dGridView_Base_CellClick_1(object sender, DataGridViewCellEventArgs e)
        {
            DataGridView dg = (DataGridView)sender;
            if (e.RowIndex < 0) return;
