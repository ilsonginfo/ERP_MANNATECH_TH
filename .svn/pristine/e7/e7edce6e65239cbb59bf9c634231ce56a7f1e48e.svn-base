...
{
    public partial class frmClose_2_Select_01 : Form
    {
        ...
        private void frmBase_Resize(object sender, EventArgs e)
        {
            ...
            cfm.button_flat_change(button_Close_Re);
        ...
        private void frmBase_From_Load(object sender, EventArgs e)
        {
            ...
            txt_ETC13.BackColor = cls_app_static_var.txt_Enable_Color;
            txt_ETC14.BackColor = cls_app_static_var.txt_Enable_Color;
        ...
        private void Make_Base_Query(ref string Tsql)
        {
            ...
            //                   , "기간판매PV" , "기간반품액"     , "기간반품PV"  , "후원보너스"  ,"추천매칭"      
            //                   , "추천보너스"  , "후원보너스-대"  , "추천매칭-후원", ""   , ""   
            //                    , ""  , ""   , "" , ""     , ""     
            //                     , "후원공제"   , "반품공제액"  , "기타보너스" , "기간수당합계"  , "기간소득세"  
            //                    , "기간주민세"  ,"기간실지급액"        , "기간지급률" , "", "" 
            ...
            Tsql = Tsql + " , Allowance3 , Allowance30, Allowance5 , Allowance6 , Allowance7 ";

            //Tsql = Tsql + " , 0 ,0 ,  Isnull(Allowance27,0)  Allowance27 , Allowance28 ";

            Tsql = Tsql + " , Allowance8,   Allowance9,Allowance13,Isnull(Allowance28,0)  Allowance28 ,Isnull(Allowance29,0)  Allowance29, SumAllowance , SumInComeTax  ";

            Tsql = Tsql + " , SumResidentTax , SumTruePayment , SumAllowanceRate  ";
            Tsql = Tsql + " , SumAllowance_2 , SumAllowanceRate_2 ";


            Tsql = Tsql + " ,(Select Count(Mbid) From tbl_ClosePay_02_Mod (nolock) Where tbl_ClosePay_02_Mod.toEndDate = tbl_CloseTotal_02.ToEndDate And DayPV01 > 0  ) ";
            ...
            Tsql = Tsql + " LEFT JOIN tbl_WeekCount (nolock) ON tbl_WeekCount.ENDDATE = tbl_CloseTotal_02.ToEndDate  ";       
        ...
        private void dGridView_Base_Header_Reset()
        {
            ...
            string[] g_HeaderText = {"웹적용여부","_마감주차","마감_시작일"  ,"마감_종료일"  ,"지급_일자"  
                               ...
                               , "기간반품CV"    , "_PC_기간판매CV"  , "_PC_기간반품CV", "후원보너스"  ,"추천매칭보너스"    
                               , "추천보너스"  , "기타보너스"  , "_올스타팩보너스_소급"   , "_직급달성보너스"  , "_추천보너스"   
                                 , "_리더십보너스"     , "_PB"  ,"_37프로모션" ,"기타공제" , "반품공제액"
        ...
        private void put_Sum_Dataview(DataSet ds, int ReCnt)
        {
            ...
            for (int fi_cnt = 0; fi_cnt <= ReCnt - 1; fi_cnt++)
            {
                ...
                Sum_dic["추천매칭보너스"] = Sum_dic["추천매칭보너스"] + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt]["Allowance2"].ToString());
                Sum_dic["추천보너스"] = Sum_dic["추천보너스"] + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt]["Allowance3"].ToString());
                //Sum_dic["올스타팩보너스"] = Sum_dic["올스타팩보너스"] + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt]["Allowance4"].ToString());
                //Sum_dic["올스타팩보너스_소급"] = Sum_dic["올스타팩보너스_소급"] + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt]["Allowance5"].ToString());

                //Sum_dic["직급달성보너스"] = Sum_dic["직급달성보너스"] + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt]["Allowance6"].ToString());
                ...
                //Sum_dic["PB"] = Sum_dic["PB"] + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt]["Allowance9"].ToString());
                ...
                //Sum_dic["매칭공제"] = Sum_dic["매칭공제"] + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt]["Allowance26"].ToString());
                //Sum_dic["추천보너스공제"] = Sum_dic["추천보너스공제"] + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt]["Allowance27"].ToString());
                //Sum_dic["Cap공제"] = Sum_dic["Cap공제"] + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt]["Allowance28"].ToString());
        ...
        private void Allowance_Detail(string ToEndDate)
        {
            ...
            for (int f_cnt = 1; f_cnt <= 7; f_cnt++)
            {
                ...
                //{
                //    Pay_c = "37프로모션";
                //    fild_name = " Allowance13 ";
                //}

                //if (f_cnt == 10)
                //{
                //    Pay_c = "PB";
                //    fild_name = " Allowance12 ";
                //}
                //if (f_cnt == 11)
                //{
                //    Pay_c = "기타";
                ...
                else
                {
                    ...
                    cls_Grid_Base cgb_P1 = new cls_Grid_Base();
                    //if (fild_name == " Allowance4 " || fild_name == " Allowance5 ")
                    //{
                    //    dGridView_Base_Header_GGrade_Reset(t_DGV, cgb_P1); //디비그리드 헤더와 기본 셋팅을 한다.
                    //}
                    //else
                    //{
        ...
        private void Real_Allowance_Detail(string ToEndDate, string fild_name,  cls_Grid_Base cgb_P)
        {
            ...
            if (cls_app_static_var.Member_Number_1 > 0)
                StrSql = StrSql + " CL_02.Mbid + '-' + Convert(Varchar,CL_02.Mbid2) ";
            else
                StrSql = StrSql + " CL_02.Mbid2 ";

            StrSql = StrSql + ",CL_02.M_Name ,  " + fild_name + "   ";

            //if (fild_name == " Allowance4 " || fild_name == " Allowance5 ")
            //{
            //    StrSql = StrSql + ",View_Grade_Pay.Names , ''";
            //}
            //else
                StrSql = StrSql + ", '', '' ";

            StrSql = StrSql + " From  tbl_ClosePay_02_Mod (nolock)  CL_02 ";
            //if (fild_name ==" Allowance4 " || fild_name == " Allowance5 ")
            //{
            //    StrSql = StrSql + " LEFT JOin View_Grade_Pay ON View_Grade_Pay.Mbid = CL_02.Mbid And View_Grade_Pay.Mbid2  = CL_02.Mbid2 And  View_Grade_Pay. Give_ToEndDate = CL_02.ToEndDate  ";
            //}


            StrSql = StrSql + " Where CL_02.ToEndDate = '" + ToEndDate + "'";
            StrSql = StrSql + " And CL_02." + fild_name + " > 0 ";
            StrSql = StrSql + " Order By CL_02.Mbid, CL_02.Mbid2 ";
        ...
        private void dGridView_Base_Header_GGrade_Reset(DataGridView dGridView, cls_Grid_Base cgb_P)
        {
            ...
            string[] g_HeaderText = {"회원번호","성명", "금액", "직급"  ,""                             
                                };
            cgb_P.grid_col_header_text = g_HeaderText;

            int[] g_Width = { 100, 100 , 100, 120, 0
                            };
            cgb_P.grid_col_w = g_Width;
        ...
        private void button_Close_Re_Click(object sender, EventArgs e)
        {
            ...
            StrSql = StrSql + " From  tbl_CloseTotal_02 (nolock)  ";
            StrSql = StrSql + " Where ToendDate > '" + textToEndDate.Text.Trim() + "'";
            ...
            try
            {
                StrSql = "EXEC Usp_Close5_1_Re_100 '" + FromEndDate + "','" + T_ToEndDate + "'";
                Temp_Connect.Insert_Data(StrSql, Conn, tran);

                StrSql = "EXEC Usp_Close5_1_Re_200 '" + FromEndDate + "','" + T_ToEndDate + "'";
                Temp_Connect.Insert_Data(StrSql, Conn, tran);


                StrSql = "Insert Into tbl_Close_Log_FLAG Values ('W','" + FromEndDate + "','" + T_ToEndDate + "', 0 ,'" + cls_User.gid + "', Convert(Varchar(25),GetDate(),21) )   ";   
                Temp_Connect.Insert_Data(StrSql, Conn, tran);
