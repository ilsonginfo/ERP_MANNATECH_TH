...
namespace MLM_Program
{
    public partial class frmMileage_Select_Rest : Form
    {
        ...
        {
            Excel_Export_File_Name = this.Text; // "Mileage_Rest_Select";
        ...
        private Boolean Search_Check_TextBox_Error()
        {
            ...
            if (mtxtInDate2.Text.Replace("-", "").Trim() == "" && mtxtInDate3.Text.Replace("-", "").Trim() == "")
            {
                MessageBox.Show(cls_app_static_var.app_msg_rm.GetString("Msg_txt_Not_Data")
                        + "-" + cls_app_static_var.app_msg_rm.GetString("Msg_Not_Input_WDate")
                       + "\n" +
                       cls_app_static_var.app_msg_rm.GetString("Msg_Re_Action"));
                mtxtInDate2.Focus(); return false;
        ...
        private void Base_Sub_Grid_Set()
        {
            ...
            if (mtxtInDate3.Text.Replace("-","").Trim() == "")
                mtxtInDate3.Text = mtxtInDate2.Text.Replace("-", "").Trim();
            ...
            Tsql = Tsql + " From ufn_Mem_Mileage_Search_Mem_T02 ( '" + mtxtInDate2.Text.Replace("-", "").Trim() + "','" + mtxtInDate3.Text.Replace("-", "").Trim() + "')  ";

            string strSql = " Where   M_Name <> ''  "; 
            ...
            if (txtName2.Text.Trim() != "")
                strSql = strSql + " And M_Name Like '%" + txtName2.Text.Trim() + "%'";


            //회원센타로 검색
            if (txtCenter_Code.Text.Trim() != "")
                strSql = strSql + " And T_BusCode ='" + txtCenter_Code.Text.Trim() + "'";


            strSql = strSql + " And T_BusCode in ( Select Center_Code From ufn_User_In_Center ('" + cls_User.gid_CenterCode + "','" + cls_User.gid_CountryCode  + "') )";            
            ...
            for (int fi_cnt = 0; fi_cnt <= ReCnt - 1; fi_cnt++)
            {
                ...
                Sum_03 = Sum_03 + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt]["Ded_Value"].ToString());

                T_ver = ds.Tables[base_db_name].Rows[fi_cnt]["T_BusName"].ToString();

                if (T_ver != "")
                {
                    if (Center_Pr.ContainsKey(T_ver) == true)
                        Center_Pr[T_ver] = Center_Pr[T_ver] + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt]["Ded_Value"].ToString());  //금액                    
                    else
                        Center_Pr[T_ver] = double.Parse(ds.Tables[base_db_name].Rows[fi_cnt]["Ded_Value"].ToString());  //금액                    
                }
            }
        ...
        private void dGridView_Base_Sub_Header_Reset()
        {
            ...
            string[] g_HeaderText = {"회원번호",  "성명"  , "센타명"   , "이월액"  , "적립액"          
                                , "사용액"  , "잔액"   , ""    , ""   , ""                                
                                };
            cg_Sub.grid_col_header_text = g_HeaderText;

            int[] g_Width = { 90,  110, 90, 100, 110                             
                             , 100 ,100 , 0 ,  0 , 0
                            };
        ...
        private void Save_Nom_Line_Chart()
        {
            ...
            series_Item.Name = cm._chang_base_caption_search("잔여액");
