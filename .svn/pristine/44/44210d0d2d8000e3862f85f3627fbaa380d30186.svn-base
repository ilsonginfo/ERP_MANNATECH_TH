...
namespace MLM_Program
{
    public partial class frmStock_Select_Item__2 : Form
    {
        ...
        public frmStock_Select_Item__2()
        {
            InitializeComponent();
        }
        ...
        private void frm_Base_Activated(object sender, EventArgs e)
        {
           //19-03-11 깜빡임제거 this.Refresh();
            ...
            if (Form_Load_TF == 0)
            {
                ...
                dGridView_Base_Header_Reset__2(); //디비그리드 헤더와 기본 셋팅을 한다.
        ...
        {   
            ...
            string ItemCode = txt_ItemName_Code2.Text.Trim();                     

            Tsql = "EXEC Usp_Stock_Sell_ALL_01_1 '" + SDAte1 + "','" + SDate2 + "','" + ItemCode + "'"; 
            ...
            {
                //tab_Center_Item_Chart_Make( ds, ReCnt);
            }
        ...
        private void dGridView_Base_Header_Reset()
        {
            ...
            string[] g_HeaderText = {
                                 "상품코드"   , "상품명"  ,"총재고"    , "전일반입고"   , "전반품일반입고"   
                                 , "전반품불량입고"   ,  "전주문출고"   , "전주문출고직접"   , "전주문출고배송"  , "전주문출고센타"     
                                 , "전소진출고"    , "일반입고"    , "반품일반입고"   , "반품불량입고"  ,  "주문출고" 
                                 , "주문출고직접"  , "주문출고배송"   , "주문출고센타"  ,"주문출고배송정보X" , "소진출고" 
                                 , "총일반입고"  , "총반품일반입고"  ,"총반품불량입고"  , "총주문출고"  , "총주문출고직접"  
                                 , "총주문출고배송"  ,"총주문출고센타"  , "총주문출고배송정보X"  , "총소진출고"  , "총재고"  
                                 ,"" ,"", "", "" ,""
                                 ,"전교환입고","교환입고"
                                    };
            cgb.grid_col_header_text = g_HeaderText;

            int[] g_Width = { 90, 120, 100, 100, 100  
                             ,100, 100, 0, 0, 0  
                             ,100 , 100, 100, 100, 100
                             ,0 , 0, 0 , 0 , 100
                             ,100 , 100 , 100 , 100 , 0
            ...
            Color[] g_Color = { SystemColors.Window   , SystemColors.Window  , Color.PaleTurquoise ,  SystemColors.Window  , SystemColors.Window
                                ...
                                ,SystemColors.Window  , SystemColors.Window ,  SystemColors.Window ,  SystemColors.Window  ,SystemColors.Window                               
                                ,SystemColors.Window  , SystemColors.Window ,  SystemColors.Window ,  SystemColors.Window  ,SystemColors.Window
                                ,  SystemColors.Window  ,SystemColors.Window
                             };
        ...
        {
            Excel_Export_File_Name = this.Text; // "Stock_Item_Select";
        ...
        private void dGridView_Base_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (((sender as DataGridView).CurrentRow != null) && ((sender as DataGridView).CurrentRow.Cells[0].Value != null))
            {
                ...
                string Send_OrderNumber = (sender as DataGridView).CurrentRow.Cells[0].Value.ToString();
                ...
                this.Cursor = System.Windows.Forms.Cursors.WaitCursor;
                Base_Grid_Set__2(Send_OrderNumber);  //뿌려주는 곳
                this.Cursor = System.Windows.Forms.Cursors.Default;
            }  
        ...
        private void dGridView_Base_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            string ItemCode = (sender as DataGridView).CurrentRow.Cells[0].Value.ToString();
            string ItemName = (sender as DataGridView).CurrentRow.Cells[1].Value.ToString();

            double T_SumCnt1 = double.Parse((sender as DataGridView).CurrentRow.Cells[2].Value.ToString());
            T_SumCnt1 = T_SumCnt1 + double.Parse((sender as DataGridView).CurrentRow.Cells[3].Value.ToString());
            T_SumCnt1 = T_SumCnt1 + double.Parse((sender as DataGridView).CurrentRow.Cells[4].Value.ToString());
            //T_SumCnt1 = T_SumCnt1 + double.Parse((sender as DataGridView).CurrentRow.Cells[8].Value.ToString());

            T_SumCnt1 = T_SumCnt1 - double.Parse((sender as DataGridView).CurrentRow.Cells[5].Value.ToString());
            T_SumCnt1 = T_SumCnt1 - double.Parse((sender as DataGridView).CurrentRow.Cells[10].Value.ToString());
            //T_SumCnt1 = T_SumCnt1 - double.Parse((sender as DataGridView).CurrentRow.Cells[15].Value.ToString());


            Put_Stock_Detail(T_SumCnt1, ItemCode, ItemName);        
        }


        private void Put_Stock_Detail(double T_SumCnt, string ItemCode, string ItemName)
        {   
            string strSql = "";
            ...
            strSql = "EXEC Usp_Stock_Sell_ALL_02 '" + ItemCode + "','" + ItemName + "','" + SDAte1 + "','" + SDate2 + "'"; 
            ...
            for (int fi_cnt = 0; fi_cnt <= ReCnt - 1; fi_cnt++)
            {
                ...
                T6 = double.Parse(ds.Tables[base_db_name].Rows[fi_cnt][9].ToString());
        ...
        private void dGridView_Sell_Item_Header_Reset()
        {
            ...
            string[] g_HeaderText = {"일자"  , "일반입고"   , "반품일반입고"  , "반품불량입고"   , "주문출고"        
                                , "주문출고직접"    , "주문출고배송"  , "주문출고센타" , "주문출고배송정보X"  , "소진출고"   
                                , "재고량"  , "" , "" , "" , ""
                                };
        ...
        private void tab_Center_Item_Chart_Make(DataSet ds, int ReCnt)
        {
            //cls_form_Meth cm = new cls_form_Meth();
            //string itemName = ""; int itemCnt = 0;            
                       
            //chart_Item.Series.Clear();
            //tabControl1.TabPages[0].Text = cm._chang_base_caption_search("상품별");
            ...
            //for (int fi_cnt = 0; fi_cnt <= ReCnt - 1; fi_cnt++)
            //{                
            //    itemName = ds.Tables[base_db_name].Rows[fi_cnt][1].ToString();
        ...
        private void Base_Grid_Set__2(string ItemCode)
        {
            string Tsql = "";
            //Make_Base_Query(ref Tsql);
            //Make_Base_Query_(ref Tsql);
            ...
            string CenUserName = cm._chang_base_caption_search("센타");


            Tsql = "EXEC Usp_Stock_Sell_ALL_Center_01 '" + SDAte1 + "','" + SDate2 + "','" + ItemCode + "','" + CenterCode + "','C','" + CenUserName + "'";
            ...
            for (int fi_cnt = 0; fi_cnt <= ReCnt - 1; fi_cnt++)
            {
                Set_gr_dic__2(ref ds, ref gr_dic_text, fi_cnt);  //데이타를 배열에 넣는다.
        ...
        private void dGridView_Base_Header_Reset__2()
        {

            cgb_Item.grid_col_Count = 40;
            cgb_Item.basegrid = dGridView_Base__2;
            cgb_Item.grid_select_mod = DataGridViewSelectionMode.FullRowSelect;
            cgb_Item.grid_Frozen_End_Count = 5;
            cgb_Item.basegrid.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.AllCells;

            string[] g_HeaderText = { "센타코드"   , "센타명" , "_상품코드"   , "_상품명"  , "총재고"      
                                , "전일반입고"   , "전반품일반입고"    , "전반품불량입고"   , "전이동입고"    , "전주문출고"
                                , "_전주문출고직접"   , "_전주문출고배송"  , "_전주문출고센타"   , "전주문출고배송정보X"   ,"전소진출고"
                                , "전이동출고"     , "일반입고"    , "반품일반입고" , "반품불량입고"     , "이동입고"
                                , "주문출고", "주문출고직접"  , "주문출고배송"  , "주문출고센타"  ,"주문출고배송정보X"
                                , "소진출고", "이동출고"  , "총일반입고"  , "총반품일반입고"  ,"총반품불량입고"
                                 , "총이동입고", "총주문출고"  , "_총주문출고직접"  , "_총주문출고배송"  ,"_총주문출고센타"
                                , "_총주문출고배송정보X", "총소진출고"  , "총이동출고"  , "총재고"  ,""
                                    };
            cgb_Item.grid_col_header_text = g_HeaderText;

            int[] g_Width = {  90, 120, 0, 0  ,90
                             ,100, 100, 100, 100, 100  
                             ,0 , 0, 0, 0, 100
                             ,100 , 100, 100 , 100 , 100
                             ,100 , 0 , 0 , 0 , 0
            ...
            Color[] g_Color = { SystemColors.Window   , SystemColors.Window  ,  SystemColors.Window ,  SystemColors.Window  ,Color.PaleTurquoise
                                ,SystemColors.Window  , SystemColors.Window ,  SystemColors.Window ,  SystemColors.Window  ,SystemColors.Window
                                ,SystemColors.Window  , SystemColors.Window ,  SystemColors.Window ,  SystemColors.Window  ,SystemColors.Window
                                ,SystemColors.Window  , Color.PaleTurquoise ,  Color.PaleTurquoise ,  Color.PaleTurquoise  ,Color.PaleTurquoise
                                ,Color.PaleTurquoise ,  Color.PaleTurquoise ,  Color.PaleTurquoise ,  Color.PaleTurquoise  ,Color.PaleTurquoise
                                ,Color.PaleTurquoise ,  Color.PaleTurquoise ,  SystemColors.Window ,  SystemColors.Window  ,SystemColors.Window
                                ,SystemColors.Window  , SystemColors.Window ,  SystemColors.Window ,  SystemColors.Window  ,SystemColors.Window
                                ,SystemColors.Window  , SystemColors.Window ,  SystemColors.Window ,  SystemColors.Window  ,SystemColors.Window
                             };
        ...
        private void Set_gr_dic__2(ref DataSet ds, ref Dictionary<int, object[]> gr_dic_text, int fi_cnt)
        {
            object[] row0 = { ds.Tables[base_db_name].Rows[fi_cnt][0]
                                ,ds.Tables[base_db_name].Rows[fi_cnt][1]  
                                ,ds.Tables[base_db_name].Rows[fi_cnt][2]
