...
namespace MLM_Program
{
    public partial class frmSell_Select_Union : Form
    {
        ...
        private void frmBase_From_Load(object sender, EventArgs e)
        {
            ...
            txt_P_1_2.BackColor = cls_app_static_var.txt_Enable_Color;
            txt_P_2_2.BackColor = cls_app_static_var.txt_Enable_Color;
            txt_P_3_2.BackColor = cls_app_static_var.txt_Enable_Color;
            txt_P_4_2.BackColor = cls_app_static_var.txt_Enable_Color;
        }
        ...
        private void Make_Base_Query(ref string Tsql)
        {
            ...
            Tsql = Tsql + ", Case When tbl_Memberinfo.Cpno <> '' then tbl_Memberinfo.Cpno  ELSE Right(BirthDay + BirthDay_M + BirthDay_D , 6) + '0000000'  End Cpno ";
            ...
            Tsql = Tsql + " , Case When  tbl_SalesDetail.union_Seq > 0 And InsuranceNumber <> '' Then InsuranceNumber ";
        ...
        private void Make_Base_Query_(ref string Tsql)
        {
            ...
            strSql = strSql + "    (tbl_SalesDetail.Ga_Order = 0 And tbl_SalesDetail.ReturnTF = 1 And (insurancenumber ='' OR union_Seq = 0 ) ) ";  //공제번호발행이 안된내역
            //strSql = strSql + " OR (tbl_SalesDetail.Ga_Order = 0 And tbl_SalesDetail.ReturnTF = 1 And tbl_SalesDetail.union_Seq > 0  And  TotalPrice = TotalInputPrice And T_REALMLM.ERRCODE <> '0000'  ) ";  //이미신고된 내역인데 
            

            strSql = strSql + " OR (tbl_SalesDetail.ReturnTF = 2 And tbl_SalesDetail.union_Seq = 0 " ;
            strSql = strSql + " And Re_BaseOrderNumber in ";
            strSql = strSql + " (Select OrderNumber From tbl_SalesDetail (nolock)  Where insurancenumber <> '') " ; //정상내역이 조합에 신고된거만.. 조회 되도록 한다.            
            strSql = strSql + " ) ";  //반품이나 교환 부분반품 중에서 미신고된 내역.


            strSql = strSql + " OR (tbl_SalesDetail.ReturnTF = 3 And tbl_SalesDetail.union_Seq = 0   "; 
            strSql = strSql + " And Re_BaseOrderNumber in ";
            strSql = strSql + " (Select OrderNumber From tbl_SalesDetail (nolock)   Where insurancenumber <> '' "; //정상내역이 조합에 신고된거만.. 조회 되도록 한다.
            strSql = strSql + " ) ";
            strSql = strSql + " ) ";   //교환인 경우에 추가 결제된 내역이 잇다.
            

            strSql = strSql + " OR (tbl_SalesDetail.ReturnTF = 4 And tbl_SalesDetail.union_Seq = 0   ) ";
            strSql = strSql + " And Re_BaseOrderNumber in ";
            strSql = strSql + " (Select OrderNumber From tbl_SalesDetail (nolock)   Where insurancenumber <> ''"; //정상내역이 조합에 신고된거만.. 조회 되도록 한다.
            strSql = strSql + " ) ";
            strSql = strSql + " ) ";         //부분 반품인 경우에 추가 결제된 내역이 잇다.

            strSql = strSql + "  And tbl_SalesDetail.SellCode <> '' ";
            ////20190430 구현호 1000001,1000002,1000003 제거
            //20190731 지성경 다 가게끔처리원하심
            //strSql = strSql + "  And tbl_SalesDetail.mbid2 <> '1000001' ";
            //strSql = strSql + "  And tbl_SalesDetail.mbid2 <> '1000002' ";
            //strSql = strSql + "  And tbl_SalesDetail.mbid2 <> '1000003' ";
            ...
            strSql = strSql + " And  ISNULL(tbl_SalesDetail.BusCode, '')  in ( Select Center_Code From ufn_User_In_Center ('" + cls_User.gid_CenterCode + "','" + cls_User.gid_CountryCode  + "') )";
        ...
        {   
            ...
            double Sum_1 = 0; double Sum_1_2 = 0;
            double Sum_2 = 0; double Sum_2_2 = 0;
            double Sum_3 = 0; double Sum_3_2 = 0;
            double Sum_4 = 0; double Sum_4_2 = 0;
            string ReturnTF = "";
            for (int fi_cnt = 0; fi_cnt <= ReCnt - 1; fi_cnt++)
            {
                Set_gr_dic(ref ds, ref gr_dic_text, fi_cnt);  //데이타를 배열에 넣는다.

                ReturnTF = ds.Tables[base_db_name].Rows[fi_cnt]["ReturnTFName"].ToString();

                if (ReturnTF == cm._chang_base_caption_search("정상"))
                {
                    Sum_1  ++;
                    Sum_1_2 = Sum_1_2 + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt]["TotalPrice"].ToString());
                }
                if (ReturnTF == cm._chang_base_caption_search("반품"))
                {
                    Sum_2  ++;
                    Sum_2_2 = Sum_2_2 + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt]["TotalPrice"].ToString());
                }

                if (ReturnTF == cm._chang_base_caption_search("교환"))
                 {
                    Sum_3  ++;
                    Sum_3_2 = Sum_3_2 + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt]["TotalPrice"].ToString());
                }   

                if (ReturnTF == cm._chang_base_caption_search("부분반품"))
                {
                    Sum_4++;
                    Sum_4_2 = Sum_4_2 + double.Parse(ds.Tables[base_db_name].Rows[fi_cnt]["TotalPrice"].ToString());
                }                                           
            }
            ...
            {
                txt_P_1.Text = Sum_1.ToString () + " / " + string.Format(cls_app_static_var.str_Currency_Type, Sum_1_2);
                txt_P_2.Text = Sum_2.ToString() + " / " + string.Format(cls_app_static_var.str_Currency_Type, Sum_2_2);
                txt_P_3.Text = Sum_3.ToString() + " / " + string.Format(cls_app_static_var.str_Currency_Type, Sum_3_2);
                txt_P_4.Text = Sum_4.ToString() + " / " + string.Format(cls_app_static_var.str_Currency_Type, Sum_4_2);                
            }
        ...
        private void Set_gr_dic(ref DataSet ds, ref Dictionary<int, object[]> gr_dic_text, int fi_cnt)
        {
            object[] row0 = { ds.Tables[base_db_name].Rows[fi_cnt][0]
                                ...
                                ,encrypter.Decrypt ( ds.Tables[base_db_name].Rows[fi_cnt][3].ToString (),"Cpno_U")
                                ...
                                ,encrypter.Decrypt (ds.Tables[base_db_name].Rows[fi_cnt][13].ToString ())
                                ,encrypter.Decrypt (ds.Tables[base_db_name].Rows[fi_cnt][14].ToString ())

                                ,encrypter.Decrypt (ds.Tables[base_db_name].Rows[fi_cnt][15].ToString ())
                                ,encrypter.Decrypt (ds.Tables[base_db_name].Rows[fi_cnt][16].ToString ())
        ...
        private void Base_Select_Button_Click(object sender, EventArgs e)
        {
            ...
            if (bt.Name == "butt_Check_01")
            {
                ...
                for (int i = 0; i < dGridView_Base.RowCount; i++)
                {
                    ...
                    {
                        Sum_1++;
                        Sum_1_2 = Sum_1_2 + double.Parse(dGridView_Base.Rows[i].Cells[11].Value.ToString());
                    }
                    if (ReturnTF == cm._chang_base_caption_search("반품"))
                    {
                        Sum_2++;
                        Sum_2_2 = Sum_2_2 + double.Parse(dGridView_Base.Rows[i].Cells[11].Value.ToString());
                    }

                    if (ReturnTF == cm._chang_base_caption_search("교환"))
                    {
                        Sum_3++;
                        Sum_3_2 = Sum_3_2 + double.Parse(dGridView_Base.Rows[i].Cells[11].Value.ToString());
                    }

                    if (ReturnTF == cm._chang_base_caption_search("부분반품"))
                    {
                        Sum_4++;
                        Sum_4_2 = Sum_4_2 + double.Parse(dGridView_Base.Rows[i].Cells[11].Value.ToString());
                    }                                           

                    dGridView_Base.Rows[i].Cells[0].Value = "V";
                }

                txt_P_1_2.Text = Sum_1.ToString() + " / " + string.Format(cls_app_static_var.str_Currency_Type, Sum_1_2);
                txt_P_2_2.Text = Sum_2.ToString() + " / " + string.Format(cls_app_static_var.str_Currency_Type, Sum_2_2);
                txt_P_3_2.Text = Sum_3.ToString() + " / " + string.Format(cls_app_static_var.str_Currency_Type, Sum_3_2);
                txt_P_4_2.Text = Sum_4.ToString() + " / " + string.Format(cls_app_static_var.str_Currency_Type, Sum_4_2);                            
            }
        ...
        private Boolean Chang_CenterCode()
        {
            ...
            {
                //string T_Mbid = "";                 
                string OrderNumber = ""; string ReturnTF = ""; string Re_BaseOrderNumber = "";
                string hptel = "", hometel = "", add_r1 = "", add_r2 = "" , Cpno = ""; 
                ...
                for (int i = 0; i < dGridView_Base.Rows.Count; i++)
                {
                    if (dGridView_Base.Rows[i].Cells[0].Value.ToString() == "V")
                    {
                        ...
                        ReturnTF = dGridView_Base.Rows[i].Cells[9].Value.ToString();
                        Re_BaseOrderNumber = dGridView_Base.Rows[i].Cells[10].Value.ToString();

                        Cpno = dGridView_Base.Rows[i].Cells[3].Value.ToString();
                        hometel =  dGridView_Base.Rows[i].Cells[13].Value.ToString();
                        hptel = dGridView_Base.Rows[i].Cells[14].Value.ToString();
                        add_r1 =  dGridView_Base.Rows[i].Cells[15].Value.ToString();
                       
                        if (add_r1.Contains("'"))
                        {
                            add_r1 = add_r1.Replace("'", "''");
                        }
                        add_r2 = dGridView_Base.Rows[i].Cells[16].Value.ToString();
                  
                        if (add_r2.Contains("'"))
                        {
                            add_r2 = add_r2.Replace("'", "''");
                        }
                        if (ReturnTF == cm._chang_base_caption_search("정상"))
                            StrSql = "EXEC p_mlmunion_Order_2 '" + cls_app_static_var.T_Company_Code + "','" + OrderNumber + "','" + Cpno + "','" + hptel + "','" + hometel + "','" + add_r1 + "','" + add_r2 + "',1";

                        if (ReturnTF == cm._chang_base_caption_search("반품"))
                            StrSql = "EXEC p_mlmunion_Return '" + cls_app_static_var.T_Company_Code + "','" + OrderNumber + "',1";

                        if (ReturnTF == cm._chang_base_caption_search("교환"))
                            StrSql = "EXEC p_mlmunion_ExChange '" + cls_app_static_var.T_Company_Code + "','" + OrderNumber + "',4,1,0,0,0";

                        if (ReturnTF == cm._chang_base_caption_search("부분반품")) 
                            StrSql = "EXEC p_mlmunion_ExChange '" + cls_app_static_var.T_Company_Code + "','" + OrderNumber + "',3,1,0,0,0";
        ...
        private void Base_Button_Click(object sender, EventArgs e)
        {
            ...
            if (bt.Name == "butt_Clear")
            {
                ...
                txt_P_1_2.Text = ""; txt_P_2_2.Text = ""; txt_P_3_2.Text = ""; txt_P_4_2.Text = ""; 
        ...
        private void dGridView_Base_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            double Sum_1 = 0; double Sum_1_2 = 0; double pr_T = 0;
            string[] T_cut = null;

            ReturnTF = dg.CurrentRow.Cells[9].Value.ToString();
            pr_T = double.Parse(dg.CurrentRow.Cells[11].Value.ToString());
            ...
            if (ReturnTF == cm._chang_base_caption_search("정상"))
            {
                ...
                {
                    T_cut = txt_P_1_2.Text.Trim().Split('/');
                    Sum_1 = double.Parse(T_cut[0]);
                    Sum_1_2 = double.Parse(T_cut[1].Replace (",",""));
                }              
            }
            if (ReturnTF == cm._chang_base_caption_search("반품"))
            {
                ...
                {
                    T_cut = txt_P_2_2.Text.Trim().Split('/');
                    Sum_1 = double.Parse(T_cut[0]);
                    Sum_1_2 = double.Parse(T_cut[1].Replace(",", ""));
                }   
            }
            ...
            if (ReturnTF == cm._chang_base_caption_search("교환"))
            {
                ...
                {
                    T_cut = txt_P_3_2.Text.Trim().Split('/');
                    Sum_1 = double.Parse(T_cut[0]);
                    Sum_1_2 = double.Parse(T_cut[1].Replace(",", ""));
                }  
            }
            ...
            if (ReturnTF == cm._chang_base_caption_search("부분반품"))
            {
                ...
                {
                    T_cut = txt_P_4_2.Text.Trim().Split('/');
                    Sum_1 = double.Parse(T_cut[0]);
                    Sum_1_2 = double.Parse(T_cut[1].Replace(",", ""));
                }  
            }                                          
            ...
            if (ReturnTF == cm._chang_base_caption_search("반품"))
                txt_P_2_2.Text = Sum_1.ToString() + " / " + string.Format(cls_app_static_var.str_Currency_Type, Sum_1_2);

            if (ReturnTF == cm._chang_base_caption_search("교환"))
                txt_P_3_2.Text = Sum_1.ToString() + " / " + string.Format(cls_app_static_var.str_Currency_Type, Sum_1_2);

            if (ReturnTF == cm._chang_base_caption_search("부분반품"))
                txt_P_4_2.Text = Sum_1.ToString() + " / " + string.Format(cls_app_static_var.str_Currency_Type, Sum_1_2);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dGridView_Base.Rows.Count; i++)
            {
                if (dGridView_Base.Rows[i].Selected == true)
                {
                    ...
                    {
                        Check_Cnt(i, ref Sum_1, ref Sum_1_2, ref pr_T);
                        dGridView_Base.Rows[i].Cells[0].Value = "V";

                        Sum_1++;
                        Sum_1_2 = Sum_1_2 + pr_T;

                        string ReturnTF = dGridView_Base.Rows[i].Cells[9].Value.ToString();
        ...
        private void Check_Cnt(int i, ref double Sum_1, ref double Sum_1_2, ref double pr_T)
        {
            string ReturnTF = "";
            cls_form_Meth cm = new cls_form_Meth();
            ...
            ReturnTF = dGridView_Base.Rows[i].Cells[9].Value.ToString();
            pr_T = double.Parse(dGridView_Base.Rows[i].Cells[11].Value.ToString());
