...
{
    public partial class frmClose_Pay_Not_Cut : clsForm_Extends
    {
        ...
        private const string base_db_name = "tbl_Sham_PayNotCut";
        private int Data_Set_Form_TF;

        public frmClose_Pay_Not_Cut()
        {
            InitializeComponent();
        ...
        private void frmBase_From_Load(object sender, EventArgs e)
        {
            ...
            txt_Mile_W.BackColor = cls_app_static_var.txt_Enable_Color;
        ...
        private void Base_Sub_Grid_Set()
        {
            ...
            if (cls_app_static_var.Member_Number_1 > 0)
                Tsql = Tsql + " tbl_Sham_PayNotCut.mbid + '-' + Convert(Varchar,tbl_Sham_PayNotCut.mbid2) ";
            else
                Tsql = Tsql + " tbl_Sham_PayNotCut.mbid2 ";
            Tsql = Tsql + " ,tbl_Sham_PayNotCut.M_Name ";            
            Tsql = Tsql + " , tbl_Sham_PayNotCut.Apply_Date ";
            Tsql = Tsql + " ,Apply_Pv ";
            Tsql = Tsql + " , Case When Sellcode = 'W' then '주간' ELSE '월간' END  ";

            Tsql = Tsql + " ,tbl_Sham_PayNotCut.ETC " ;
            Tsql = Tsql + " ,tbl_Sham_PayNotCut.RecordID " ;
            Tsql = Tsql + " ,tbl_Sham_PayNotCut.RecordTime ";
            Tsql = Tsql + " ,tbl_Sham_PayNotCut.seq ";
            Tsql = Tsql + " ,tbl_Sham_PayNotCut.Sellcode";
            Tsql = Tsql + " From tbl_Sham_PayNotCut  (nolock) ";
            Tsql = Tsql + " Left Join tbl_Memberinfo   (nolock) On tbl_Memberinfo.Mbid  = tbl_Sham_PayNotCut.Mbid  And tbl_Memberinfo.Mbid2 = tbl_Sham_PayNotCut.Mbid2  ";
          ...
          //  Tsql = Tsql + " LEFT Join tbl_SellType   (nolock) ON tbl_Sham_PayNotCut.SellCode = tbl_SellType.SellCode ";
                                   
            string strSql = " Where   tbl_Sham_PayNotCut.M_Name <> '' ";
                ...
                )
            {
                ...
                {
                    if (Mbid != "")
                        strSql = strSql + " And tbl_Sham_PayNotCut.Mbid ='" + Mbid + "'";

                    if (Mbid2 >= 0)
                        strSql = strSql + " And tbl_Sham_PayNotCut.Mbid2 = " + Mbid2;
                }
            }
            ...
            if (txtName2.Text.Trim() != "")
                strSql = strSql + " And tbl_Sham_PayNotCut.M_Name Like '%" + txtName2.Text.Trim() + "%'";

            if (combo_Se_Code2.Text.Trim() != "")
                strSql = strSql + " And tbl_Sham_PayNotCut.SellCode  = '" + combo_Se_Code2.Text.Trim() + "'";
            ...
            if ((mtxtSellDate2.Text.Replace("-", "").Trim() != "") && (mtxtSellDate3.Text.Replace("-", "").Trim() == ""))
                strSql = strSql + " And tbl_Sham_PayNotCut.Apply_Date = '" + mtxtSellDate2.Text.Replace("-", "").Trim() + "'";

            //가입일자로 검색 -2
            if ((mtxtSellDate2.Text.Replace("-", "").Trim() != "") && (mtxtSellDate3.Text.Replace("-", "").Trim() != ""))
            {
                strSql = strSql + " And tbl_Sham_PayNotCut.Apply_Date >= '" + mtxtSellDate2.Text.Replace("-", "").Trim() + "'";
                strSql = strSql + " And tbl_Sham_PayNotCut.Apply_Date <= '" + mtxtSellDate3.Text.Replace("-", "").Trim() + "'";
            }


            //기록일자로 검색 -1
            if ((mtxtMakeDate1.Text.Replace("-", "").Trim() != "") && (mtxtMakeDate2.Text.Replace("-", "").Trim() == ""))
                strSql = strSql + " And Replace(Left( tbl_Sham_PayNotCut.recordtime ,10),'-','') = '" + mtxtMakeDate1.Text.Replace("-", "").Trim() + "'";

            //기록일자로 검색 -2
            if ((mtxtMakeDate1.Text.Replace("-", "").Trim() != "") && (mtxtMakeDate2.Text.Replace("-", "").Trim() != ""))
            {
                strSql = strSql + " And Replace(Left( tbl_Sham_PayNotCut.recordtime ,10),'-','') >= '" + mtxtMakeDate1.Text.Replace("-", "").Trim() + "'";
                strSql = strSql + " And Replace(Left( tbl_Sham_PayNotCut.recordtime ,10),'-','') <= '" + mtxtMakeDate2.Text.Replace("-", "").Trim() + "'";
            }


             if (txtR_Id_Code.Text.Trim() != "")
                strSql = strSql + " And tbl_Sham_PayNotCut.recordid = '" + txtR_Id_Code.Text.Trim() + "'";
            ...
            Tsql = Tsql + strSql;
            Tsql = Tsql + " Order by tbl_Sham_PayNotCut.Mbid ,  tbl_Sham_PayNotCut.Mbid2   ";
        ...
        private void dGridView_Base_Sub_Header_Reset()
        {
            ...
            string[] g_HeaderText = {"회원번호",  "성명"  , "적용_일자"   , "적용_금액"  , "구분"          
                                , "비고"  , "기록자"   , "기록일자"    , "_seq"   , "_SellCode"                                       
                                };
            cg_Sub.grid_col_header_text = g_HeaderText;

            int[] g_Width = { 90,  110, 100, 80, 90                             
                             , 110 ,70 , 130 ,  0 , 0                               
                            };
        ...
        private Boolean Check_TextBox_Error()
        {
            ...
            if (txt_Pv.Text == "0")
            {
                MessageBox.Show("차감할 미지급액을 입력해 주십시요."
                       + "\n" +
                       cls_app_static_var.app_msg_rm.GetString("Msg_Re_Action"));
            ...
            if (double.Parse(txt_Pv.Text.Replace(",", "").Trim() )>0 )
            {
                ...
                {
                    if (double.Parse(txt_Mile_W.Text.Replace(",", "").Trim()) < double.Parse(txt_Pv.Text.Replace(",", "").Trim()))
                    {
                        MessageBox.Show("저장할 차감할 미지급액이 잔여 미지급액 보다 클수 없습니다."
                               + "\n" +
                               cls_app_static_var.app_msg_rm.GetString("Msg_Re_Action"));
                        txt_Pv.Focus(); return false;
                    }
                ...
                {
                    if (double.Parse(txt_Mile.Text.Replace(",", "").Trim()) < double.Parse(txt_Pv.Text.Replace(",", "").Trim()))
                    {
                        MessageBox.Show("저장할 차감할 미지급액이 잔여 미지급액 보다 클수 없습니다."
                               + "\n" +
                               cls_app_static_var.app_msg_rm.GetString("Msg_Re_Action"));
        ...
        private void Save_Base_Data(ref int Save_Error_Check)
        {
            ...
            try
            {
                ...
                string SellCode = "W";//cls_User.gid  + ' ' + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                ...
                StrSql = "INSERT INTO tbl_Sham_PayNotCut ";
                StrSql = StrSql + " (";
                StrSql = StrSql + "  mbid , mbid2 , M_Name ";
                StrSql = StrSql + " , Apply_Date, Apply_PV , SellCode ";
          ...
          //Seq_Check:
          //      StrSql = "Select Seq  ";
          //      StrSql = StrSql + " From tbl_Sham_PayNotCut (nolock) ";
          ...
          //      if (ReCnt >0 )
          //          Seq = int.Parse(ds.Tables["tbl_Sales_OrdNumber"].Rows[0]["Seq"].ToString());

          //      if (Seq == 0)
          //          goto Seq_Check;


          //      Chang_RetunPay_Table(Temp_Connect, Conn, tran, Mbid, Mbid2, Seq.ToString ()); 
        ...
        private void Chang_RetunPay_Table(cls_Connect_DB Temp_Connect, SqlConnection Conn, SqlTransaction tran, string Mbid, int Mbid2, string SellCode)
        {
            ...
            StrSql = StrSql + " AP_ToEndDate = '" + SellCode + "'";
            StrSql = StrSql + " WHERE Mbid = '" + Mbid + "'";
            StrSql = StrSql + " And   Mbid2 = " + Mbid2;
            StrSql = StrSql + " And   SumAllAllowance > 0 ";
            StrSql = StrSql + " And   AP_ToEndDate = '' ";

            Temp_Connect.Insert_Data(StrSql, Conn, tran);
            
            Cut_Pay = double.Parse(txt_Mile.Text.Replace(",", "").Trim()) - double.Parse(txt_Pv.Text.Replace(",", "").Trim());

            if (Cut_Pay > 0)
            {
                StrSql = "insert into  tbl_Close_Not_Pay  values ";
                StrSql = StrSql + " ( ";
                StrSql = StrSql + " '" + SellCode + "'";
                StrSql = StrSql + " ,''";
                StrSql = StrSql + ", " + Mbid2;
                ...
                StrSql = StrSql + ", ''";
                StrSql = StrSql + ", " + Cut_Pay; 
                StrSql = StrSql + ", '미지급액차감_M'";
                StrSql = StrSql + " ) ";
            ...
            //StrSql = "Select SumAllAllowance Return_Pay2, Mbid C_Mbid,Mbid2 C_Mbid2, Seq T_index , '' Re_BaseOrderNumber ";
            ...
            //            if (RR_Cut_Pay != 0)
            //            {
            //                StrSql = "Insert into tbl_Sales_Put_Return_Pay (ToEndDate, C_mbid,C_mbid2 , C_M_Name , Return_Pay  ";
        ...
        {                      
            ...
            try
            {
                ...
                StrSql = "INSERT INTO tbl_Sham_PayNotCut_Mod ";
                StrSql = StrSql + " Select  * ";
                StrSql = StrSql + ",'" + cls_User.gid + "'";
                StrSql = StrSql + ",Convert(Varchar(25),GetDate(),21) ";
                StrSql = StrSql + " From tbl_Sham_PayNotCut ";
                StrSql = StrSql + " Where seq = " + txtKey.Text; 

                Temp_Connect.Insert_Data(StrSql, base_db_name, Conn, tran);


                StrSql = "UpDate  tbl_Sham_PayNotCut Set ";
        ...
        private void Delete_Base_Data(ref int Delete_Error_Check)
        {
            ...
            //Tsql = Tsql + " Where Insert_Seq = " + txtKey.Text;
            //Tsql = Tsql + " OR    Up_Seq = " + txtKey.Text;
            ...
            //    MessageBox.Show("삭제하실려는 내역은 이미 미지급내역에 반영된 상태 입니다. 처리가 불가능 합니다. "
            //           + "\n" +
            //           cls_app_static_var.app_msg_rm.GetString("Msg_Re_Action"));
            ...
            try
            {
                ...
                //StrSql = StrSql + " Select *  ";
                //StrSql = StrSql + ",3,'" + cls_User.gid + "', Convert(Varchar(25),GetDate(),21) From tbl_Close_Not_Pay (nolock) ";
                //StrSql = StrSql + " Where ToEndDate = " + txtKey.Text;
                //StrSql = StrSql + " OR  Insert_Seq = " + txtKey.Text;
                ...
                //StrSql = "Insert into  tbl_Close_Not_Pay_Mod ";
                //StrSql = StrSql + " Select *  ";
                //StrSql = StrSql + ",4,'" + cls_User.gid + "', Convert(Varchar(25),GetDate(),21) From tbl_Close_Not_Pay (nolock) ";
                //StrSql = StrSql + " Where (AP_ToEndDate = " + txtKey.Text;
                //StrSql = StrSql + " And AP_ToEndDate <> '') ";
                //StrSql = StrSql + " OR  Up_Seq = " + txtKey.Text;
                ...
                //StrSql = "Delete From tbl_Close_Not_Pay  ";
                //StrSql = StrSql + " Where ToEndDate  = '" + txtKey.Text + "'";
                //StrSql = StrSql + " OR  Insert_Seq = " + txtKey.Text;

                //Temp_Connect.Update_Data(StrSql, Conn, tran, this.Name, this.Text);

                //StrSql = "Update tbl_Close_Not_Pay SET AP_ToEndDate = '' , Up_Seq = 0  ";
                //StrSql = StrSql + " Where (AP_ToEndDate = " + txtKey.Text;
                //StrSql = StrSql + " And AP_ToEndDate <> '') ";
                //StrSql = StrSql + " OR  Up_Seq = " + txtKey.Text;
                ...
                StrSql = "Insert into  tbl_Sham_PayNotCut_Mod ";
                StrSql = StrSql + " Select *  ";
                StrSql = StrSql + ",'" + cls_User.gid + "', Convert(Varchar(25),GetDate(),21) From tbl_Sham_PayNotCut ";
                StrSql = StrSql + " Where seq = " + txtKey.Text;
                
                Temp_Connect.Insert_Data(StrSql, base_db_name, Conn, tran);


                StrSql = "Delete From tbl_Sham_PayNotCut  ";
                StrSql = StrSql + " Where seq = " + txtKey.Text;
        ...
        private void dGridView_Base_Sub_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (((sender as DataGridView).CurrentRow != null) && ((sender as DataGridView).CurrentRow.Cells[0].Value != null))
            {
                ...
                T_SellCode =  (sender as DataGridView).CurrentRow.Cells[9].Value.ToString();
        ...
        private void Set_Form_Date(string T_Mbid)
        {
            ...
            if (csb.Member_Nmumber_Split(T_Mbid, ref Mbid, ref Mbid2) == 1)
            {
                ...
                txt_Mile_W.Text = string.Format(cls_app_static_var.str_Currency_Type, T_p);

                T_p = Using_Cut_Return_Pay_Search_Month(Mbid, Mbid2);
        ...
        private double Using_Cut_Return_Pay_Search(string Mbid, int Mbid2)
        {
            ...
            //============================================================================================================
            Tsql = "SELECT Isnull( Sum(SumAllAllowance) ,0) A1    ";
            Tsql = Tsql + " From tbl_Close_Not_Pay  (nolock)  ";

            if (Mbid.Length == 0)
                Tsql = Tsql + " Where Convert(Varchar,Mbid2) = " + Mbid2.ToString();
            else
            {
                Tsql = Tsql + " Where Mbid = '" + Mbid + "' ";
                Tsql = Tsql + " And  Mbid2 = " + Mbid2.ToString();
            ...
            Tsql = Tsql + " And     (AP_ToEndDate = ''  ";
            Tsql = Tsql + "         OR    (  AP_ToEndDate <> ''  And  AP_ToEndDate > (Select Max(ToEndDate) From tbl_CloseTotal_02 (nolock)) )";
            Tsql = Tsql + "        ) "; 
            Tsql = Tsql + " And     ToEndDate <= (Select Max(ToEndDate) From tbl_CloseTotal_02 (nolock)) "; 
        ...
        private double Using_Cut_Return_Pay_Search_Month(string Mbid, int Mbid2)
        {
            double Sum_M = 0; double Sum_P = 0;
            cls_Connect_DB Temp_Connect = new cls_Connect_DB();
            string Tsql;
            ...
            Tsql = Tsql + " And     SumAllAllowance > 0  ";
            Tsql = Tsql + " And     (AP_ToEndDate = ''  ";
            Tsql = Tsql + "         OR    (  AP_ToEndDate <> ''  And  AP_ToEndDate > (Select Max(ToEndDate) From tbl_CloseTotal_04 (nolock)) )";
            Tsql = Tsql + "        ) ";
            Tsql = Tsql + " And     ToEndDate <= (Select Max(ToEndDate) From tbl_CloseTotal_04 (nolock)) ";
            Tsql = Tsql + " Group By Mbid ,Mbid2  ";
