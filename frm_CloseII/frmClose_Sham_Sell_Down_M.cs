...
namespace MLM_Program
{
    public partial class frmClose_Sham_Sell_Down_M : clsForm_Extends
    {
        ...
        private const string base_db_name = "tbl_Sham_Line_New_M";
        private int Data_Set_Form_TF;

        public frmClose_Sham_Sell_Down_M()
        {
            InitializeComponent();
        ...
        private void Base_Sub_Grid_Set()
        {
            ...
            if (cls_app_static_var.Member_Number_1 > 0)
                Tsql = Tsql + " tbl_Sham_Line_New_M.mbid + '-' + Convert(Varchar,tbl_Sham_Line_New_M.mbid2) ";
            else
                Tsql = Tsql + " tbl_Sham_Line_New_M.mbid2 ";
            Tsql = Tsql + " ,tbl_Sham_Line_New_M.M_Name ";
            Tsql = Tsql + " ,LEFT(Apply_Date,4) +'-' + LEFT(RIGHT(Apply_Date,4),2) + '-' + RIGHT(Apply_Date,2) ";
            Tsql = Tsql + " ,Apply_Pv ";
            Tsql = Tsql + " , Case When SortKind2 = 1  then '좌'  ELSE '우' END ";

            Tsql = Tsql + " ,tbl_Sham_Line_New_M.ETC ";
            Tsql = Tsql + " ,tbl_Sham_Line_New_M.RecordID ";
            Tsql = Tsql + " ,tbl_Sham_Line_New_M.RecordTime ";
            Tsql = Tsql + " ,tbl_Sham_Line_New_M.seq ";
            Tsql = Tsql + " ,tbl_Sham_Line_New_M.SortKind2";
            Tsql = Tsql + " From tbl_Sham_Line_New_M  (nolock) ";
            Tsql = Tsql + " Left Join tbl_Memberinfo   (nolock) On tbl_Memberinfo.Mbid  = tbl_Sham_Line_New_M.Mbid  And tbl_Memberinfo.Mbid2 = tbl_Sham_Line_New_M.Mbid2  ";
            ...
            //Tsql = Tsql + " LEFT Join tbl_SellType   (nolock) ON tbl_Sham_Line_New_M.SellCode = tbl_SellType.SellCode ";

            string strSql = " Where   tbl_Sham_Line_New_M.M_Name <> '' ";
                ...
                )
            {
                ...
                {
                    if (Mbid != "")
                        strSql = strSql + " And tbl_Sham_Line_New_M.Mbid ='" + Mbid + "'";

                    if (Mbid2 >= 0)
                        strSql = strSql + " And tbl_Sham_Line_New_M.Mbid2 = " + Mbid2;
                }
            }
            ...
            if (txtName2.Text.Trim() != "")
                strSql = strSql + " And tbl_Sham_Line_New_M.M_Name Like '%" + txtName2.Text.Trim() + "%'";

            if (combo_Se_Code2.Text.Trim() != "")
                strSql = strSql + " And tbl_Sham_Line_New_M.SellCode  = '" + combo_Se_Code2.Text.Trim() + "'";
            ...
            if ((mtxtSellDate2.Text.Replace("-", "").Trim() != "") && (mtxtSellDate3.Text.Replace("-", "").Trim() == ""))
                strSql = strSql + " And tbl_Sham_Line_New_M.Apply_Date = '" + mtxtSellDate2.Text.Replace("-", "").Trim() + "'";

            //가입일자로 검색 -2
            if ((mtxtSellDate2.Text.Replace("-", "").Trim() != "") && (mtxtSellDate3.Text.Replace("-", "").Trim() != ""))
            {
                strSql = strSql + " And tbl_Sham_Line_New_M.Apply_Date >= '" + mtxtSellDate2.Text.Replace("-", "").Trim() + "'";
                strSql = strSql + " And tbl_Sham_Line_New_M.Apply_Date <= '" + mtxtSellDate3.Text.Replace("-", "").Trim() + "'";
            }


            //기록일자로 검색 -1
            if ((mtxtMakeDate1.Text.Replace("-", "").Trim() != "") && (mtxtMakeDate2.Text.Replace("-", "").Trim() == ""))
                strSql = strSql + " And Replace(Left( tbl_Sham_Line_New_M.recordtime ,10),'-','') = '" + mtxtMakeDate1.Text.Replace("-", "").Trim() + "'";

            //기록일자로 검색 -2
            if ((mtxtMakeDate1.Text.Replace("-", "").Trim() != "") && (mtxtMakeDate2.Text.Replace("-", "").Trim() != ""))
            {
                strSql = strSql + " And Replace(Left( tbl_Sham_Line_New_M.recordtime ,10),'-','') >= '" + mtxtMakeDate1.Text.Replace("-", "").Trim() + "'";
                strSql = strSql + " And Replace(Left( tbl_Sham_Line_New_M.recordtime ,10),'-','') <= '" + mtxtMakeDate2.Text.Replace("-", "").Trim() + "'";
            }


            if (txtR_Id_Code.Text.Trim() != "")
                strSql = strSql + " And tbl_Sham_Line_New_M.recordid = '" + txtR_Id_Code.Text.Trim() + "'";
            ...
            Tsql = Tsql + strSql;
            Tsql = Tsql + " Order by tbl_Sham_Line_New_M.Mbid ,  tbl_Sham_Line_New_M.Mbid2   ";
        ...
        private void dGridView_Base_Sub_Header_Reset()
        {
            ...
            string[] g_HeaderText = {"회원번호",  "성명"  , "적용_일자"   , "적용_CV"  , "적용라인"
                                , "비고"  , "기록자"   , "기록일자"    , "_seq"   , "_SellCode"
                                };
            cg_Sub.grid_col_header_text = g_HeaderText;

            int[] g_Width = { 90,  110, 100, 90, 90
                             , 110 ,70 , 100 ,  0 , 0
                            };
        ...
        private Boolean Check_TextBox_Error()
        {
            ...
            if (txtKey.Text != "") //수정일 경우에는 수정 프로시져로 가고 이 프로시져를 빠져나가라
            {
                ...
                Tsql = Tsql + " And  LEFT(Apply_Date,6)  = '" + mtxtSellDate.Text.Replace("-", "").Trim().Substring (0,6) + "'";
                ...
                if (ReCnt > 0)  //실제로 존재하는 회원 번호 인가.
                {
                    ...
                    MessageBox.Show("동일한 회원이 동일 적용월로 해서 입력 하신 내역이 있습니다."
                           + "\n" +
                           cls_app_static_var.app_msg_rm.GetString("Msg_Re_Action"));
                    mtxtSellDate.Focus(); return false;
                }
        ...
        private void Save_Base_Data(ref int Save_Error_Check)
        {
            ...
            try
            {
                ...
                StrSql = "INSERT INTO tbl_Sham_Line_New_M ";
                StrSql = StrSql + " (";
        ...
        private void Save_Base_Data_UpDate(ref int Save_Error_Check)
        {
            ...
            try
            {
                ...
                StrSql = "INSERT INTO tbl_Sham_Line_New_M_Mod ";
                StrSql = StrSql + " Select  * ";
                StrSql = StrSql + ",'" + cls_User.gid + "'";
                StrSql = StrSql + ",Convert(Varchar(25),GetDate(),21) ";
                StrSql = StrSql + " From tbl_Sham_Line_New_M ";
                StrSql = StrSql + " Where seq = " + txtKey.Text;

                Temp_Connect.Insert_Data(StrSql, base_db_name, Conn, tran);


                StrSql = "UpDate  tbl_Sham_Line_New_M Set ";
        ...
        private void Delete_Base_Data(ref int Delete_Error_Check)
        {
            ...
            try
            {
                ...
                StrSql = "Insert into  tbl_Sham_Line_New_M_Mod ";
                StrSql = StrSql + " Select *  ";
                StrSql = StrSql + ",'" + cls_User.gid + "', Convert(Varchar(25),GetDate(),21) From tbl_Sham_Line_New_M ";
                StrSql = StrSql + " Where seq = " + txtKey.Text;

                Temp_Connect.Insert_Data(StrSql, base_db_name, Conn, tran);


                StrSql = "Delete From tbl_Sham_Line_New_M  ";
                StrSql = StrSql + " Where seq = " + txtKey.Text;
        ...
        private void dGridView_Base_Sub_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (((sender as DataGridView).CurrentRow != null) && ((sender as DataGridView).CurrentRow.Cells[0].Value != null))
            {
                ...
                cls_Search_DB csd = new cls_Search_DB();
                if (csd.Close_Check_SellDate("tbl_CloseTotal_02", mtxtSellDate.Text.Replace("-", "").Trim()) == false
                    || csd.Close_Check_SellDate("tbl_CloseTotal_04", mtxtSellDate.Text.Replace("-", "").Trim()) == false
                    )
                {
