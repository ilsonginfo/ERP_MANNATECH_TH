...
{
    public partial class frmClose_Sham_Grade : clsForm_Extends
    {
        ...
        private const string base_db_name = "tbl_Sham_Grade";
        private int Data_Set_Form_TF;

        public frmClose_Sham_Grade()
        {
            InitializeComponent();
        ...
        private void frmBase_From_Load(object sender, EventArgs e)
        {
            ...
            cpbf.Put_Close_Grade_ComboBox(combo_Pay2, combo_Pay_Code2);
            cpbf.Put_Close_Grade_ComboBox(combo_Pay, combo_Pay_Code);
        ...
        private void Base_Sub_Grid_Set()
        {
            ...
            if (cls_app_static_var.Member_Number_1 > 0)
                Tsql = Tsql + " tbl_Sham_Grade.mbid + '-' + Convert(Varchar,tbl_Sham_Grade.mbid2) ";
            else
                Tsql = Tsql + " tbl_Sham_Grade.mbid2 ";
            Tsql = Tsql + " ,tbl_Sham_Grade.M_Name ";            
            Tsql = Tsql + " ,LEFT(Apply_Date,4) +'-' + LEFT(RIGHT(Apply_Date,4),2) + '-' + RIGHT(Apply_Date,2) " ;
            Tsql = Tsql + " ,0 ";
            Tsql = Tsql + " , Isnull(tbl_Class.Grade_Name,'')  ";

            Tsql = Tsql + " ,tbl_Sham_Grade.ETC " ;
            Tsql = Tsql + " ,tbl_Sham_Grade.RecordID " ;
            Tsql = Tsql + " ,tbl_Sham_Grade.RecordTime ";
            Tsql = Tsql + " ,tbl_Sham_Grade.seq ";
            Tsql = Tsql + " ,tbl_Sham_Grade.Apply_Grade";
            Tsql = Tsql + " From tbl_Sham_Grade  (nolock) ";
            Tsql = Tsql + " Left Join tbl_Memberinfo   (nolock) On tbl_Memberinfo.Mbid  = tbl_Sham_Grade.Mbid  And tbl_Memberinfo.Mbid2 = tbl_Sham_Grade.Mbid2  ";
            ...
            Tsql = Tsql + " LEFT Join tbl_Class   (nolock) ON tbl_Class.Grade_Cnt = tbl_Sham_Grade.Apply_Grade ";
                                   
            string strSql = " Where   tbl_Sham_Grade.M_Name <> '' ";
                ...
                )
            {
                ...
                {
                    if (Mbid != "")
                        strSql = strSql + " And tbl_Sham_Grade.Mbid ='" + Mbid + "'";

                    if (Mbid2 >= 0)
                        strSql = strSql + " And tbl_Sham_Grade.Mbid2 = " + Mbid2;
                }
            }
            ...
            if (txtName2.Text.Trim() != "")
                strSql = strSql + " And tbl_Sham_Grade.M_Name Like '%" + txtName2.Text.Trim() + "%'";

            //회원명으로 검색
            if (combo_Pay_Code2.Text.Trim() != "")
                strSql = strSql + " And tbl_Sham_Grade.Apply_Grade = '" + combo_Pay_Code2.Text.Trim() + "'";


            //가입일자로 검색 -1
            if ((mtxtSellDate2.Text.Replace("-", "").Trim() != "") && (mtxtSellDate3.Text.Replace("-", "").Trim() == ""))
                strSql = strSql + " And tbl_Sham_Grade.Apply_Date = '" + mtxtSellDate2.Text.Replace("-", "").Trim() + "'";

            //가입일자로 검색 -2
            if ((mtxtSellDate2.Text.Replace("-", "").Trim() != "") && (mtxtSellDate3.Text.Replace("-", "").Trim() != ""))
            {
                strSql = strSql + " And tbl_Sham_Grade.Apply_Date >= '" + mtxtSellDate2.Text.Replace("-", "").Trim() + "'";
                strSql = strSql + " And tbl_Sham_Grade.Apply_Date <= '" + mtxtSellDate3.Text.Replace("-", "").Trim() + "'";
            }


            //기록일자로 검색 -1
            if ((mtxtMakeDate1.Text.Replace("-", "").Trim() != "") && (mtxtMakeDate2.Text.Replace("-", "").Trim() == ""))
                strSql = strSql + " And Replace(Left( tbl_Sham_Grade.recordtime ,10),'-','') = '" + mtxtMakeDate1.Text.Replace("-", "").Trim() + "'";

            //기록일자로 검색 -2
            if ((mtxtMakeDate1.Text.Replace("-", "").Trim() != "") && (mtxtMakeDate2.Text.Replace("-", "").Trim() != ""))
            {
                strSql = strSql + " And Replace(Left( tbl_Sham_Grade.recordtime ,10),'-','') >= '" + mtxtMakeDate1.Text.Replace("-", "").Trim() + "'";
                strSql = strSql + " And Replace(Left( tbl_Sham_Grade.recordtime ,10),'-','') <= '" + mtxtMakeDate2.Text.Replace("-", "").Trim() + "'";
            }

             if (txtR_Id_Code.Text.Trim() != "")
                strSql = strSql + " And tbl_Sham_Grade.recordid = '" + txtR_Id_Code.Text.Trim() + "'";
            ...
            Tsql = Tsql + strSql;
            Tsql = Tsql + " Order by tbl_Sham_Grade.Mbid ,  tbl_Sham_Grade.Mbid2   ";
        ...
        private void dGridView_Base_Sub_Header_Reset()
        {
            ...
            string[] g_HeaderText = {"회원번호",  "성명"  , "적용_일자"   , ""  , "인정_직급"          
        ...
        private void Save_Base_Data(ref int Save_Error_Check)
        {
            ...
            try
            {
               // double app_Pv =   double.Parse(txt_Pv.Text.Trim().Replace(",", ""));                
                ...
                StrSql = "INSERT INTO tbl_Sham_Grade ";
                StrSql = StrSql + " (";
        ...
        {                      
            ...
            try
            {
                ...
                StrSql = "INSERT INTO tbl_Sham_Grade_Mod ";
                StrSql = StrSql + " Select  * ";
                StrSql = StrSql + ",'" + cls_User.gid + "'";
                StrSql = StrSql + ",Convert(Varchar(25),GetDate(),21) ";
                StrSql = StrSql + " From tbl_Sham_Grade ";
        ...
        private void Delete_Base_Data(ref int Delete_Error_Check)
        {
            ...
            try
            {
                ...
                StrSql = "Insert into  tbl_Sham_Grade_Mod ";
                StrSql = StrSql + " Select *  ";
                StrSql = StrSql + ",'" + cls_User.gid + "', Convert(Varchar(25),GetDate(),21) From tbl_Sham_Grade ";
                StrSql = StrSql + " Where seq = " + txtKey.Text;
                
                Temp_Connect.Insert_Data(StrSql, base_db_name, Conn, tran);


                StrSql = "Delete From tbl_Sham_Grade  ";
                StrSql = StrSql + " Where seq = " + txtKey.Text;
