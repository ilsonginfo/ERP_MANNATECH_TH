...
{
    public partial class frmClose_Pay_Cut : clsForm_Extends
    {
        ...
        private void Base_Sub_Grid_Set()
        {
            ...
            Tsql = Tsql + " ,tbl_Sales_Put_Return_Pay.Return_Pay , '', '','',tbl_Sales_Put_Return_Pay.T_index ";
            Tsql = Tsql + " ,tbl_Sales_Put_Return_Pay.R_ETC ";
            Tsql = Tsql + " ,tbl_Sales_Put_Return_Pay.RecordID " ;
            Tsql = Tsql + " ,tbl_Sales_Put_Return_Pay.RecordTime ";

            Tsql = Tsql + " From tbl_Sales_Put_Return_Pay  (nolock) ";
            Tsql = Tsql + " LEFT JOIN tbl_Memberinfo (nolock) ON tbl_Memberinfo.Mbid = tbl_Sales_Put_Return_Pay.C_Mbid And tbl_Memberinfo.Mbid2 = tbl_Sales_Put_Return_Pay.C_Mbid2 ";

            string strSql = " Where   tbl_Sales_Put_Return_Pay.Cl_TF  = -1000  ";
            ...
            //    strSql = strSql + " And tbl_Sales_Put_Return_Pay.SortKind2 = '" + combo_Pay_Code2.Text.Trim() + "'";


            ////가입일자로 검색 -1
            //if ((mtxtSellDate2.Text.Replace("-", "").Trim() != "") && (mtxtSellDate3.Text.Replace("-", "").Trim() == ""))
            //    strSql = strSql + " And tbl_Sales_Put_Return_Pay.Apply_Date = '" + mtxtSellDate2.Text.Replace("-", "").Trim() + "'";

            ////가입일자로 검색 -2
            //if ((mtxtSellDate2.Text.Replace("-", "").Trim() != "") && (mtxtSellDate3.Text.Replace("-", "").Trim() != ""))
            //{
            //    strSql = strSql + " And tbl_Sales_Put_Return_Pay.Apply_Date >= '" + mtxtSellDate2.Text.Replace("-", "").Trim() + "'";
            //    strSql = strSql + " And tbl_Sales_Put_Return_Pay.Apply_Date <= '" + mtxtSellDate3.Text.Replace("-", "").Trim() + "'";
            //}


            //기록일자로 검색 -1
            if ((mtxtMakeDate1.Text.Replace("-", "").Trim() != "") && (mtxtMakeDate2.Text.Replace("-", "").Trim() == ""))
                strSql = strSql + " And Replace(Left( tbl_Sales_Put_Return_Pay.recordtime ,10),'-','') = '" + mtxtMakeDate1.Text.Replace("-", "").Trim() + "'";

            //기록일자로 검색 -2
            if ((mtxtMakeDate1.Text.Replace("-", "").Trim() != "") && (mtxtMakeDate2.Text.Replace("-", "").Trim() != ""))
            {
                strSql = strSql + " And Replace(Left( tbl_Sales_Put_Return_Pay.recordtime ,10),'-','') >= '" + mtxtMakeDate1.Text.Replace("-", "").Trim() + "'";
                strSql = strSql + " And Replace(Left( tbl_Sales_Put_Return_Pay.recordtime ,10),'-','') <= '" + mtxtMakeDate2.Text.Replace("-", "").Trim() + "'";
            }

             if (txtR_Id_Code.Text.Trim() != "")
                strSql = strSql + " And tbl_Sales_Put_Return_Pay.recordid = '" + txtR_Id_Code.Text.Trim() + "'";
        ...
        private Boolean Check_TextBox_Error()
        {
            ...
            if (txt_Pv.Text == "0")
            {
                MessageBox.Show("가감할 공제액을 입력해 주십시요."
                       + "\n" +
                       cls_app_static_var.app_msg_rm.GetString("Msg_Re_Action"));
        ...
        private void Save_Base_Data(ref int Save_Error_Check)
        {
            ...
            try
            {
                ...
                if (app_Pv > 0)
                {
                    ...
                    StrSql = StrSql + ",'" + Mbid + "'";
                    StrSql = StrSql + "," + Mbid2 + ",'" + txtName.Text + "',''," + app_Pv + "," + app_Pv + ",-1000 ";
                ...
                {
                    int Cut_Be_Cnt = 0;
                    Chang_RetunPay_Table(Temp_Connect, Conn, tran, Mbid, Mbid2, ref Cut_Be_Cnt);
                }


                //입력한 로그를 남겨 놓는다. 계산 관련 내역만 남기 때문에
                StrSql = "INSERT INTO tbl_Sham_ReturnPay_Cut_Log ";
                StrSql = StrSql + " (";
        ...
        private void Chang_RetunPay_Table(cls_Connect_DB Temp_Connect, SqlConnection Conn, SqlTransaction tran, string Mbid, int Mbid2, ref int Cut_Be_Cnt)
        {
