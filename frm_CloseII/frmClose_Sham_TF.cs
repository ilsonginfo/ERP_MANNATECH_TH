...
{
    public partial class frmClose_Sham_TF : clsForm_Extends
    {
        ...
        private const string base_db_name = "tbl_Sham_Sell_TF";
        private int Data_Set_Form_TF;

        public frmClose_Sham_TF()
        {
            InitializeComponent();
        ...
        void e_f_Send_MemNumber_Info(ref string searchMbid, ref int searchMbid2, ref string seachName, string Ui_TF)
        {
            seachName = "";
            Ui_TF = "";
        ...
        private void Base_Sub_Grid_Set()
        {
            ...
            if (cls_app_static_var.Member_Number_1 > 0)
                Tsql = Tsql + " tbl_Sham_Sell_TF.mbid + '-' + Convert(Varchar,tbl_Sham_Sell_TF.mbid2) ";
            else
                Tsql = Tsql + " tbl_Sham_Sell_TF.mbid2 ";
            Tsql = Tsql + " ,tbl_Sham_Sell_TF.M_Name ";            
            Tsql = Tsql + " ,LEFT(Apply_Date,4) +'-' + LEFT(RIGHT(Apply_Date,4),2) + '-' + RIGHT(Apply_Date,2) " ;
            Tsql = Tsql + " ,Apply_Pv ";
            Tsql = Tsql + " , Isnull(tbl_SellType.SellTypeName,'')  ";

            Tsql = Tsql + " ,tbl_Sham_Sell_TF.ETC " ;
            Tsql = Tsql + " ,tbl_Sham_Sell_TF.RecordID " ;
            Tsql = Tsql + " ,tbl_Sham_Sell_TF.RecordTime ";
            Tsql = Tsql + " ,tbl_Sham_Sell_TF.seq ";
            Tsql = Tsql + " ,tbl_Sham_Sell_TF.Sellcode";
            Tsql = Tsql + " From tbl_Sham_Sell_TF  (nolock) ";
            Tsql = Tsql + " Left Join tbl_Memberinfo   (nolock) On tbl_Memberinfo.Mbid  = tbl_Sham_Sell_TF.Mbid  And tbl_Memberinfo.Mbid2 = tbl_Sham_Sell_TF.Mbid2  ";
            ...
            Tsql = Tsql + " LEFT Join tbl_SellType   (nolock) ON tbl_Sham_Sell_TF.SellCode = tbl_SellType.SellCode ";
                                   
            string strSql = " Where   tbl_Sham_Sell_TF.M_Name <> '' ";
                ...
                )
            {
                ...
                {
                    if (Mbid != "")
                        strSql = strSql + " And tbl_Sham_Sell_TF.Mbid ='" + Mbid + "'";

                    if (Mbid2 >= 0)
                        strSql = strSql + " And tbl_Sham_Sell_TF.Mbid2 = " + Mbid2;
                }
            }
            ...
            if (txtName2.Text.Trim() != "")
                strSql = strSql + " And tbl_Sham_Sell_TF.M_Name Like '%" + txtName2.Text.Trim() + "%'";

            if (combo_Se_Code2.Text.Trim() != "")
                strSql = strSql + " And tbl_Sham_Sell_TF.SellCode  = '" + combo_Se_Code2.Text.Trim() + "'";
            ...
            if ((mtxtSellDate2.Text.Replace("-", "").Trim() != "") && (mtxtSellDate3.Text.Replace("-", "").Trim() == ""))
                strSql = strSql + " And tbl_Sham_Sell_TF.Apply_Date = '" + mtxtSellDate2.Text.Replace("-", "").Trim() + "'";

            //가입일자로 검색 -2
            if ((mtxtSellDate2.Text.Replace("-", "").Trim() != "") && (mtxtSellDate3.Text.Replace("-", "").Trim() != ""))
            {
                strSql = strSql + " And tbl_Sham_Sell_TF.Apply_Date >= '" + mtxtSellDate2.Text.Replace("-", "").Trim() + "'";
                strSql = strSql + " And tbl_Sham_Sell_TF.Apply_Date <= '" + mtxtSellDate3.Text.Replace("-", "").Trim() + "'";
            }


            //기록일자로 검색 -1
            if ((mtxtMakeDate1.Text.Replace("-", "").Trim() != "") && (mtxtMakeDate2.Text.Replace("-", "").Trim() == ""))
                strSql = strSql + " And Replace(Left( tbl_Sham_Sell_TF.recordtime ,10),'-','') = '" + mtxtMakeDate1.Text.Replace("-", "").Trim() + "'";

            //기록일자로 검색 -2
            if ((mtxtMakeDate1.Text.Replace("-", "").Trim() != "") && (mtxtMakeDate2.Text.Replace("-", "").Trim() != ""))
            {
                strSql = strSql + " And Replace(Left( tbl_Sham_Sell_TF.recordtime ,10),'-','') >= '" + mtxtMakeDate1.Text.Replace("-", "").Trim() + "'";
                strSql = strSql + " And Replace(Left( tbl_Sham_Sell_TF.recordtime ,10),'-','') <= '" + mtxtMakeDate2.Text.Replace("-", "").Trim() + "'";
            }


             if (txtR_Id_Code.Text.Trim() != "")
                strSql = strSql + " And tbl_Sham_Sell_TF.recordid = '" + txtR_Id_Code.Text.Trim() + "'";
            ...
            Tsql = Tsql + strSql;
            Tsql = Tsql + " Order by tbl_Sham_Sell_TF.Mbid ,  tbl_Sham_Sell_TF.Mbid2   ";
        ...
        private void dGridView_Base_Sub_Header_Reset()
        {
            ...
            string[] g_HeaderText = {"회원번호",  "성명"  , "적용_일자"   , "_적용_PV"  , "_구매_종류"          
        ...
        private void Save_Base_Data(ref int Save_Error_Check)
        {
            ...
            try
            {
                double app_Pv = 0; // double.Parse(txt_Pv.Text.Trim().Replace(",", ""));                
                               
                StrSql = "INSERT INTO tbl_Sham_Sell_TF ";
                StrSql = StrSql + " (";
                ...
                StrSql = StrSql + ",'" + txtSellCode_Code.Text + "'";
        ...
        {                      
            ...
            try
            {
                ...
                StrSql = "INSERT INTO tbl_Sham_Sell_TF_Mod ";
                StrSql = StrSql + " Select  * ";
                StrSql = StrSql + ",'" + cls_User.gid + "'";
                StrSql = StrSql + ",Convert(Varchar(25),GetDate(),21) ";
                StrSql = StrSql + " From tbl_Sham_Sell_TF ";
                StrSql = StrSql + " Where seq = " + txtKey.Text; 

                Temp_Connect.Insert_Data(StrSql, base_db_name, Conn, tran);


                StrSql = "UpDate  tbl_Sham_Sell_TF Set ";
        ...
        private void Delete_Base_Data(ref int Delete_Error_Check)
        {
            ...
            try
            {
                ...
                StrSql = "Insert into  tbl_Sham_Sell_TF_Mod ";
                StrSql = StrSql + " Select *  ";
                StrSql = StrSql + ",'" + cls_User.gid + "', Convert(Varchar(25),GetDate(),21) From tbl_Sham_Sell_TF ";
                StrSql = StrSql + " Where seq = " + txtKey.Text;
                
                Temp_Connect.Insert_Data(StrSql, base_db_name, Conn, tran);


                StrSql = "Delete From tbl_Sham_Sell_TF  ";
                StrSql = StrSql + " Where seq = " + txtKey.Text;
