...
{
    public partial class frmClose_Sham_Grade_P : clsForm_Extends
    {
        ...
        private const string base_db_name = "tbl_Sham_Grade_P";
        private int Data_Set_Form_TF;

        public frmClose_Sham_Grade_P()
        {
            InitializeComponent();
        ...
        private void frmBase_From_Load(object sender, EventArgs e)
        {
            ...
            cpbf.Put_Close_GradeP_ComboBox(combo_Pay2, combo_Pay_Code2);
            cpbf.Put_Close_GradeP_ComboBox(combo_Pay, combo_Pay_Code);
        ...
        {
            cls_form_Meth cm = new cls_form_Meth();
            Excel_Export_File_Name = this.Text; // cm._chang_base_caption_search ( "인정_직급") ;
        ...
        private void Base_Sub_Grid_Set()
        {
            ...
            if (cls_app_static_var.Member_Number_1 > 0)
                Tsql = Tsql + " tbl_Sham_Grade_P.mbid + '-' + Convert(Varchar,tbl_Sham_Grade_P.mbid2) ";
            else
                Tsql = Tsql + " tbl_Sham_Grade_P.mbid2 ";
            Tsql = Tsql + " ,tbl_Sham_Grade_P.M_Name ";            
            Tsql = Tsql + " ,LEFT(Apply_Date,4) +'-' + LEFT(RIGHT(Apply_Date,4),2) + '-' + RIGHT(Apply_Date,2) " ;
            Tsql = Tsql + " ,0 ";
            Tsql = Tsql + " , Isnull(tbl_Class_P.Grade_Name,'')  ";

            Tsql = Tsql + " ,tbl_Sham_Grade_P.ETC " ;
            Tsql = Tsql + " ,tbl_Sham_Grade_P.RecordID " ;
            Tsql = Tsql + " ,tbl_Sham_Grade_P.RecordTime ";
            Tsql = Tsql + " ,tbl_Sham_Grade_P.seq ";
            Tsql = Tsql + " ,tbl_Sham_Grade_P.Apply_Grade";
            Tsql = Tsql + " From tbl_Sham_Grade_P  (nolock) ";
            Tsql = Tsql + " Left Join tbl_Memberinfo   (nolock) On tbl_Memberinfo.Mbid  = tbl_Sham_Grade_P.Mbid  And tbl_Memberinfo.Mbid2 = tbl_Sham_Grade_P.Mbid2  ";
            ...
            Tsql = Tsql + " LEFT Join tbl_Class_P   (nolock) ON tbl_Class_P.Grade_Cnt = tbl_Sham_Grade_P.Apply_Grade ";
                                   
            string strSql = " Where   tbl_Sham_Grade_P.M_Name <> '' ";
                ...
                )
            {
                ...
                {
                    if (Mbid != "")
                        strSql = strSql + " And tbl_Sham_Grade_P.Mbid ='" + Mbid + "'";

                    if (Mbid2 >= 0)
                        strSql = strSql + " And tbl_Sham_Grade_P.Mbid2 = " + Mbid2;
                }
            }
            ...
            if (txtName2.Text.Trim() != "")
                strSql = strSql + " And tbl_Sham_Grade_P.M_Name Like '%" + txtName2.Text.Trim() + "%'";

            //회원명으로 검색
            if (combo_Pay_Code2.Text.Trim() != "")
                strSql = strSql + " And tbl_Sham_Grade_P.Apply_Grade = '" + combo_Pay_Code2.Text.Trim() + "'";


            //가입일자로 검색 -1
            if ((mtxtSellDate2.Text.Replace("-", "").Trim() != "") && (mtxtSellDate3.Text.Replace("-", "").Trim() == ""))
                strSql = strSql + " And tbl_Sham_Grade_P.Apply_Date = '" + mtxtSellDate2.Text.Replace("-", "").Trim() + "'";

            //가입일자로 검색 -2
            if ((mtxtSellDate2.Text.Replace("-", "").Trim() != "") && (mtxtSellDate3.Text.Replace("-", "").Trim() != ""))
            {
                strSql = strSql + " And tbl_Sham_Grade_P.Apply_Date >= '" + mtxtSellDate2.Text.Replace("-", "").Trim() + "'";
                strSql = strSql + " And tbl_Sham_Grade_P.Apply_Date <= '" + mtxtSellDate3.Text.Replace("-", "").Trim() + "'";
            }


            //기록일자로 검색 -1
            if ((mtxtMakeDate1.Text.Replace("-", "").Trim() != "") && (mtxtMakeDate2.Text.Replace("-", "").Trim() == ""))
                strSql = strSql + " And Replace(Left( tbl_Sham_Grade_P.recordtime ,10),'-','') = '" + mtxtMakeDate1.Text.Replace("-", "").Trim() + "'";

            //기록일자로 검색 -2
            if ((mtxtMakeDate1.Text.Replace("-", "").Trim() != "") && (mtxtMakeDate2.Text.Replace("-", "").Trim() != ""))
            {
                strSql = strSql + " And Replace(Left( tbl_Sham_Grade_P.recordtime ,10),'-','') >= '" + mtxtMakeDate1.Text.Replace("-", "").Trim() + "'";
                strSql = strSql + " And Replace(Left( tbl_Sham_Grade_P.recordtime ,10),'-','') <= '" + mtxtMakeDate2.Text.Replace("-", "").Trim() + "'";
            }

             if (txtR_Id_Code.Text.Trim() != "")
                strSql = strSql + " And tbl_Sham_Grade_P.recordid = '" + txtR_Id_Code.Text.Trim() + "'";
            ...
            Tsql = Tsql + strSql;
            Tsql = Tsql + " Order by tbl_Sham_Grade_P.Mbid ,  tbl_Sham_Grade_P.Mbid2   ";
        ...
        private void dGridView_Base_Sub_Header_Reset()
        {
            ...
            string[] g_HeaderText = {"회원번호",  "성명"  , "적용_일자"   , ""  , "인정_등급"          
                                , "비고"  , "기록자"   , "기록일자"    , "_seq"   , "_Apply_Grade"
                                };
        ...
        private Boolean Check_TextBox_Error()
        {
            ...
            if (combo_Pay.Text == "" || combo_Pay_Code.Text == "" )
            {
                MessageBox.Show(cls_app_static_var.app_msg_rm.GetString("Msg_Not_Input")
                       + "-" + cls_app_static_var.app_msg_rm.GetString("Msg_Sort_Grade")
                      + "\n" +
                      cls_app_static_var.app_msg_rm.GetString("Msg_Re_Action"));
        ...
        private void Save_Base_Data(ref int Save_Error_Check)
        {
            ...
            try
            {
               // double app_Pv =   double.Parse(txt_Pv.Text.Trim().Replace(",", ""));                
                ...
                StrSql = "INSERT INTO tbl_Sham_Grade_P ";
                StrSql = StrSql + " (";
                StrSql = StrSql + "  mbid , mbid2 , M_Name ";
                StrSql = StrSql + " , Apply_Date, Apply_Grade  ";
                StrSql = StrSql + " ,  ETC ";
                ...
                StrSql = StrSql + ",'" + combo_Pay_Code.Text +  "'"  ;
        ...
        {                      
            ...
            try
            {
                ...
                StrSql = "INSERT INTO tbl_Sham_Grade_P_Mod ";
                StrSql = StrSql + " Select  * ";
                StrSql = StrSql + ",'" + cls_User.gid + "'";
                StrSql = StrSql + ",Convert(Varchar(25),GetDate(),21) ";
                StrSql = StrSql + " From tbl_Sham_Grade_P ";
                StrSql = StrSql + " Where seq = " + txtKey.Text; 

                Temp_Connect.Insert_Data(StrSql, base_db_name, Conn, tran);

                StrSql = "UpDate  tbl_Sham_Grade_P Set ";
                StrSql = StrSql + "  Apply_Grade = " + int.Parse (combo_Pay_Code.Text) ;
        ...
        private void Delete_Base_Data(ref int Delete_Error_Check)
        {
            ...
            try
            {
                ...
                StrSql = "Insert into  tbl_Sham_Grade_P_Mod ";
                StrSql = StrSql + " Select *  ";
                StrSql = StrSql + ",'" + cls_User.gid + "', Convert(Varchar(25),GetDate(),21) From tbl_Sham_Grade_P ";
                StrSql = StrSql + " Where seq = " + txtKey.Text;
                
                Temp_Connect.Insert_Data(StrSql, base_db_name, Conn, tran);


                StrSql = "Delete From tbl_Sham_Grade_P  ";
                StrSql = StrSql + " Where seq = " + txtKey.Text;
        ...
        private void dGridView_Base_Sub_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (((sender as DataGridView).CurrentRow != null) && ((sender as DataGridView).CurrentRow.Cells[1].Value != null))
            {
                ...
                double T_PV = 0; string Grade_Name = ""; string T_ETC = "";
                int t_Seq = 0; string Apply_Grade = "";
                ...
                Grade_Name = (sender as DataGridView).CurrentRow.Cells[4].Value.ToString();
                ...
                Apply_Grade = (sender as DataGridView).CurrentRow.Cells[9].Value.ToString();
                ...
                combo_Pay.Text = Grade_Name;
                combo_Pay_Code.Text = Apply_Grade;
