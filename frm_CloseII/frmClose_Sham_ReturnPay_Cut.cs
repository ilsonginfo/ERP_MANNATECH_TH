...
{
    public partial class frmClose_Sham_ReturnPay_Cut : clsForm_Extends
    {
        ...
        private const string base_db_name = "tbl_Sham_ReturnPay_Cut";
        private int Data_Set_Form_TF;

        public frmClose_Sham_ReturnPay_Cut()
        {
            InitializeComponent();
        ...
        private void Form_Clear_()
        {
            ...
            //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            dGridView_Base_Sub.DataSource = null;
            dGridView_Base_Sub_Header_Reset(); //디비그리드 헤더와 기본 셋팅을 한다.
        ...
        private void Base_Sub_Grid_Set()
        {
            ...
            ////    Tsql = Tsql + " tbl_Sham_ReturnPay_Cut.mbid + '-' + Convert(Varchar,tbl_Sham_ReturnPay_Cut.mbid2) ";
            ////else
            ////    Tsql = Tsql + " tbl_Sham_ReturnPay_Cut.mbid2 ";
            ...
            if (cls_app_static_var.Member_Number_1 > 0 && cls_app_static_var.Member_Number_2 > 0)
                ...
                Tsql = Tsql + " tbl_Sham_ReturnPay_Cut.mbid  ";
            ...
            //Tsql = Tsql + " , Isnull(tbl_SellType.SellTypeName,'')  ";

            Tsql = Tsql + " , Case  ";
            Tsql = Tsql + "  When tbl_Sham_ReturnPay_Cut.Sellcode  = 'W' then '주간' ";
            Tsql = Tsql + "  When tbl_Sham_ReturnPay_Cut.Sellcode  = 'M' then '월간' ";
            ...
            Tsql = Tsql + " ,tbl_Sham_ReturnPay_Cut.ETC " ;
            Tsql = Tsql + " ,tbl_Sham_ReturnPay_Cut.RecordID " ;
            Tsql = Tsql + " ,tbl_Sham_ReturnPay_Cut.RecordTime ";
            Tsql = Tsql + " ,tbl_Sham_ReturnPay_Cut.seq ";
            Tsql = Tsql + " ,tbl_Sham_ReturnPay_Cut.Sellcode";
            Tsql = Tsql + " From tbl_Sham_ReturnPay_Cut  (nolock) ";
            Tsql = Tsql + " Left Join tbl_Memberinfo   (nolock) On tbl_Memberinfo.Mbid  = tbl_Sham_ReturnPay_Cut.Mbid  And tbl_Memberinfo.Mbid2 = tbl_Sham_ReturnPay_Cut.Mbid2  ";
            ...
            //Tsql = Tsql + " LEFT Join tbl_SellType   (nolock) ON tbl_Sham_ReturnPay_Cut.SellCode = tbl_SellType.SellCode ";
                                   
            string strSql = " Where   tbl_Sham_ReturnPay_Cut.Mbid2 > 0 ";
                ...
                )
            {
                ...
                {
                    if (Mbid != "")
                        strSql = strSql + " And tbl_Sham_ReturnPay_Cut.Mbid ='" + Mbid + "'";

                    if (Mbid2 >= 0)
                        strSql = strSql + " And tbl_Sham_ReturnPay_Cut.Mbid2 = " + Mbid2;
                }
            }
            ...
            if (txtName2.Text.Trim() != "")
                strSql = strSql + " And tbl_Sham_ReturnPay_Cut.M_Name Like '%" + txtName2.Text.Trim() + "%'";

            if (combo_Se_Code2.Text.Trim() != "")
                strSql = strSql + " And tbl_Sham_ReturnPay_Cut.SellCode  = '" + combo_Se_Code2.Text.Trim() + "'";
            ...
            if ((mtxtSellDate2.Text.Replace("-", "").Trim() != "") && (mtxtSellDate3.Text.Replace("-", "").Trim() == ""))
                strSql = strSql + " And tbl_Sham_ReturnPay_Cut.Apply_Date = '" + mtxtSellDate2.Text.Replace("-", "").Trim() + "'";

            //가입일자로 검색 -2
            if ((mtxtSellDate2.Text.Replace("-", "").Trim() != "") && (mtxtSellDate3.Text.Replace("-", "").Trim() != ""))
            {
                strSql = strSql + " And tbl_Sham_ReturnPay_Cut.Apply_Date >= '" + mtxtSellDate2.Text.Replace("-", "").Trim() + "'";
                strSql = strSql + " And tbl_Sham_ReturnPay_Cut.Apply_Date <= '" + mtxtSellDate3.Text.Replace("-", "").Trim() + "'";
            }


            //기록일자로 검색 -1
            if ((mtxtMakeDate1.Text.Replace("-", "").Trim() != "") && (mtxtMakeDate2.Text.Replace("-", "").Trim() == ""))
                strSql = strSql + " And Replace(Left( tbl_Sham_ReturnPay_Cut.recordtime ,10),'-','') = '" + mtxtMakeDate1.Text.Replace("-", "").Trim() + "'";

            //기록일자로 검색 -2
            if ((mtxtMakeDate1.Text.Replace("-", "").Trim() != "") && (mtxtMakeDate2.Text.Replace("-", "").Trim() != ""))
            {
                strSql = strSql + " And Replace(Left( tbl_Sham_ReturnPay_Cut.recordtime ,10),'-','') >= '" + mtxtMakeDate1.Text.Replace("-", "").Trim() + "'";
                strSql = strSql + " And Replace(Left( tbl_Sham_ReturnPay_Cut.recordtime ,10),'-','') <= '" + mtxtMakeDate2.Text.Replace("-", "").Trim() + "'";
            }


             if (txtR_Id_Code.Text.Trim() != "")
                strSql = strSql + " And tbl_Sham_ReturnPay_Cut.recordid = '" + txtR_Id_Code.Text.Trim() + "'";
            ...
            Tsql = Tsql + strSql;
            Tsql = Tsql + " Order by tbl_Sham_ReturnPay_Cut.Mbid ,  tbl_Sham_ReturnPay_Cut.Mbid2   ";
            ...
            //if (ReCnt == 0)
            //{
            //    dGridView_Base_Sub.Columns.Clear();
            //    dGridView_Base_Sub.DataSource = ds.Tables[0];
            //    cg_Sub.d_Grid_view_DataSource_Header_Reset(dGridView_Base_Sub, g_HeaderText, g_Width, g_ReadOnly, g_Alignment, 0);
            //    return;
            //}
            //else
            //{
            //    dGridView_Base_Sub.Columns.Clear();
            //    dGridView_Base_Sub.DataSource = ds.Tables[0];
            //    Class.cls_Function.DoubleBuffered(dGridView_Base_Sub, true);

            //    cg_Sub.d_Grid_view_DataSource_Header_Reset(dGridView_Base_Sub, g_HeaderText, g_Width, g_ReadOnly, g_Alignment, 2);
            //    dGridView_Base_Sub.Columns[3].DefaultCellStyle.Format = cls_app_static_var.str_Grid_Currency_Type;
            //}
        ...
        private void Save_Base_Data(ref int Save_Error_Check)
        {
            ...
            string SellCode = "";
            if (radioB_ETC.Checked == true) SellCode = "W";
            if (radioB_Save.Checked == true) SellCode = "M";

            try
            {
                double app_Pv =   double.Parse(txt_Pv.Text.Trim().Replace(",", ""));                
                               
                StrSql = "INSERT INTO tbl_Sham_ReturnPay_Cut ";
                StrSql = StrSql + " (";
        ...
        {                      
            ...
            try
            {
                ...
                StrSql = "INSERT INTO tbl_Sham_ReturnPay_Cut_Mod ";
                StrSql = StrSql + " Select  * ";
                StrSql = StrSql + ",'" + cls_User.gid + "'";
                StrSql = StrSql + ",Convert(Varchar(25),GetDate(),21) ";
                StrSql = StrSql + " From tbl_Sham_ReturnPay_Cut ";
                StrSql = StrSql + " Where seq = " + txtKey.Text; 

                Temp_Connect.Insert_Data(StrSql, base_db_name, Conn, tran);


                StrSql = "UpDate  tbl_Sham_ReturnPay_Cut Set ";
                StrSql = StrSql + "  Apply_Pv = " + app_Pv;
                StrSql = StrSql + ", Apply_Date = '" + mtxtSellDate.Text.Replace("-", "").Trim()  + "'";
                StrSql = StrSql + ", SellCode  = '" + SellCode + "'";
        ...
        private void Delete_Base_Data(ref int Delete_Error_Check)
        {
            ...
            try
            {
                ...
                StrSql = "Insert into  tbl_Sham_ReturnPay_Cut_Mod ";
                StrSql = StrSql + " Select *  ";
                StrSql = StrSql + ",'" + cls_User.gid + "', Convert(Varchar(25),GetDate(),21) From tbl_Sham_ReturnPay_Cut ";
                StrSql = StrSql + " Where seq = " + txtKey.Text;
                
                Temp_Connect.Insert_Data(StrSql, base_db_name, Conn, tran);


                StrSql = "Delete From tbl_Sham_ReturnPay_Cut  ";
                StrSql = StrSql + " Where seq = " + txtKey.Text;
        ...
        private void dGridView_Base_Sub_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (((sender as DataGridView).CurrentRow != null) && ((sender as DataGridView).CurrentRow.Cells[0].Value != null))
            {
                ...
                if (T_SellCode == "W") radioB_ETC.Checked = true;
                if (T_SellCode == "M") radioB_Save.Checked = true;
                ...
                //마감정산이 이루어진 판매 날짜인지 체크한다.                
                cls_Search_DB csd = new cls_Search_DB();
                if (T_SellCode == "W" && csd.Close_Check_SellDate("tbl_CloseTotal_02", mtxtSellDate.Text.Replace("-", "").Trim()) == false)
                {
                ...
                if (T_SellCode == "M" && csd.Close_Check_SellDate("tbl_CloseTotal_04", mtxtSellDate.Text.Replace("-", "").Trim()) == false)
                {
                    mtxtSellDate.ReadOnly = true;
                    mtxtSellDate.BorderStyle = BorderStyle.FixedSingle;
                    mtxtSellDate.BackColor = cls_app_static_var.txt_Enable_Color;
