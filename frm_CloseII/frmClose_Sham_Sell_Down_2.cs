...
{
    public partial class frmClose_Sham_Sell_Down_2 : clsForm_Extends
    {
        ...
        public frmClose_Sham_Sell_Down_2()
        {
            InitializeComponent();
        ...
        private void Base_Sub_Grid_Set()
        {
            ...
            if (cls_app_static_var.Member_Number_1 > 0)
                Tsql = Tsql + " tbl_Sham_Line_New.mbid + '-' + Convert(Varchar,tbl_Sham_Line_New.mbid2) ";
            else
                Tsql = Tsql + " tbl_Sham_Line_New.mbid2 ";
            Tsql = Tsql + " ,tbl_Sham_Line_New.M_Name ";            
            ...
            Tsql = Tsql + " ,tbl_Sham_Line_New.ETC " ;
            Tsql = Tsql + " ,tbl_Sham_Line_New.RecordID " ;
            Tsql = Tsql + " ,tbl_Sham_Line_New.RecordTime ";
            Tsql = Tsql + " ,tbl_Sham_Line_New.seq ";
            Tsql = Tsql + " ,tbl_Sham_Line_New.SortKind2";
            Tsql = Tsql + " From tbl_Sham_Line_New  (nolock) ";
            Tsql = Tsql + " Left Join tbl_Memberinfo   (nolock) On tbl_Memberinfo.Mbid  = tbl_Sham_Line_New.Mbid  And tbl_Memberinfo.Mbid2 = tbl_Sham_Line_New.Mbid2  ";
            ...
            //Tsql = Tsql + " LEFT Join tbl_SellType   (nolock) ON tbl_Sham_Line_New.SellCode = tbl_SellType.SellCode ";
                                   
            string strSql = " Where   tbl_Sham_Line_New.M_Name <> '' ";
                ...
                )
            {
                ...
                {
                    if (Mbid != "")
                        strSql = strSql + " And tbl_Sham_Line_New.Mbid ='" + Mbid + "'";

                    if (Mbid2 >= 0)
                        strSql = strSql + " And tbl_Sham_Line_New.Mbid2 = " + Mbid2;
                }
            }
            ...
            if (txtName2.Text.Trim() != "")
                strSql = strSql + " And tbl_Sham_Line_New.M_Name Like '%" + txtName2.Text.Trim() + "%'";

            if (combo_Se_Code2.Text.Trim() != "")
                strSql = strSql + " And tbl_Sham_Line_New.SellCode  = '" + combo_Se_Code2.Text.Trim() + "'";
            ...
            if ((mtxtSellDate2.Text.Replace("-", "").Trim() != "") && (mtxtSellDate3.Text.Replace("-", "").Trim() == ""))
                strSql = strSql + " And tbl_Sham_Line_New.Apply_Date = '" + mtxtSellDate2.Text.Replace("-", "").Trim() + "'";

            //가입일자로 검색 -2
            if ((mtxtSellDate2.Text.Replace("-", "").Trim() != "") && (mtxtSellDate3.Text.Replace("-", "").Trim() != ""))
            {
                strSql = strSql + " And tbl_Sham_Line_New.Apply_Date >= '" + mtxtSellDate2.Text.Replace("-", "").Trim() + "'";
                strSql = strSql + " And tbl_Sham_Line_New.Apply_Date <= '" + mtxtSellDate3.Text.Replace("-", "").Trim() + "'";
            }


            //기록일자로 검색 -1
            if ((mtxtMakeDate1.Text.Replace("-", "").Trim() != "") && (mtxtMakeDate2.Text.Replace("-", "").Trim() == ""))
                strSql = strSql + " And Replace(Left( tbl_Sham_Line_New.recordtime ,10),'-','') = '" + mtxtMakeDate1.Text.Replace("-", "").Trim() + "'";

            //기록일자로 검색 -2
            if ((mtxtMakeDate1.Text.Replace("-", "").Trim() != "") && (mtxtMakeDate2.Text.Replace("-", "").Trim() != ""))
            {
                strSql = strSql + " And Replace(Left( tbl_Sham_Line_New.recordtime ,10),'-','') >= '" + mtxtMakeDate1.Text.Replace("-", "").Trim() + "'";
                strSql = strSql + " And Replace(Left( tbl_Sham_Line_New.recordtime ,10),'-','') <= '" + mtxtMakeDate2.Text.Replace("-", "").Trim() + "'";
            }


             if (txtR_Id_Code.Text.Trim() != "")
                strSql = strSql + " And tbl_Sham_Line_New.recordid = '" + txtR_Id_Code.Text.Trim() + "'";
            ...
            Tsql = Tsql + strSql;
            Tsql = Tsql + " Order by tbl_Sham_Line_New.Mbid ,  tbl_Sham_Line_New.Mbid2   ";
        ...
        private Boolean Check_TextBox_Error()
        {
            ...
            if (txtKey.Text != "") //수정일 경우에는 수정 프로시져로 가고 이 프로시져를 빠져나가라
            {
                ...
                Tsql = Tsql + " And  Apply_Date  = '" + mtxtSellDate.Text.Replace("-", "").Trim() + "'";
                ...
                if (ReCnt > 0)  //실제로 존재하는 회원 번호 인가.
                {
                    ...
                    MessageBox.Show("동일한 회원이 동일 적용일로 해서 입력 하신 내역이 있습니다."
                           + "\n" +
                           cls_app_static_var.app_msg_rm.GetString("Msg_Re_Action"));
                    mtxtSellDate.Focus(); return false;
                }
        ...
        private void Save_Base_Data(ref int Save_Error_Check)
        {
            ...
            try
            {
                ...
                StrSql = "INSERT INTO tbl_Sham_Line_New ";
                StrSql = StrSql + " (";
        ...
        private void Delete_Base_Data(ref int Delete_Error_Check)
        {
            ...
            try
            {
                ...
                StrSql = "Insert into  tbl_Sham_Line_New_Mod ";
                StrSql = StrSql + " Select *  ";
                StrSql = StrSql + ",'" + cls_User.gid + "', Convert(Varchar(25),GetDate(),21) From tbl_Sham_Line_New ";
                StrSql = StrSql + " Where seq = " + txtKey.Text;
                
                Temp_Connect.Insert_Data(StrSql, base_db_name, Conn, tran);


                StrSql = "Delete From tbl_Sham_Line_New  ";
                StrSql = StrSql + " Where seq = " + txtKey.Text;
