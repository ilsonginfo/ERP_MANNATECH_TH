...
{
    public partial class frmStock_Close_Select : clsForm_Extends
    {
        ...
        {   
            ...
            strSQL = strSQL + " CenterCode  ,    isnull(tbl_Business.name,'')           CenterName  ,";
            strSQL = strSQL + " ItemCode ,       isnull(tbl_Goods.name,'')       ItemName  ,";
            strSQL = strSQL + " PreStockQty ,";
        
            strSQL = strSQL + " InQty  ,   " ;//'  ----기간 거래처 입고 IN_FL = 003"
            strSQL = strSQL + " ReturnInQty  ,   "; //'  ----기간 반품 입고"
            strSQL = strSQL + " ChangeInQty  ,   " ;//'  ----기간 교환 입고"
            strSQL = strSQL + " MoveInQty  ,   "; //'  ----기간 재고이동에서 입고"
    
            strSQL = strSQL + " InQty + ReturnInQty + ChangeInQty +  MoveInQty ,";
            strSQL = strSQL + " '  ',";
    
    
            strSQL = strSQL + " SaleOutQty  ,   " ; // ' ----기간 판매 출고 Out_FL = 001 중에서 교환빼고"
            strSQL = strSQL + " ChangeOrderOutQty ,"; // '    ----기간 교환 출고 Out_FL = 001 중에서 교환만"
            strSQL = strSQL + " DisuseOutQty,   "; // '  ----기간 IR출고 Out_FL = 013"
            strSQL = strSQL + " ReSendOutQty,  " ; //'   ----품질검사 출고 Out_FL = 012"
            //strSQL = strSQL + " GOutQty  ,     " ; //'----미조립 출고 Out_FL = 014"
            strSQL = strSQL + " MoveOutQty  ,     "; //'----이동출고
            strSQL = strSQL + " EtcOutQty  , " ; //'    ----기타 출고 -  출고 기타출고  Out_FL = 200"
        
    
            strSQL = strSQL + " SaleOutQty + ChangeOrderOutQty + DisuseOutQty +  ReSendOutQty + GOutQty + MoveOutQty + EtcOutQty  ," ;
    
    
            strSQL = strSQL + " pooraddqty,";

            strSQL = strSQL + " ToDayStock,";                           
            //strSQL = strSQL + " PreStockQty + (InQty + ReturnInQty + ChangeInQty + MoveInQty)"; 
            //strSQL = strSQL + " - (SaleOutQty + ChangeOrderOutQty + DisuseOutQty +  ReSendOutQty + GOutQty + MoveOutQty + EtcOutQty) - pooraddqty";

            strSQL = strSQL + " Pre_Chang_StockQty ,Pre_Chang_StockQty";
            strSQL = strSQL + " ,Case when GoodUse  = 1 then '1' ELSE '' End ";
            ...
            strSQL = strSQL + "  Where CenterCode <> '' "; 
            ...
            if (CenterCode != "")
                strSQL = strSQL + "  And  CenterCode = '" + CenterCode + "'";

            strSQL = strSQL + "  Order by StockDay DESC ,  CenterCode , Case when GoodUse  = 1 then '1' ELSE '' End "; 
        ...
        private void dGridView_Base_Header_Reset()
        {
            ...
            string[] g_HeaderText = { "마감일", "센타코드"   , "센타명" , "상품코드"   , "상품명"      
