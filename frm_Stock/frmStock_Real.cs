...
{
    public partial class frmStock_Real : clsForm_Extends
    {
        ...
        private void Base_Sub_Grid_Set()
        {
            ...
            //string[] g_HeaderText = {"재고_마감일",  "조정지"  , "상품코드"   , "상품명"  , "수량"          
            //                    , "비고"  , "기록자"   , "기록일자"    , "_CenterCode"   , "_seq"       
            //                    , ""    , ""   , ""    , ""   , ""   
            //                    };

            Tsql = "Select LEFT(DayStock_Real.StockDay,4) +'-' + LEFT(RIGHT(DayStock_Real.StockDay,4),2) + '-' + RIGHT(DayStock_Real.StockDay,2) ";
            Tsql = Tsql + " ,Isnull(tbl_Business.name,'') ";
            Tsql = Tsql + " ,DayStock_Real.ItemCode ";
            Tsql = Tsql + " ,Isnull(tbl_Goods.name,'') ";
            Tsql = Tsql + " ,DayStock_Real.Real_Cnt ";

            Tsql = Tsql + " ,Day_ETC ";
            Tsql = Tsql + " ,DayStock_Real.Recordid ";
            Tsql = Tsql + " ,DayStock_Real.Recordtime ";
            Tsql = Tsql + " ,DayStock_Real.CenterCode ";
            Tsql = Tsql + " ,DayStock_Real.Seq ";            
            
            Tsql = Tsql + " ,'','','','','' ";
            Tsql = Tsql + " From DayStock_Real  (nolock) ";
            Tsql = Tsql + " LEFT Join tbl_Goods  (nolock) ON ItemCode = tbl_Goods.Ncode ";
            Tsql = Tsql + " LEFT Join tbl_Business (nolock)  ON tbl_Business.Ncode = DayStock_Real.CenterCode  ";

            Tsql = Tsql + " Where (DayStock_Real.Seq > 0  )";

            if (mtxtInDate2.Text.Replace("-", "").Trim() != "" && mtxtInDate3.Text.Replace("-", "").Trim() == "")
                Tsql = Tsql + " And DayStock_Real.StockDay = '" + mtxtInDate2.Text + "'";

            if (mtxtInDate2.Text.Replace("-", "").Trim() != "" && mtxtInDate3.Text.Replace("-", "").Trim() != "")
            {
                Tsql = Tsql + " And DayStock_Real.StockDay >= '" + mtxtInDate2.Text.Replace("-", "").Trim() + "'";
                Tsql = Tsql + " And DayStock_Real.StockDay <= '" + mtxtInDate3.Text.Replace("-", "").Trim() + "'";
            }



            if (txtCenter_Code2.Text != "" )
                Tsql = Tsql + " And DayStock_Real.CenterCode = '" + txtCenter_Code2.Text + "'";

            if (txt_ItemName_Code2.Text != "")
                Tsql = Tsql + " And DayStock_Real.ItemCode = '" + txt_ItemName_Code2.Text + "'";

            if (txtR_Id_Code2.Text != "")
                Tsql = Tsql + " And DayStock_Real.Recordid = '" + txtR_Id_Code2.Text + "'";

            Tsql = Tsql + " Order by DayStock_Real.StockDay DESC , DayStock_Real.ItemCode  , DayStock_Real.Seq ";
        ...
        private void dGridView_Base_Sub_Header_Reset()
        {
            ...
            cg_Sub.grid_Frozen_End_Count = 4;
        ...
        {           
            ...
            string Tsql = "";
            Tsql = "Select NCode , Name , '' ,price ,''    ";
            //Tsql = "Select '', Name , NCode ,price ,''    ";
            Tsql = Tsql + " , '', '' ,'' ,'' ,'' "; 
        ...
        private void dGridView_Base_Header_Reset()
        {
            ...
            string[] g_HeaderText = { "상품코드", "상품명" , "수량" ,    "_소비자가"   , ""        
                                , ""   , ""    , ""   , ""    , ""                                
                                    };
            ...
            int[] g_Width = { 95, 230, 60, 0, 0                             
                             ,0 , 0 ,  0 , 0 ,  0                             
                            };
            cgb.grid_col_w = g_Width;

            Boolean[] g_ReadOnly = { true , true,  false,  true ,true                                     
                                    ,true , true,  true,  true ,true                                                                         
                                   };
        ...
        private Boolean Check_TextBox_Error()
        {
            ...
            me = T_R.Text_Null_Check(mtxtInDate, "Msg_Sort_Stock_Close_Date"); //입고일자를
            if (me != "")
            {
                MessageBox.Show(me);
                mtxtInDate.Focus();
            ...
            me = T_R.Text_Null_Check(txtCenter_Code, "Msg_Sort_Stock_Close_Center"); //입고지를
            if (me != "")
            {
                MessageBox.Show(me);
                txtCenter.Focus();
            ...
            for (int i = 0; i <= dGridView_Base.Rows.Count - 1; i++)
            {
                ...
                if (dGridView_Base.Rows[i].Cells[2].Value.ToString() != "")
                {
                    ItemCode = dGridView_Base.Rows[i].Cells[0].Value.ToString();    
                    chk_cnt++;
                    if (txtKey.Text.Trim() == "")
                    {
                        string Tsql = "";

                        Tsql = "Select StockDay , CenterCode , ItemCode   ";
                        Tsql = Tsql + " From DayStock_Real (nolock) ";
                        Tsql = Tsql + " Where  StockDay = '" + mtxtInDate.Text.Replace("-", "").Trim() + "'";   //입고일자                        
                        Tsql = Tsql + " And  CenterCode = '" + txtCenter_Code.Text.Trim() + "'";  //센타/창고 코드 번호
                        Tsql = Tsql + " And  ItemCode = '" + ItemCode + "'";       //상품코드
                        ...
                        if (ReCnt > 0)
                        {
                            MessageBox.Show(cls_app_static_var.app_msg_rm.GetString("Msg_Same_Data") + "\n" +
                                cls_app_static_var.app_msg_rm.GetString("Msg_Re_Action"));
                            return false;
                        }
                    }
            ...
            if (csd.Check_Stock_Close(txtCenter_Code.Text.Trim(), mtxtInDate.Text.Replace("-", "").Trim()) == false)
            {
                mtxtInDate.Focus(); return false;
            }
        ...
        private void Save_Base_Data(ref int Save_Error_Check)
        {
            ...
            {               
                ...
                for (int i = 0; i < dGridView_Base.Rows.Count; i++)
                {
                    if (dGridView_Base.Rows[i].Cells[2].Value.ToString() != "")
                    {                       
                        ItemCnt = int.Parse(dGridView_Base.Rows[i].Cells[2].Value.ToString());
                        ItemCode = dGridView_Base.Rows[i].Cells[0].Value.ToString();                          
                        
                        StrSql = "Insert into DayStock_Real (";
                        StrSql = StrSql + " StockDay , CenterCode ,ItemCode , Real_Cnt ";
                        StrSql = StrSql + " , Day_ETC ";
        ...
        private void Save_Base_Data_UpDate(ref int Save_Error_Check)
        {
            ...
            for (int i = 0; i < dGridView_Base.Rows.Count; i++)
            {
                if (dGridView_Base.Rows[i].Cells[2].Value.ToString() != "")
                {
                    ...
                    StrSql = "INSERT INTO DayStock_Real_Mod ";                    
                    StrSql = StrSql + " Select  * ";                    
                    StrSql = StrSql + ",'" + cls_User.gid  + "'" ;
                    StrSql = StrSql + ",Convert(Varchar(25),GetDate(),21) ";
                    StrSql = StrSql + " From DayStock_Real ";
                    StrSql = StrSql + " Where Seq = '" + txtKey.Text.Trim() + "'";

                    Temp_Connect.Insert_Data(StrSql, base_db_name, Conn, tran);
                    

                    StrSql = "UpDate  DayStock_Real Set ";                    
                    StrSql = StrSql + "  StockDay = '" + mtxtInDate.Text.Replace("-", "").Trim() + "'";
                    StrSql = StrSql + ", CenterCode  = '" + txtCenter_Code.Text.Trim() + "'";
                    StrSql = StrSql + ", ItemCode = '" + ItemCode + "'";
                    StrSql = StrSql + ", Real_Cnt = " + ItemCnt;
                    StrSql = StrSql + ", Day_ETC = '" + txtRemark.Text.Trim() + "'";                                        
        ...
        private void Delete_Base_Data(ref int Delete_Error_Check)
        {
            ...
            try
            {
                ...
                StrSql = "Insert into  DayStock_Real_Mod ";
                StrSql = StrSql + " Select *  ";
                StrSql = StrSql + ",'" + cls_User.gid + "', Convert(Varchar(25),GetDate(),21) From DayStock_Real ";
                StrSql = StrSql + " Where Seq = " + txtKey.Text.Trim();
                
                Temp_Connect.Insert_Data(StrSql, base_db_name, Conn, tran);


                StrSql = "Delete From DayStock_Real  ";
                StrSql = StrSql + " Where Seq = " + txtKey.Text.Trim();
        ...
        private void dGridView_Base_Sub_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                string Seq = ""; string In_Date = ""; string In_FL = "";
                string In_C_Code = ""; string ItemCode = ""; int ItemCnt = 0;
                string Remarks1 = "";  string In_Name ="" ; string Center_Name = "";
                string U_Name = "";

                Seq = (sender as DataGridView).CurrentRow.Cells[9].Value.ToString();
                In_Date = (sender as DataGridView).CurrentRow.Cells[0].Value.ToString();
                ...
                ItemCnt = int.Parse ((sender as DataGridView).CurrentRow.Cells[4].Value.ToString());
                Remarks1 = (sender as DataGridView).CurrentRow.Cells[5].Value.ToString();

                In_C_Code = (sender as DataGridView).CurrentRow.Cells[8].Value.ToString();                
                ...
                {
                    //빈칸으로 들어간 내역을 0으로 바꾼다
                    if (dGridView_Base.Rows[i].Cells[0].Value.ToString() == ItemCode)
                        dGridView_Base.Rows[i].Cells[2].Value = ItemCnt;
                    else
                        dGridView_Base.Rows[i].Cells[2].Value = "";
                }
