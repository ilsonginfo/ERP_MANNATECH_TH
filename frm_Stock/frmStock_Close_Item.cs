...
{
    public partial class frmStock_Close_Item : clsForm_Extends
    {
        ...
        private int Set_Serarch = 0;

        public frmStock_Close_Item()
        {
            InitializeComponent();
        }
        ...
        private Boolean Check_TextBox_Error()
        {
            ...
            //{
            //    MessageBox.Show("조회 상품 정보를 입력해 주십시요."
            //            + "\n" +
            ...
            if (mtxtSDate.Text.Replace("-", "").Trim() == "" && mtxtSDate2.Text.Replace("-", "").Trim() == "" && txtCenter3_Code.Text != "")
            {
                Search_Stock_Close_Date();
                //mtxtSDate2.Focus();
                //return false;
        ...
        {   
            ...
            if (check_Set.Checked == true )
                Set_Serarch = 1;  //셋트 상품을 넣고 조회를 한 흔적이 잇다 그럼... 마감안되게 막아 놓는다.
            ...
            SDAte1 = "";
            SDate2 = cls_User.gid_date_time;
            ////SDate2 = "20151231";

            string CenterCode = txt_ItemName_Code2.Text.Trim();
            int SET_TF = 0;
            if (check_Set.Checked == true)                SET_TF = 1;

            Tsql = "EXEC Usp_Stock_Close_Total_Na_Item '" + SDAte1 + "','" + SDate2 + "','" + CenterCode + "','" + cls_User.gid_CountryCode + "'," + SET_TF;
        ...
        private void butt_Close_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(cls_app_static_var.app_msg_rm.GetString("Msg_Not_Stock_Close_Not_Na") // 국가 미지정 사용자는 재고 마감을 할수 없습니다."
                         + "\n" +
                        cls_app_static_var.app_msg_rm.GetString("Msg_Re_Action"));
            ...
            if (Set_Serarch == 1 )
            {
                MessageBox.Show(cls_app_static_var.app_msg_rm.GetString("Msg_Stock_Close_Set_Err")
                          + "\n" +
                         cls_app_static_var.app_msg_rm.GetString("Msg_Re_Action"));
            ...
            {
                string strSql = "Select Isnull(Max(StockDay),'') , CenterCode From DayStock (nolock)  ";                 
                strSql = strSql + "  LEFT JOIN tbl_Business (nolock) ON  tbl_Business.Ncode = DayStock.CenterCode";                
                strSql = strSql + "  Where tbl_Business.Na_Code = '" + cls_User.gid_CountryCode + "'"; 
                strSql = strSql  + "  Group By CenterCode";
                        ...
                        if (ds.Tables[base_db_name].Rows[fi_cnt][0].ToString() != BaseDate)
                        {
                            MessageBox.Show(cls_app_static_var.app_msg_rm.GetString("Msg_Stock_Close_Err1")
                              ...
                              cls_app_static_var.app_msg_rm.GetString("Msg_Stock_Close_Err2")
                               +"\n" +
                             cls_app_static_var.app_msg_rm.GetString("Msg_Stock_Close_Err3"));
        ...
        private void Close_Work_Real(cls_Connect_DB Temp_Connect, SqlConnection Conn, SqlTransaction tran)
        {
            ...
            for (int i = 0; i < dGridView_Base.Rows.Count; i++)
            {
                ...
                strSQL = "Insert into DayStock (StockDay,CenterCode , ItemCode ,";
...

        {
            ...
            if (txtCenter3_Code.Text.Trim() == "")  //센타 선택 안하고 전체로 마감을 돌릴경우에
            {
                    ...
                    for (int fi_cnt = 0; fi_cnt <= ReCnt - 1; fi_cnt++)
                    {
                        if (ds.Tables[base_db_name].Rows[fi_cnt][0].ToString() != BaseDate)
                        {
                            MessageBox.Show(cls_app_static_var.app_msg_rm.GetString("Msg_Stock_Close_Err1")
                              ...
                              cls_app_static_var.app_msg_rm.GetString("Msg_Stock_Close_Err2")
                               + "\n" +
                             cls_app_static_var.app_msg_rm.GetString("Msg_Stock_Close_Err4"));
                ...
                else                 
                {
                    ...
                    if (cls_User.gid_CountryCode != "")
                    {
                        strSql = strSql + "  LEFT JOIN tbl_Business (nolock) ON  tbl_Business.Ncode = tbl_StockInput.In_C_Code";
                        strSql = strSql + "  Where tbl_Business.Na_Code = '" + cls_User.gid_CountryCode + "'";
                    }
                    ...
                    if (cls_User.gid_CountryCode != "")
                    {
                        strSql = strSql + "  LEFT JOIN tbl_Business (nolock) ON  tbl_Business.Ncode = tbl_StockOutput.Out_C_Code";
                        strSql = strSql + "  Where tbl_Business.Na_Code = '" + cls_User.gid_CountryCode + "'";
                    }
                    ...
                    if (cls_User.gid_CountryCode != "")
                    {
                        strSql = strSql + "  LEFT JOIN tbl_Business (nolock) ON  tbl_Business.Ncode = DayStock_Real.CenterCode";
                        strSql = strSql + "  Where tbl_Business.Na_Code = '" + cls_User.gid_CountryCode + "'";
                    }
