...
{
    public partial class frmStock_OUT_OrderNumber_Cancel : clsForm_Extends
    {
        ...
        public frmStock_OUT_OrderNumber_Cancel()
        {
            InitializeComponent();
        }
        ...
        private void Make_Base_Query(ref string Tsql)
        {
            ...
            Tsql = "Select ''  ";
            Tsql = Tsql + " , Isnull((Select  Top 1 LEFT(Out_Date,4) +'-' + LEFT(RIGHT(Out_Date,4),2) + '-' + RIGHT(Out_Date,2)   From tbl_StockOutput  (nolock) Where tbl_StockOutput.OrderNumber = tbl_SalesDetail.OrderNumber And Out_FL ='001' Order by Salesitemindex ),'')  ";
            Tsql = Tsql + " ,  Isnull((Select Top 1 Out_Index  From tbl_StockOutput  (nolock) Where tbl_StockOutput.OrderNumber = tbl_SalesDetail.OrderNumber And Out_FL ='001' Order by Salesitemindex ),'')  ";
            ...
            Tsql = Tsql + " , Isnull((Select Sum(ItemCount) From tbl_SalesitemDetail  (nolock) Where tbl_SalesitemDetail.OrderNumber = tbl_SalesDetail.OrderNumber  And LEft(SellState,1) = 'N' ),0) ";
            Tsql = Tsql + " , Isnull((Select Sum(ItemCount) From tbl_StockOutput  (nolock) Where tbl_StockOutput.OrderNumber = tbl_SalesDetail.OrderNumber And Out_FL ='001'  ),0)  ";

            Tsql = Tsql + " ,  Isnull((Select Top 1 Isnull(tbl_Business.Name,'')  From tbl_StockOutput (nolock) LEFT JOIN tbl_Business (nolock) ON tbl_StockOutput.Out_C_Code = tbl_Business.NCode Where tbl_StockOutput.OrderNumber = tbl_SalesDetail.OrderNumber And Out_FL ='001' Order by Salesitemindex ),'')  ";

            Tsql = Tsql + " , Isnull(tbl_SellType.SellTypeName,'')  SellCodeName  ";
            ...
            //Tsql = Tsql + " LEFT JOIN tbl_SalesDetail (nolock)  ON tbl_StockOutput.OrderNumber = tbl_SalesDetail.OrderNumber ";
        ...
        {
            string strSql = " Where Isnull((Select Sum(ItemCount) From tbl_StockOutput Where tbl_StockOutput.OrderNumber = tbl_SalesDetail.OrderNumber And Out_FL ='001'  ),0) >0  ";
            strSql = strSql + " And  tbl_SalesDetail.M_name is not null   ";
            ...
            if ((mtxtOutDate.Text.Replace("-", "").Trim() != "") && (mtxtOutDate2.Text.Replace("-", "").Trim() == ""))
                strSql = strSql + " And Isnull((Select Min(Out_Date)  From tbl_StockOutput Where tbl_StockOutput.OrderNumber = tbl_SalesDetail.OrderNumber And Out_FL ='001'  ),'') = '" + mtxtOutDate.Text.Replace("-", "").Trim() + "'";

            //가입일자로 검색 -2
            if ((mtxtOutDate.Text.Replace("-", "").Trim() != "") && (mtxtOutDate2.Text.Replace("-", "").Trim() != ""))
            {
                strSql = strSql + " And Isnull((Select Min(Out_Date)  From tbl_StockOutput Where tbl_StockOutput.OrderNumber = tbl_SalesDetail.OrderNumber And Out_FL ='001'  ),'')  >= '" + mtxtOutDate.Text.Replace("-", "").Trim() + "'";
                strSql = strSql + " And Isnull((Select Min(Out_Date)  From tbl_StockOutput Where tbl_StockOutput.OrderNumber = tbl_SalesDetail.OrderNumber And Out_FL ='001'  ),'')  <= '" + mtxtOutDate2.Text.Replace("-", "").Trim() + "'";
            }
        ...
        private void dGridView_Base_Header_Reset()
        {
            ...
            string[] g_HeaderText = {"선택" ,"출고_일자", "출고번호"  , "주문번호"  , "주문_일자"   
                                 ,  "회원_번호"   , "성명"  , "_주민번호"     , "_상품코드"    , "_상품명" 
                                 , "주문_수량"    , "출고_수량" , "출고지" , "주문_종류"   , "_구분"   
                                 , "배송구분" , "_등록_센타명"    , "주문_센타명"      , ""    , ""                              
                                  , "" , ""    , ""      , ""  , ""
                                    };
            ...
            try
            {
                ...
                for (int i = 0; i < dGridView_Base.Rows.Count; i++)
                {
                    if (dGridView_Base.Rows[i].Cells[0].Value.ToString() == "V")
                    {
                        ...
                        StrSql = StrSql + " Send_itemCount1 = 0 "; 
        ...
        private void dGridView_Base_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else
            {
                ...
                //{
                //string Ordernumber = (sender as DataGridView).CurrentRow.Cells[3].Value.ToString();
               
                //cls_Grid_Base_info_Put cgbp = new cls_Grid_Base_info_Put();
                //cgbp.dGridView_Put_baseinfo(this, dGridView_Sell, "sell","", Ordernumber);

                //cls_Grid_Base_info_Put cgbp2 = new cls_Grid_Base_info_Put();
                //cgbp2.dGridView_Put_baseinfo(this, dGridView_Sell_Item, "item", "", Ordernumber);

                //cls_Grid_Base_info_Put cgbp3 = new cls_Grid_Base_info_Put();
                //cgbp3.dGridView_Put_baseinfo(this, dGridView_Sell_Cacu, "cacu", "", Ordernumber);

                //cls_Grid_Base_info_Put cgbp4 = new cls_Grid_Base_info_Put();
                //cgbp4.dGridView_Put_baseinfo(this, dGridView_Sell_Rece, "rece", "", Ordernumber);
                //}
            }
