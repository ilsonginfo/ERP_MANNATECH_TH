...
{
    public partial class frm_Coupon_Import_Member : clsForm_Extends
    {
        ...
        public frm_Coupon_Import_Member()
        {
            InitializeComponent();
        }

        private void frm_Coupon_Import_Member_Load(object sender, EventArgs e)
        {
        ...
        private void dGridView_Base_Header_Reset()
        {
            ...
            string[] g_HeaderText = { "회원번호" , "쿠폰가", "쿠폰번호","등록시간","사용시간"
                                     ,"사용유무"
                                    };

            string[] g_ColsName = {"mbid2", "saleprice" ,"couponnumber" , "recordtime","useenddate"
                                    ,"useyn"};
            cgb.grid_col_header_text = g_HeaderText;

            int[] g_Width = { 80, 120, 120, 200, 200
                              , 120
                            };
        ...
        {
            Tsql = " SELECT  mbid2,  saleprice ,couponnumber , recordtime,";
            Tsql = Tsql + " case when useyn = 'Y' THEN  useenddate ELSE NULL END ,useyn  from TLS_COUPON  (nolock)  order by mbid2";
         ...
         private void LoadNewFile()
        {
         ...
         private Boolean Chang_CenterCode()
         {
            ...
            else if (raButt_IN_2.Checked == true)
            {
                checkprice = "10000";
            }
            else if (raButt_IN_3.Checked == true)
            {
                checkprice = "20000";
            }
            cls_form_Meth cm = new cls_form_Meth();
            ...
            DataSet ds = new DataSet();
            Temp_Connect.Open_Data_Set("SELECT mbid2,SALEPRICE FROM TLS_COUPON  (nolock) ", "TLS_COUPON", ds, "", "");
            DataTable dtBulk = ds.Tables["TLS_COUPON"].Copy();
            ...
            //    nR["SALEPRICE"] = Convert.ToString(row[1]);
            ...
            //        string Tsql1;
            //    Tsql1 = "select * from TLS_COUPON where mbid2 = '" + nR["mbid2"] + "' and SALEPRICE = '"+ checkprice + "' and USEYN = 'N'";
            ...
            //if (ReCnt1 > 0)
            //{
            //    if (MessageBox.Show("기존 내용에 중복되는 회원번호, 금액의 쿠폰이 '" + ReCnt1 + "'개 있습니다." +
            //        " \n 기존 내용은 제외하고 쿠폰이 발급됩니다." +
            //        " \n 계속 진행 하시겠습니까?", ""
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                nR["mbid2"] = Convert.ToInt32(row[0]);
                nR["SALEPRICE"] = checkprice;

                //cls_Connect_DB Temp_Connect2 = new cls_Connect_DB();

                //string Tsql2;
                //Tsql2 = "select * from TLS_COUPON where mbid2 = '" + nR["mbid2"] + "' and SALEPRICE = '" + nR["SALEPRICE"] + "' and USEYN = 'N'";
                ...
                //    Tsql3 = Tsql3 + "set mbid2 = '" + nR["mbid2"] + "' and SALEPRICE = '" + nR["SALEPRICE"] + "'";
                //    Tsql3 = Tsql3 + " where mbid2 =  mbid2 = '" + nR["mbid2"] + "' and SALEPRICE = '" + nR["SALEPRICE"] + "'";
                ...
                //else
                //{
                cls_Connect_DB Temp_Connect_insert = new cls_Connect_DB();
                    string insertsql;
                    insertsql = "exec USP_INSERT_TLS_COUPON_CS '" + nR["mbid2"] + "', '" + checkprice + "' ,'" + cls_User.gid + "', '"+ mtxt_ETC.Text+ "'";
                    Temp_Connect_insert.Insert_Data(insertsql, base_db_name, Conn, tran);
         ...
         private Boolean Delete_Data_Copy()
         {
             ...
             try
             {
                 ...
                 Temp_Connect.Insert_Data(StrSql, "TLS_COUPON", Conn, tran, this.Text, this.Name);
            ...
            if ((sender as DataGridView).CurrentCell.ColumnIndex == 0)
            {
                ...
                || (T_DGv.CurrentCell.Value.ToString() == ""))
                {
                   ...
                   // mtxtdate_2.Text = T_DGv.CurrentRow.Cells[2].Value.ToString();
                    ...
                    Tsql = "Select b.m_name from TLS_COUPON  (nolock) a join tbl_memberinfo (nolock)  b on a.mbid2 = b.mbid2  where b.mbid2 = '" + mtxtMbid.Text + "'";
            ...
            try
            {
                ...
                for (int i = 0; i < dGridView_Base.Rows.Count; i++)
                {
                    if (dGridView_Base.Rows[i].Cells[0].Value.ToString() == "V")
                    {
                        ...
                        StrSql = " EXEC Usp_End_Memberinfo_Coupon '" + mbid2 + "', '" + date + "', '" + cls_User.gid + "' ";
                        DataSet ds = new DataSet();
        ...
        private void butt_Save_Click(object sender, EventArgs e)
        {
            ...
            //중복값이면 업데이트
            string Tsql = "";
            Tsql = " SELECT * from TLS_COUPON  (nolock) where mbid2 = '" + mtxtMbid.Text + "' ";
            ...
            try
            {
                ...
                {
                        string StrSql = "";
                        StrSql = " update  TLS_COUPON set mbid2 = '" + mtxtMbid.Text + "' where mbid2= '" + mbid2_2 + "' AND date = '" + date_2 + "'";
                        Temp_Connect.Insert_Data(StrSql, "TLS_COUPON", Conn, tran, this.Text, this.Name);
                ...
                else
                {
                    ...
                    StrSql = " insert into TLS_COUPON values ('" + mtxtMbid.Text + "')";
                    Temp_Connect.Insert_Data(StrSql, "TLS_COUPON", Conn, tran, this.Text, this.Name);
