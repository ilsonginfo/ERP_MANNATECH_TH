...
{
    public partial class frm_barcord_Import_Member : clsForm_Extends
    {
        ...
        private const string base_db_name = "tbl_Memberinfo_barcord";
        public frm_barcord_Import_Member()
        {
            InitializeComponent();
        }

        private void frm_barcord_Import_Member_Load(object sender, EventArgs e)
        {
        ...
        private void dGridView_Base_Header_Reset()
        {
            ...
            string[] g_HeaderText = {"선택", "회원번호" , "날짜"
                                    };

            string[] g_ColsName = {"Selected", "mbid2" , "date",
                                    };
        ...
        {
            Tsql = " SELECT '' as Selected,* from tbl_memberinfo_barcord  (nolock) ";
         ...
         private void LoadNewFile()
        {
         ...
         private Boolean Chang_CenterCode()
         {
            ...
            //if (MessageBox.Show("기존내용이 있으면 모든 내용이 삭제되고 해당 엑셀내용으로 전환됩니다. 계속 진행 하시겠습니까?", ""
            ...
            //DataSet ds = new DataSet();
            //Temp_Connect.Open_Data_Set("SELECT * FROM tbl_memberinfo_barcord", "tbl_memberinfo_barcord", ds, "", "");
            //DataTable dtBulk = ds.Tables["tbl_memberinfo_barcord"].Copy();
            ...
            //    nR["date"] = Convert.ToInt32(row[1]);
            //    dtBulk.Rows.Add(nR);
            //}

            //bulkCopy.DestinationTableName = "tbl_memberinfo_barcord";
            ...
            DataSet ds = new DataSet();
            Temp_Connect.Open_Data_Set("SELECT * FROM tbl_memberinfo_barcord  (nolock) ", "tbl_memberinfo_barcord", ds, "", "");
            DataTable dtBulk = ds.Tables["tbl_memberinfo_barcord"].Copy();
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                if (row[0] == DBNull.Value || row[0].ToString() == string.Empty) continue;
                nR["mbid2"] = Convert.ToInt32(row[0]);
                nR["date"] = Convert.ToString(row[1]);


                cls_Connect_DB Temp_Connect1 = new cls_Connect_DB();

                string Tsql1;
                Tsql1 = "select * from tbl_memberinfo_barcord  (nolock)  where mbid2 = '" + nR["mbid2"] + "'  and date = '" + nR["date"] + "'";
            ...
            if (ReCnt1 > 0)
            {
                if (MessageBox.Show("기존 내용에 중복되는 회원번호, 날짜가 '" + ReCnt1 + "'개 있습니다." +
                    " \n 기존 내용이 엑셀내용으로 변환됩니다." +
                    " \n 계속 진행 하시겠습니까?", ""
                 , MessageBoxButtons.YesNo) == DialogResult.No) return false;
                if (Check_TextBox_Save_Error() == false) return false;
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                string Tsql2;
                Tsql2 = "select * from tbl_memberinfo_barcord  (nolock) where mbid2 = '" + nR["mbid2"] + "' and date = '" + nR["date"] + "'";
                ...
                if (ReCnt2 > 0)
                {
                    ...
                    string Tsql3;
                    Tsql3 = "update tbl_memberinfo_barcord ";
                    Tsql3 = Tsql3 + "set mbid2 = '" + nR["mbid2"] + "'  , date = '" + nR["date"] + "'";
                    Tsql3 = Tsql3 + " where mbid2 = '" + nR["mbid2"] + "' and date = '" + nR["date"] + "'";
         ...
         private Boolean Delete_Data_Copy()
         {
             ...
             try
             {
                 ...
                 StrSql = " DELETE FROM tbl_memberinfo_barcord ";
                 Temp_Connect.Insert_Data(StrSql, "tbl_memberinfo_barcord", Conn, tran, this.Text, this.Name);
            ...
            if ((sender as DataGridView).CurrentCell.ColumnIndex == 0)
            {
                ...
                || (T_DGv.CurrentCell.Value.ToString() == ""))
                {
                    ...
                    if (A == 0)
                    {
                        ...
                        mtxtDate.Text = T_DGv.CurrentRow.Cells[2].Value.ToString();
                        mtxtMbid_2.Text = T_DGv.CurrentRow.Cells[1].Value.ToString();
                        string Tsql = "";


                        Tsql = "Select b.m_name from tbl_memberinfo_barcord  (nolock)  a join tbl_memberinfo (nolock)  b on a.mbid2 = b.mbid2  where b.mbid2 = '" + mtxtMbid.Text + "'";
                   ...
                   // mtxtdate_2.Text = T_DGv.CurrentRow.Cells[2].Value.ToString();
            ...
            try
            {
                ...
                for (int i = 0; i < dGridView_Base.Rows.Count; i++)
                {
                    if (dGridView_Base.Rows[i].Cells[0].Value.ToString() == "V")
                    {
                        ...
                        date = dGridView_Base.Rows[i].Cells[2].Value.ToString().Replace("-","");

                        StrSql = " EXEC Usp_End_Memberinfo_barcord '" + mbid2 + "', '" + date + "', '" + cls_User.gid + "' ";
                        DataSet ds = new DataSet();
        ...
        private void butt_Save_Click(object sender, EventArgs e)
        {
            ...
            //중복값이면 업데이트
            string Tsql = "";
            Tsql = " SELECT * from tbl_memberinfo_barcord  (nolock) where mbid2 = '" + mtxtMbid.Text + "' and date = '" + mtxtDate.Text.Replace("-","") + "'";
            ...
            try
            {
                ...
                if (ReCnt > 0)
                {
                    MessageBox.Show("이미 해당 회원번호로 날짜가 등록 되어 있습니다.");
                    Base_Clear();
                    return;
                }
                ...
                {
                        string StrSql = "";
                        StrSql = " update  tbl_memberinfo_barcord set mbid2 = '" + mtxtMbid.Text + "', date = '" + mtxtDate.Text.Replace("-", "") + "' where mbid2= '" + mbid2_2 + "' AND date = '" + date_2 + "'";
                        Temp_Connect.Insert_Data(StrSql, "tbl_memberinfo_barcord", Conn, tran, this.Text, this.Name);
                ...
                else
                {
                    ...
                    StrSql = " insert into tbl_memberinfo_barcord values ('" + mtxtMbid.Text + "','" + mtxtDate.Text.Replace("-","") + "')";
                    Temp_Connect.Insert_Data(StrSql, "tbl_memberinfo_barcord", Conn, tran, this.Text, this.Name);
        ...
        private bool Base_Error_Check__01(int SellCode_TF = 0)
        {
            ...
            if (A > 1)
            {
                MessageBox.Show("다중선택시 수정 및 삽일을 할 수 없습니다.");
                return false;
            }
