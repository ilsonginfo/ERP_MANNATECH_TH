...
{
    public partial class frmSell_Select_SellTF_Cancel : clsForm_Extends
    {
        ...
        public frmSell_Select_SellTF_Cancel()
        {
            InitializeComponent();
        }
        ...
        private void Make_Base_Query(ref string Tsql)
        {
            ...
            Tsql = Tsql + " , tbl_SalesDetail.InsuranceNumber_Date ";
        ...
        private void Make_Base_Query_(ref string Tsql)
        {
            ...
            string strSql = " Where Ga_Order = 0  And tbl_SalesDetail.ReturnTF = 1 ";
            //string strSql = " Where Ga_Order = 0  And tbl_SalesDetail.ReturnTF = 1 ";

            strSql = strSql + " And tbl_SalesDetail.InsuranceNumber = '' ";  //공제번호발행이 안된내역
            strSql = strSql + " And  tbl_SalesDetail.OrderNumber NOT IN (Select Re_BaseOrderNumber From tbl_SalesDetail) "; //반품이나 교환의 대상이 안된 내역만 불러온다.
            strSql = strSql + " And  tbl_SalesDetail.Ga_Order_C_Time  <> '' ";  //가주문 승인난 내역은 여기에 승인 날짜를 넣는다.
        ...
        private Boolean Chang_CenterCode()
        {
            ...
            {
                //string T_Mbid = "";                 
                string OrderNumber = ""; string InsuranceNumber_Date = ""; //int Mbid2 = 0;
                ...
                for (int i = 0; i < dGridView_Base.Rows.Count; i++)
                {
                    if (dGridView_Base.Rows[i].Cells[0].Value.ToString() == "V")
                    {
                        ...
                        //StrSql = "Insert into tbl_SalesDetail_TF_Mod  ";
                        //StrSql = StrSql + " Select * ,'" + cls_User.gid + "',";
                        //StrSql = StrSql + " Convert(Varchar(25),GetDate(),21) ";
                        //StrSql = StrSql + " From tbl_SalesDetail_TF ";
                        //StrSql = StrSql + " Where OrderNumber ='" + OrderNumber + "'";

                        //Temp_Connect.Update_Data(StrSql, Conn, tran, this.Name.ToString(), this.Text);


                        //StrSql = "Update tbl_SalesDetail_TF SET ";
                        //StrSql = StrSql + "  SellTF = 0 ";
                        //StrSql = StrSql + " Where OrderNumber ='" + OrderNumber + "'";


                        //Temp_Connect.Update_Data(StrSql, Conn, tran, this.Name.ToString(), this.Text);
                        
                        InsuranceNumber_Date = dGridView_Base.Rows[i].Cells[8].Value.ToString(); //승인내역중에 판매일자 변동이 있엇던 것이 있으면 원래 날짜로 돌려 놓기 위함.
                                                
                        StrSql = "Update tbl_SalesDetail SET ";
                        StrSql = StrSql + " SellDate = Ga_Order_BaseDate";
                        StrSql = StrSql + " , Ga_Order_BaseDate = '' ";                            
                        StrSql = StrSql + " , Ga_Order_C_Time =  Convert(Varchar(25),GetDate(),21) ";
                        StrSql = StrSql + " , Ga_Order_C_ID ='" + cls_User.gid + "'"; 
                        StrSql = StrSql + " , Ga_Order = 101 "; //승인되었다가 취소된 내역은  101을 넣는다
                        StrSql = StrSql + " Where OrderNumber ='" + OrderNumber + "'";
