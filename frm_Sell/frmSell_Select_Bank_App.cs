...
namespace MLM_Program
{
    public partial class frmSell_Select_Bank_App : Form
    {
        ...
        public frmSell_Select_Bank_App()
        {
            InitializeComponent();
        }
        ...
        private void Make_Base_Query(ref string Tsql)
        {
            ...
            Tsql = " Select '', " ;
            Tsql = Tsql + " tbl_SalesDetail.OrderNumber,tbl_Sales_Cacu.C_index  ";
            ...
            //Tsql = Tsql + ", C_Name1 , C_ETC , C_Cash_Number2 , C_Cash_Send_Nu , '' ";
            Tsql = Tsql + ", C_Name1 , C_ETC , C_Cash_Number , C_Cash_Send_Nu , '' ";
            ...
            //Tsql = Tsql + " LEFT JOIN tbl_Sales_Cacu_Bank  (nolock) ON tbl_Sales_Cacu_Bank.OrderNumber = tbl_Sales_Cacu.OrderNumber And tbl_Sales_Cacu_Bank.C_index =tbl_Sales_Cacu.C_index     ";
            //Tsql = Tsql + " And Seqno IN (Select Max(Seqno) From tbl_Sales_Cacu_Bank (nolock) Where CanCel_Flag ='A' Group by OrderNumber, C_index   ) "; 
        ...
        private void Make_Base_Query_(ref string Tsql)
        {
            ...
            //if (rdo_Y.Checked == true)
            //    strSql = strSql + " And tbl_Sales_Cacu.C_Cash_Number <> '' ";  //KSnet 거래번호가 있는 거는 현금영수증이 발행 되엇다는 거임.
            //else
            //    strSql = strSql + " And tbl_Sales_Cacu.C_Cash_Number = '' ";  //KSnet 거래번호가 없는 거는 현금영수증이 발행안되었다느 거임.

            strSql = strSql + " And tbl_Sales_Cacu.C_Cash_Number <> '' ";  //현금영수증 거래 번호가 잇는 거만 불러온다.
            strSql = strSql + " AND tbl_Sales_Cacu.C_CancelTF <> '1'"; //2018-09-11 지성경 추가 현금영수증이 이지 취소되어있는것은 가져오지말아야한다.
            //strSql = strSql + " And tbl_Sales_Cacu.OrderNumber +'-' + Convert(varchar, tbl_Sales_Cacu.C_index ) ";
            //strSql = strSql + " IN (Select OrderNumber +'-' + Convert(varchar, C_index )  From tbl_Sales_Cacu_Bank (nolock) Where C_Number3 <> '' ) ";


            //strSql = strSql + " And tbl_Sales_Cacu.C_Price1 > 0 ";

            //strSql = strSql + " And tbl_Sales_Cacu.C_Cash_Number2 <> '' ";
            //strSql = strSql + " And tbl_Sales_Cacu.C_Cash_Send_Nu <> '' ";
            //strSql = strSql + " And tbl_Sales_Cacu.C_Cash_Send_TF > 0 ";
            ...
            try
            {
                ...
                for (int i = 0; i < dGridView_Base.Rows.Count; i++)
                {
                    if (dGridView_Base.Rows[i].Cells[0].Value.ToString() == "V")
                    {
                        ...
                        //C_Number3 = dGridView_Base.Rows[i].Cells[13].Value.ToString();

                        web.Dir_VR_Cash_Receipt_All_Cancel(OrderNumber, C_index);
