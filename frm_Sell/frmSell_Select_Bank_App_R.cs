...
{
    public partial class frmSell_Select_Bank_App_R : clsForm_Extends
    {
        ...
        public frmSell_Select_Bank_App_R()
        {
            InitializeComponent();
        }
        ...
        private void Make_Base_Query(ref string Tsql)
        {
            ...
            /*
            string[] g_HeaderText = {"선택","주문번호"  , "C_Index" , "주문_일자"   , "회원_번호"     
                                , "성명"     , "결제금액"    , "결제일자"  , "은행명"   , "계좌가명"    
                                , "계좌번호"    , "입금자명"    , "비고"   , "신고번호"  , "핸드폰"  
                                , "현금영수증신청번호"     , "개인/사업자"    , "TempCol1"  , "TempCol2"     , "TempCol3"                            
                                    };

            string[] g_Cols = {"Selected", "OrderNumber"  , "C_Index" , "SellDate"   , "mbid2"     
                                , "m_name"     , "price"    , "PaymentDate"  , "Bankname"   , "accName_VR"    
                                , "accNumber"    , "DepositName"    , "ETC"   , "SingoNumber"  , "hpTel"  
                                , "Receipt_Num"     , "SingoGubun"     , "TempCol1"  , "TempCol2"     , "TempCol3"                           
                                    };
            ...
            Tsql = Tsql + ", tbl_Bank.BankName , C_codeName , C_Number1   ";
            Tsql = Tsql + ", C_Name1 , C_ETC , C_Cash_Send_Nu , tbl_Memberinfo.Hptel ";

            Tsql = Tsql + " ,C_Cash_Send_Nu ,C_Cash_Send_TF,'','','' ";        
            ...
            //Tsql = Tsql + " LEFT JOIN tbl_Bank  (nolock) ON tbl_Bank.Ncode =tbl_Sales_Cacu.C_Code  And  tbl_Bank.Na_Code =tbl_SalesDetail.Na_Code ";
            Tsql = Tsql + " LEFT JOIN tbl_Bank  (nolock) ON tbl_Bank.Ncode =tbl_Sales_Cacu.C_Code ";
            cls_NationService.SQL_BankNationCode(ref Tsql);
            Tsql = Tsql + " LEFT JOIN tbl_Tax (nolock) ON  tbl_SalesDetail.Mbid = tbl_Tax.Mbid ";
        ...
        private void Make_Base_Query_(ref string Tsql)
        {
            ...
            if (opt_Ed_1.Checked == true )
                ...
                strSql = " Where tbl_Sales_Cacu.C_TF = 2  ";
            else if (opt_Ed_4.Checked == true)
                strSql = " Where tbl_Sales_Cacu.C_TF = 4  ";
            else if (opt_Ed_6.Checked == true)
                strSql = " Where tbl_Sales_Cacu.C_TF = 5  ";  //가상계좌를 불러온다.
            else
                strSql = " Where tbl_Sales_Cacu.C_TF <> 3  ";  //카드 이외에는 다 불러온다





            strSql = strSql + " And tbl_Sales_Cacu.C_Cash_Number = '' ";  //KSnet 거래번호가 없는 거는 현금영수증이 발행안되었다느 거임.

            //strSql = strSql + " And tbl_Sales_Cacu.OrderNumber +'-' + Convert(varchar, tbl_Sales_Cacu.C_index ) ";
            //strSql = strSql + " NOT IN (Select OrderNumber +'-' + Convert(varchar, C_index )  From tbl_Sales_Cacu_Bank (nolock) Where rStatus = 'O' ) ";

            strSql = strSql + " And (C_Cash_Sort_TF = 0 OR C_Cash_Sort_TF = 1) ";
            //strSql = strSql + " And (C_Cash_Sort_TF = 1) ";
            ...
            if (txtName.Text.Trim() != "")
                strSql = strSql + " And tbl_Sales_Cacu.C_Name2 Like '%" + txtName.Text.Trim() + "%'";
            ...
            if (txtBank_Code.Text.Trim() != "")
                strSql = strSql + " And tbl_Sales_Cacu.C_Code = '" + txtBank_Code.Text.Trim() + "'";

            if (txt_C_Bank.Text.Trim() != "")
                strSql = strSql + " And tbl_BankForCompany.BankPenName = '" + txt_C_Bank.Text.Trim() + "'";
            ...
            if (radioB_S2.Checked == true)
                strSql = strSql + " And tbl_Memberinfo.Sell_Mem_TF = 0 ";

            if (radioB_S3.Checked == true)
        ...
        private void Base_Button_Click(object sender, EventArgs e)
        {
            ...
            if (bt.Name == "butt_Clear")
            {
                ...
                opt_Ed_5.Checked = true; opt_sell_1.Checked = true;
                rdo_Y.Checked = true;
...
string[] g_Cols = {"Selected", "OrderNumber"  , "C_Index" , "SellDate"   , "mbid2"     
                ...
                */
                ...
                {
                    if (dGridView_Base.Rows[i].Cells["Selected"].Value.ToString() == "V")
                    {
                        OrderNumber = dGridView_Base.Rows[i].Cells["OrderNumber"].Value.ToString();
                        C_index = int.Parse(dGridView_Base.Rows[i].Cells["C_Index"].Value.ToString());
                        Send_Number = dGridView_Base.Rows[i].Cells["Receipt_Num"].Value.ToString().Replace("-", "").Replace(" ", "");

                        web.Dir_Cash_Receipt_Approve(OrderNumber, C_index, Send_Number);
                    }
        ...
        private Boolean Check_TextBox_Save_Error()
        {
            ...
            //    cls_Search_DB csd = new cls_Search_DB();
            //    if (csd.Close_Check_SellDate("tbl_CloseTotal_04", Min_SellDate.Trim()) == false)
            //    {
